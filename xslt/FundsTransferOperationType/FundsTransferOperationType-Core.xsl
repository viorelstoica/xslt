...
<xsl:stylesheet xmlns="http://www.odcgroup.com/InfraPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/FundsTransferOperationType" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiFundsTransferOperationType" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="b c" version="1.0">
  <xsl:template name="code">
    <code>
      <xsl:choose>
        <xsl:when test="$POSTFIX_FTOPTYPE = 'Y' ">
          <xsl:value-of select="concat('FT_',b:id,$COMPANY_POSTFIX_SEPARATOR,b:mnemonic)"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="concat('FT_',b:id)"/>
        </xsl:otherwise>
      </xsl:choose>
    </code>
  </xsl:template>
  ...
  <xsl:template name="multilingualLongName">
    ...
    <xsl:call-template name="OldNewDenomManagment">
      ...
      <xsl:with-param name="PreviousEvent" select="../../c:PreviousEvent/b:FundsTransferOperationType/b:descriptionList/b:description"/>
    </xsl:call-template>
  </xsl:template>
