...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PositionTransferScSettlement"
	...
	<xsl:template match="b:PositionTransferScSettlement" mode="filter1">
		<xsl:apply-templates select="self::b:PositionTransferScSettlement[((not(b:depositoryFrom) or b:depositoryFrom = '') and (not(b:depositoryTo) or b:depositoryTo = '')) or (b:depositoryFrom and b:depositoryFrom != '' and b:depositoryTo and b:depositoryTo != '' and (b:depositoryFrom = b:depositoryTo))]"
		                     mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -->
	<xsl:template match="b:PositionTransferScSettlement" mode="filter2">
		<xsl:apply-templates select="self::b:PositionTransferScSettlement[b:secAcctFrom and b:secAcctFrom != '']" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
	<xsl:template match="b:PositionTransferScSettlement" mode="filter3">
		<xsl:apply-templates select="self::b:PositionTransferScSettlement[b:secAcctTo and b:secAcctTo != '']" mode="filter4"/>
	</xsl:template>
	<!-- Condition 5 -->
	<xsl:template match="b:PositionTransferScSettlement" mode="filter4">
		<xsl:apply-templates select="self::b:PositionTransferScSettlement[b:secAcctTo != b:secAcctFrom]" mode="filter5"/>
	</xsl:template>
	...
	<xsl:template match="b:PositionTransferScSettlement" mode="filter5">
		<xsl:variable name="checkPortfolioConditionVar">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'PTPtfToPtfInvActualSettlement'"/>
	...
	<xsl:template match="b:PositionTransferScSettlement" mode="filter6">
		<xsl:variable name="checkPortfolioConditionVar1">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'PTPtfToPtfInvActualSettlement1'"/>
	...
	<xsl:template match="b:PositionTransferScSettlement" mode="filter9">
		<xsl:apply-templates select="self::b:PositionTransferScSettlement[contains(concat(' ',$sectPendTransNamesInSupportedTypes,' '),concat(' ',b:transCode,' ')) or contains(concat(' ',$securityTransferInSupportedTypes,' '),concat(' ',b:transCode,' '))]"
		                     mode="filter10"/>
	</xsl:template>
	<!-- Condition 11 -->
