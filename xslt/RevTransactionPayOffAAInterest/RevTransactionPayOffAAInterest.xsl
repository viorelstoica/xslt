   ...
   <xsl:template match="c:AAAFlow">
      <TransactionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('RevTransactionPayOffAAInterest',c:aatxncontext/b:aaArrangementId)"/>
         </xsl:attribute>
         <DataGroup>
				<xsl:call-template name="mappingRevTransactionPayOffAAInterest"/>
		 </DataGroup>
      </TransactionPMS>
   </xsl:template>
   <xsl:template name="mappingRevTransactionPayOffAAInterest">
      <xsl:for-each select="c:aaarecord">
         <DepositLoan>
            <xsl:attribute name="description">
               <xsl:value-of select="concat('RevTransactionPayOffAAInterest.xsl+', name(/*), '+', ../c:eventCommon/ns0:eventId, '+', ../c:eventCommon/ns0:creationTime)"/>
            </xsl:attribute>
