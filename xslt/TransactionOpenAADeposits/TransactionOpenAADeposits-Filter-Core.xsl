...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:a="http://www.temenos.com/T24/AaActivityExtractorService/AaArrInterestRec" xmlns:b="http://www.temenos.com/T24/AaActivityExtractorService/AaTransactionContext" xmlns:c="http://www.temenos.com/T24/event/AAAExtractor/AAAFlow" xmlns:d="http://www.temenos.com/T24/AaActivityExtractorService/AaArrActivityRec" xmlns:e="http://www.temenos.com/T24/AaActivityExtractorService/AaProductRec" xmlns:f="http://www.temenos.com/T24/AaActivityExtractorService/AaAccountDetailsRec" xmlns:g="http://www.temenos.com/T24/AaActivityExtractorService/AaArrAccountRec" xmlns:h="http://www.temenos.com/T24/AaActivityExtractorService/AaArrSettlementRec" xmlns:i="http://www.temenos.com/T24/AaActivityExtractorService/AaArrengementRec" xmlns:j="http://www.temenos.com/T24/AaActivityExtractorService/AaBillRec" xmlns:k="http://www.temenos.com/T24/AaActivityExtractorService/AaIntAccrualRec" xmlns:l="http://www.temenos.com/T24/AaActivityExtractorService/AaFtRecord" version="1.0" exclude-result-prefixes="a b c d e f g h i j k l">
  ...
  <xsl:template match="c:AAAFlow">
    <xsl:apply-templates select="self::c:AAAFlow[c:aaarecord/d:activity = 'DEPOSITS-APPLYPAYMENT-PR.DEPOSIT' or c:aaarecord/d:activity = 'DEPOSITS-APPLYPAYMENT-PR.EXPECTED' or (c:aaarecord/d:activity = 'DEPOSITS-NEW-ARRANGEMENT' and $AA_INTERFACE_INPUT_DEALS = 'Y')]" mode="filter1"/>
  </xsl:template>
  <!-- Condition 2 -->
  <xsl:template match="c:AAAFlow" mode="filter1">
    <xsl:apply-templates select="self::c:AAAFlow[ ( starts-with(c:aaarecord/d:txnSystemId,'AA') and (c:aatxncontext/b:aaMasterActivity = 'DEPOSITS-NEW-ARRANGEMENT' or c:aatxncontext/b:aaMasterActivity = 'DEPOSITS-MAKEDUE-SCHEDULE') ) or c:aaarecord/d:txnSystemId = 'FT' or (not(c:aaarecord/d:txnSystemId) and $AA_INTERFACE_INPUT_DEALS = 'Y')]" mode="filter2"/>
  </xsl:template>
  <!-- Condition 3 -->
  <xsl:template match="c:AAAFlow" mode="filter2">
    <xsl:apply-templates select="self::c:AAAFlow[c:accountrecord/g:portfolioId]" mode="filter3"/>
  </xsl:template>
  <!-- Condition 4 -->
  <xsl:template match="c:AAAFlow" mode="filter3">
    <xsl:apply-templates select="self::c:AAAFlow[ not(c:aatxncontext/b:aaActivityStatus) or c:aatxncontext/b:aaActivityStatus != 'REV' ]" mode="filter4"/>
  </xsl:template>
  <!-- Condition 5 -->
  <xsl:template match="c:AAAFlow" mode="filter4">
    <xsl:variable name="var_rpyBillId" select="c:recaccountdetails/f:actRepayment[starts-with(f:repayReference,../c:aatxncontext/b:aaArrActivityId)]/f:rpyBillId"/>
    <xsl:apply-templates select="self::c:AAAFlow[(c:aaarecord/d:activity != 'DEPOSITS-NEW-ARRANGEMENT' and c:billdetails[j:aaBillId = $var_rpyBillId]/j:billProperyDetails[j:property = 'ACCOUNT']) or c:aaarecord/d:activity = 'DEPOSITS-NEW-ARRANGEMENT']" mode="filter5"/>
  </xsl:template>
  <!-- Condition 6 -->
  <xsl:template match="c:AAAFlow" mode="filter5">
    ...
    <xsl:apply-templates select="self::c:AAAFlow[ $custoCondition = '1']" mode="filter-custo"/>
  </xsl:template>
  <!-- Custo placeholder -->
