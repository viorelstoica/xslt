...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/NdDeal"
	...
	<xsl:template match="b:NdDeal">
		<xsl:apply-templates select="self::b:NdDeal[contains($list_Status_FinancialInstrumentNdForward,concat(' ',b:status,' '))]" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	...
	<xsl:template match="b:NdDeal" mode="filter3">
		<xsl:apply-templates select="self::b:NdDeal[not(b:dealerBook)]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
