...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionOpenDerivativeBuyReversal"
	...
	<xsl:template match="c:MultiTransactionOpenDerivativeBuyReversal">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionOpenDerivativeBuyReversal"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionOpenDerivativeBuyReversal">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionOpenDerivativeBuyReversal" name="BatchTransactionOpenDerivativeBuyReversal">
			<xsl:apply-templates select="b:TransactionOpenDerivativeBuyReversal"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionOpenDerivativeBuyReversal">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionOpenDerivativeBuyReversal" name="BatchMultiTransactionOpenDerivativeBuyReversal">
			<xsl:apply-templates select="c:MultiTransactionOpenDerivativeBuyReversal"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionOpenDerivativeBuyReversal">
		<TransactionPMS mode="INSUPD">
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionOpenDerivativeBuyReversal',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Derivative>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionOpenDerivativeBuyReversal.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
