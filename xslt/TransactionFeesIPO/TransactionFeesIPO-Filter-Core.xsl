...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SecOpenOrder" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSecOpenOrder" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiSecOpenOrder"
	...
	<xsl:template match="b:SecOpenOrder" mode="filter1">
		<xsl:apply-templates select="self::b:SecOpenOrder[b:securityMasterBlockType != '']" mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -->
	<xsl:template match="b:SecOpenOrder" mode="filter2">
		<xsl:apply-templates select="self::b:SecOpenOrder[b:scStdSecTradeChargeTime = 'ORDER']" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
	<xsl:template match="b:SecOpenOrder" mode="filter3">
		<xsl:apply-templates select="self::b:SecOpenOrder[b:dealerBookSecAccMasterJoinList/b:dealerBookSecAccMasterJoin = '' or not(b:dealerBookSecAccMasterJoinList/b:dealerBookSecAccMasterJoin)]" mode="filter4"/>
	</xsl:template>
	<!-- Condition 5 -->
	<xsl:template match="b:SecOpenOrder" mode="filter4">
		<xsl:apply-templates select="self::b:SecOpenOrder[b:memoAccountSecAccMasterJoinList/b:memoAccountSecAccMasterJoin = 'NO']" mode="filter5"/>
	</xsl:template>
	<!-- Condition 6 -->
	<xsl:template match="b:SecOpenOrder" mode="filter5">
		<xsl:apply-templates select="self::b:SecOpenOrder[b:customerTypeCustomerSecurityJoinGroup/b:customerTypeCustomerSecurityJoinList/b:customerTypeCustomerSecurityJoin = 'CUSTOMER']" mode="filter6"/>
	</xsl:template>
	<!-- Condition 7 -->
	<xsl:template match="b:SecOpenOrder" mode="filter6">
		<xsl:apply-templates select="self::b:SecOpenOrder[
		      ((b:securityMasterBlockType = 'BLOCK' or (b:securityMasterBlockType = 'DEBIT' and b:securityMasterDebitTime = 'IMMEDIATELY')) and b:currNo = '1') 
		      or b:eventCommon/ns0:application = 'SC.IPO.ORDER.ACCTG'
		      ]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
