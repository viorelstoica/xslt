...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionCoExerciseManual"
	...
	<xsl:template match="b:TransactionCoExerciseManual">
		...
		<xsl:apply-templates select="self::b:TransactionCoExerciseManual[$opStatus != 'REVERSE-AUTHORISE']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:TransactionCoExerciseManual" mode="filter1">
		<xsl:apply-templates select="self::b:TransactionCoExerciseManual[b:unauthAuth = 'AUTHORISED']" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:TransactionCoExerciseManual" mode="filter-custo">
		<xsl:apply-templates select="self::b:TransactionCoExerciseManual" mode="filter-final"/>
	</xsl:template>
	<!-- Copy block -->
	<xsl:template match="b:TransactionCoExerciseManual" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="b:closeoutIdList/b:closeoutId" mode="filter1">
		...
		<xsl:apply-templates select="self::node()[../../b:memoAccountList/b:memoAccount[@index=$index]  = 'NO']" mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -->
	<xsl:template match="b:closeoutIdList/b:closeoutId" mode="filter2">
		<xsl:variable name="index" select="@index"/>
		<xsl:apply-templates select="self::node()[not(../../b:dealerBookList/b:dealerBook[@index=$index])]" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
	<xsl:template match="b:closeoutIdList/b:closeoutId" mode="filter3">
		<xsl:variable name="index" select="@index"/>
		<xsl:apply-templates select="self::node()[../../b:customerTypeGroup[@index=$index]/b:customerTypeList/b:customerType = 'CUSTOMER']" mode="filter4"/>
	</xsl:template>
	<!-- Condition 5 -->
