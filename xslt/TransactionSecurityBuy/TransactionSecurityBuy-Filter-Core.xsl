...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionSecurityBuy" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionSecurityBuy"
                xmlns:c="http://www.temenos.com/T24/event/TTI/MultiTransactionSecurityBuy" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSecurityBuy"
	...
	<xsl:template match="b:TransactionSecurityBuy">
		...
		<xsl:apply-templates select="self::b:TransactionSecurityBuy[$opStatus = 'INPUT-AUTHORISE']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:TransactionSecurityBuy" mode="filter1">
		<xsl:apply-templates select="self::b:TransactionSecurityBuy[($g_filterBySubAssetType='Y' and b:subAssetTypeJoin and contains ($list_SubAssetType_TransactionSecurityBuy,b:subAssetTypeJoin)) or
	...
	<!-- Condition 3 -->
	<xsl:template match="b:TransactionSecurityBuy" mode="filter2">
			...
			<xsl:for-each select="b:customerNoGroup/b:custSecAcc">
				...
				<xsl:variable name="checkPortfolioConditionVar">
					<xsl:call-template name="checkPortfolioCondition">
						<xsl:with-param name="xsltName" select="'TransactionSecurityBuy'"/>
		...
		<xsl:apply-templates select="self::b:TransactionSecurityBuy[contains($ifFilterWholeRecord,'Y')]" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
	<xsl:template match="b:TransactionSecurityBuy" mode="filter3">
		<xsl:apply-templates select="self::b:TransactionSecurityBuy[(b:parent != 'YES' or not(b:parent))]" mode="filter-custo"/>
	</xsl:template>		
	<!-- Custo placeholder -->
	<xsl:template match="b:TransactionSecurityBuy" mode="filter-custo">
		<xsl:apply-templates select="self::b:TransactionSecurityBuy" mode="filter-final"/>
	</xsl:template>
	<!-- Copy block -->
	<xsl:template match="b:TransactionSecurityBuy" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="b:customerNoGroup/b:custSecAcc" mode="filter1">
		<xsl:variable name="index" select="../@index"/>
		<xsl:apply-templates select="self::node()[contains($list_TransCode_TransactionSecurityBuy,../b:custTransCode) or (contains($peFundsTransCodeBuy,../b:custTransCode) and ../../b:peFund= 'YES' and ../../b:peType = 'CAPITAL CALL')]" mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -->
	<xsl:template match="b:customerNoGroup/b:custSecAcc" mode="filter2">
		<xsl:variable name="index" select="../@index"/>
		<xsl:apply-templates select="self::node()[not(contains($sectPendTransNamesInSupportedTypes,../b:custTransCode))]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
