...
<xsl:stylesheet xmlns="http://www.temenos.com/T24/OFSML/150" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:b="http://www.odcgroup.com/F2B" xmlns:infra="http://www.odcgroup.com/InfraPMS" exclude-result-prefixes="b" version="1.0">
	...
	<xsl:template match="b:Request">
		<xsl:apply-templates select="self::b:Request[(b:userDefinedField[infra:name='ud_instr_nature_e']/infra:value = '13') and (b:userDefinedField[infra:name='ud_instr_traspasos_f']/infra:value = '1') and (b:buySellIndicator = '4')]" mode="filterMB-custo"/>
	</xsl:template>

	<!-- Custo placeholder -->
	<xsl:template match="b:Request" mode="filterMB-custo">
		<xsl:apply-templates select="self::b:Request" mode="filterMB-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:Request" mode="filterMB-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>

	<!-- Filter out whole input xml(i.e., root element F2B) if no ordercancelletion passes the above filter -->
