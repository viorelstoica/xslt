...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/ClientPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/Portfolio"
	...
	<xsl:template match="c:MultiPortfolio">
		<xsl:apply-templates select="c:CurrentEvent/b:Portfolio"/>
	</xsl:template>
	<xsl:template match="batch:BatchPortfolio">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchPortfolio" name="BatchPortfolio">
			<xsl:apply-templates select="b:Portfolio"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiPortfolio">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiPortfolio" name="BatchMultiPortfolio">
			<xsl:apply-templates select="c:MultiPortfolio"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:Portfolio">
		<ClientPMS  processDeletes="first">
			<xsl:attribute name="mode">INSUPD</xsl:attribute>
			<xsl:attribute name="output">
				<xsl:value-of select="concat('portfolio',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Portfolio>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('Portfolio.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
