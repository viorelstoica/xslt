...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/ClientPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/CloseUpfrontPaymentOrder"
	...
	<xsl:template match="b:CloseUpfrontPaymentOrder">
		<xsl:apply-templates select="self::b:CloseUpfrontPaymentOrder[b:customerNoGroup/b:custTransCode = 'UPS' and b:customerNoGroup/b:cuOrderNosList/b:cuOrderNos != '']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:CloseUpfrontPaymentOrder" mode="filter1">
    	<xsl:apply-templates select="self::b:CloseUpfrontPaymentOrder[b:eventCommon/ns0:transactionStage = 'INPUT-AUTHORISE']" mode="filter-custo"/>
  	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:CloseUpfrontPaymentOrder" mode="filter-custo">
		<xsl:apply-templates select="self::b:CloseUpfrontPaymentOrder" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:CloseUpfrontPaymentOrder" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
