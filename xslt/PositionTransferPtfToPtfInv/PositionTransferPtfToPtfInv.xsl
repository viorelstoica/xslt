...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PositionTransferPtfToPtfInv"
	...
	<xsl:template match="c:MultiPositionTransferPtfToPtfInv">
		<xsl:apply-templates select="c:CurrentEvent/b:PositionTransferPtfToPtfInv"/>
	</xsl:template>
	<xsl:template match="batch:BatchPositionTransferPtfToPtfInv">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchPositionTransferPtfToPtfInv" name="BatchPositionTransferPtfToPtfInv">
			<xsl:apply-templates select="b:PositionTransferPtfToPtfInv"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiPositionTransferPtfToPtfInv">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiPositionTransferPtfToPtfInv" name="BatchMultiPositionTransferPtfToPtfInv">
			<xsl:apply-templates select="c:MultiPositionTransferPtfToPtfInv"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:PositionTransferPtfToPtfInv">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('positionTransferPtfToPtfInv',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<xsl:for-each select="b:securityNoGroup">
					<Security>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('PositionTransferPtfToPtfInv.xsl+', name(/*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
