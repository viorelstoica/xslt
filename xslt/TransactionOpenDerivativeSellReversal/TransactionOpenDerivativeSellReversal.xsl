...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionOpenDerivativeSellReversal"
	...
	<xsl:template match="c:MultiTransactionOpenDerivativeSellReversal">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionOpenDerivativeSellReversal"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionOpenDerivativeSellReversal">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionOpenDerivativeSellReversal" name="BatchTransactionOpenDerivativeSellReversal">
			<xsl:apply-templates select="b:TransactionOpenDerivativeSellReversal"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionOpenDerivativeSellReversal">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionOpenDerivativeSellReversal" name="BatchMultiTransactionOpenDerivativeSellReversal">
			<xsl:apply-templates select="c:MultiTransactionOpenDerivativeSellReversal"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionOpenDerivativeSellReversal">
		<TransactionPMS mode="INSUPD">
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionOpenDerivativeSellReversal',b:id)"/>
			</xsl:attribute>
			...
			<DataGroup>
				<Derivative>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionOpenDerivativeSellReversal.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					...
					<sale>
						...
						<xsl:call-template name="hedgeTrade"/>
						<xsl:call-template name="orderTypeD"/>
