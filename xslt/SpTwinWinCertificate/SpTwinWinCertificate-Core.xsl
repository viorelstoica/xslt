...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch ns0 d e f g" version="1.0" xmlns="http://www.odcgroup.com/StructuredProducts" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template name="participationLevelDownSide">
		<xsl:if test="b:downsideParticipation and b:downsideParticipation != ''">
			<participationLevelDownSide>
				<xsl:value-of select="b:downsideParticipation"/>
			</participationLevelDownSide>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="participationLevelDownSide_MultiEntity"/>
