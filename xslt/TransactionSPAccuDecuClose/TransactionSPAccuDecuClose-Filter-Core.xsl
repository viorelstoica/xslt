...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SyAccuDecu" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSyAccuDecu" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiSyAccuDecu" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiSyAccuDecu" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
  ...
  <xsl:template match="b:SyAccuDecu">
    <xsl:apply-templates select="self::b:SyAccuDecu[b:productType = 'ACCUMULATOR' or b:productType = 'DECUMULATOR']" mode="filter1"/>
  </xsl:template>
  <!-- Condition 2 -->
  <xsl:template match="b:SyAccuDecu" mode="filter1">
    <xsl:apply-templates select="self::b:SyAccuDecu[b:contractStatus = 'UNWOUND' or b:contractStatus = 'MATURED' or b:contractStatus = 'KNOCKOUT']" mode="filter2"/>
  </xsl:template>
  <!-- Condition 3 -->
  <xsl:template match="b:SyAccuDecu" mode="filter2">
    <xsl:apply-templates select="self::b:SyAccuDecu[b:knockOut='YES' or not(b:knockOut)]" mode="filter3"/>
  </xsl:template>
  <!-- Condition 4 -->
  <xsl:template match="b:SyAccuDecu" mode="filter3">
    <xsl:apply-templates select="self::b:SyAccuDecu[b:unwindGroup/b:unwind='YES' or b:unwindGroup/b:unwind='FULL' or not(b:unwindGroup/b:unwind)]" mode="filter4"/>
  </xsl:template>
  <!-- Condition 5 -->
  <xsl:template match="b:SyAccuDecu" mode="filter4">
    ...
    <xsl:apply-templates select="self::b:SyAccuDecu[number($check_MaturityDate) &gt;= number($check_toDay)]" mode="filter5"/>
  </xsl:template>
  <!-- Condition 6 -->
  <xsl:template match="b:SyAccuDecu" mode="filter5">
    <xsl:apply-templates select="self::b:SyAccuDecu[b:portfolio != '']" mode="filter6"/>
  </xsl:template>
  <!-- Condition 7 -->
  <xsl:template match="b:SyAccuDecu" mode="filter6">
    <xsl:apply-templates select="self::b:SyAccuDecu[b:dealerBook = '' or not(b:dealerBook)]" mode="filter7"/>
  </xsl:template>
  <!-- Condition 8 -->
  <xsl:template match="b:SyAccuDecu" mode="filter7">
    <xsl:apply-templates select="self::b:SyAccuDecu[b:memoAccount = 'NO']" mode="filter-custo"/>
  </xsl:template>
  <!-- Custo placeholder -->
