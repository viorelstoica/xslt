   ...
   <xsl:template match="b:FinancialInstrumentSecurity">
      <FiPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FinancialInstrumentMutualFund-Partial',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <FundShare>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('FinancialInstrumentMutualFund-Partial.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
