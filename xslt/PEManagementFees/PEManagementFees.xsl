...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PEManagementFees" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchPEManagementFees"
	...
	<xsl:template match="c:MultiPEManagementFees">
		<xsl:apply-templates select="c:CurrentEvent/b:PEManagementFees"/>
	</xsl:template>
	<xsl:template match="batch:BatchPEManagementFees">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchPEManagementFees" name="BatchPEManagementFees">
			<xsl:apply-templates select="b:PEManagementFees"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiPEManagementFees">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiPEManagementFees" name="BatchMultiPEManagementFees">
			<xsl:apply-templates select="c:MultiPEManagementFees"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:PEManagementFees">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('PEManagementFees',b:id)"/>
			</xsl:attribute>
				<DataGroup>
					<Security>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('PEManagementFees.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
