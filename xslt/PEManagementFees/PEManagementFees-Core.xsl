...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PEManagementFees" xmlns:infra="http://www.odcgroup.com/InfraPMS"
	...
	<xsl:template name="transactionFeesTaxesCounter">	
		<transactionFeesTaxesCounter>
			<xsl:choose>
				<xsl:when test="b:mgmtFees and b:mgmtFees != ''">
					<amount>
						<xsl:value-of select="b:mgmtFees"/>
					</amount>
				</xsl:when>
				<xsl:otherwise>
					<amount>
	...
	<xsl:template name="transactionCode">
		<transactionCode>
			<xsl:value-of select="concat(b:id,$COMPANY_POSTFIX_SEPARATOR,b:custPort,$COMPANY_POSTFIX_SEPARATOR,'Mgmt',$COMPANY_POSTFIX_SEPARATOR,b:mnemonic)"/>
		</transactionCode>
	</xsl:template>
	...
	<xsl:template name="tradeDate">
		...
		<xsl:call-template name="xsdate">
			<xsl:with-param name="date" select="substring-after(substring-after(b:id,'.'),'.')"/>
		</xsl:call-template>
		</tradeDate>
	</xsl:template>
	<xsl:template name="portfolio">
		<portfolio>
			<infra:code>
				<xsl:value-of select="b:custPort"/>
			</infra:code>
		</portfolio>
	</xsl:template>
