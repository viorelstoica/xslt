...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/STTransferInActualSettlement" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSTTransferInActualSettlement" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiSTTransferInActualSettlement" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiSTTransferInActualSettlement" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/>
   <xsl:template match="c:MultiSTTransferInActualSettlement">
      <xsl:apply-templates select="c:CurrentEvent/b:STTransferInActualSettlement"/>
   </xsl:template>
   <xsl:template match="batch:BatchSTTransferInActualSettlement">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchSTTransferInActualSettlement" name="BatchSTTransferInActualSettlement">
         <xsl:apply-templates select="b:STTransferInActualSettlement"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiSTTransferInActualSettlement">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiSTTransferInActualSettlement" name="BatchMultiSTTransferInActualSettlement">
         <xsl:apply-templates select="c:MultiSTTransferInActualSettlement"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:STTransferInActualSettlement">
      <TransactionPMS>
         <xsl:attribute name="mode">INSUPD</xsl:attribute>
         <xsl:attribute name="output">
            <xsl:value-of select="concat('STTransferInActualSettlement',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <Security>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('STTransferInActualSettlement.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
...
<metaInformation>
	<scenarios>
		<scenario default="yes" name="Scenario1" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\STTransferInActualSettlement\STTransferInActualSettlement.xml" htmlbaseurl=""
		          outputurl="..\..\TestFiles\STTransferInActualSettlement\STTransferInActualSettlementGWP.xml" processortype="jaxp" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath=""
