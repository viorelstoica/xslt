	...
	<xsl:template match="b:CginventoryAll">
		<InfraPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FtrateAll',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<FtRate>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FtrateAll.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					...
					<xsl:call-template name="discountFactor"/>
					<xsl:call-template name="ftConvention"/>
					<xsl:call-template name="instrList"/>
					<xsl:call-template name="longCapTaxRate"/>
					<xsl:call-template name="longTermPeriod"/>
					<xsl:call-template name="longTermPeriodUnit"/>
					<xsl:call-template name="rank"/>
					<xsl:call-template name="shortCapTaxRate"/>
				</FtRate>
			</DataGroup>
		</InfraPMS>
	</xsl:template>
