...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch ns0 d e f g" version="1.0" xmlns="http://www.odcgroup.com/StructuredProducts" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template match="b:FinancialInstrumentSecurity">
		<StructuredProducts>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('SpRevConvBondLinked',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<ReverseConvertibleNoteBondLinked>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('SpRevConvBondLinked.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					...
					<xsl:call-template name="optionStyle"/>
            ...
            <xsl:for-each select="b:interestRateGroup">
                <xsl:variable name="index" select="./@index"/>
                    <xsl:if test="../b:couponMethod != 'RFR' and ../b:interestRateGroup[@index= $index]/b:rateChDate and ../b:interestRateGroup[@index= $index]/b:rateChDate != ''
	...
	<xsl:template match="d:FinancialInstrumentME">
		<StructuredProducts>
			<xsl:attribute name="mode">INSUPD</xsl:attribute>
			<xsl:attribute name="output">
				<xsl:value-of select="concat('SpRevConvBondLinked',d:id)"/>
			</xsl:attribute>
			<DataGroup>
				<ReverseConvertibleNoteBondLinked>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('SpRevConvBondLinked.xsl+', name(/*), '+', d:eventCommon/ns0:eventId, '+', d:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					<xsl:attribute name="sequenceNumber">
						<xsl:call-template name="calcSeqNum">
							<xsl:with-param name="creationTime">
