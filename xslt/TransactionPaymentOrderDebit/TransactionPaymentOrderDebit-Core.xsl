...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PaymentOrder" xmlns:infra="http://www.odcgroup.com/InfraPMS"
	...
	<xsl:if test="b:chargeAccount != '' and ((b:creditAccount !='' and b:chargeAccount != b:creditAccount) or not(b:creditAccount))">
		<account1>
		<infra:code>
       ...
       <xsl:if test="b:narrativeList/b:narrative">
        ...
        <xsl:if test="b:beneficiaryId">     
            <notepad>
                ...
                <infra:note>
                    <xsl:choose>
                        <xsl:when test="b:beneficiaryId != ''">
                            <xsl:value-of select="b:beneficiaryId"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="concat(b:creditAccountCustomerId,'-',b:creditAccountCustomerName1List/b:creditAccountCustomerName1[@language='GB'])"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </infra:note>
                <infra:title>
                    <xsl:value-of select=" 'BENEFICIARY ID' "/>
                </infra:title>
                <infra:type>
                    <infra:code>
	...
	<xsl:template name="transactionCode">
		<transactionCode>
                 <xsl:choose>
                     <xsl:when test="b:tapRefId and b:tapOperNature='WI'">
                         <xsl:value-of select="b:tapRefId"/>
                     </xsl:when>
                     <xsl:otherwise>
                        <xsl:value-of select="concat('PO_',b:id,'_D',$COMPANY_POSTFIX_SEPARATOR,b:mnemonic)"/>
                     </xsl:otherwise>
                 </xsl:choose>		
		</transactionCode>
	</xsl:template>
    ...
    <xsl:template name="valueDate">
        <valueDate>
	        <xsl:choose>
	            <xsl:when test="b:debitValueDate">
	                    <xsl:call-template name="xsdate">
                                 <xsl:with-param name="date" select="b:debitValueDate"/>
                        </xsl:call-template>
	            </xsl:when>
	            <xsl:otherwise>
                        <xsl:call-template name="xsdate">
