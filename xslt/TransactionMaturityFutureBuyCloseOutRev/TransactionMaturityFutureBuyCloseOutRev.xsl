...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionMaturityFutureBuyCloseOutRev"
	...
	<xsl:template match="c:MultiTransactionMaturityFutureBuyCloseOutRev">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionMaturityFutureBuyCloseOutRev"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionMaturityFutureBuyCloseOutRev">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionMaturityFutureBuyCloseOutRev" name="BatchTransactionMaturityFutureBuyCloseOutRev">
			<xsl:apply-templates select="b:TransactionMaturityFutureBuyCloseOutRev"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionMaturityFutureBuyCloseOutRev">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionMaturityFutureBuyCloseOutRev" name="BatchMultiTransactionMaturityFutureBuyCloseOutRev">
			<xsl:apply-templates select="c:MultiTransactionMaturityFutureBuyCloseOutRev"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionMaturityFutureBuyCloseOutRev">
		<TransactionPMS mode="INSUPD">
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionMaturityFutureBuyCloseOutRev',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Derivative>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionMaturityFutureBuyCloseOutRev.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
