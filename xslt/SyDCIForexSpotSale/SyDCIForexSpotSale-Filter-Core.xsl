...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SyDci" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSyDci" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiSyDci" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiSyDci" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
  ...
  <xsl:template match="b:SyDci">
    <xsl:apply-templates select="self::b:SyDci[b:exerciseExpire = 'EXERCISE']" mode="filter1"/>
  </xsl:template>
  <!-- Condition 2 -->
  <xsl:template match="b:SyDci" mode="filter1">
    <xsl:apply-templates select="self::b:SyDci[b:exoticTypeGroup/b:triggerExotic = 'YES']" mode="filter2"/>
  </xsl:template>
  <!-- Condition 3 -->
  <xsl:template match="b:SyDci" mode="filter2">
    <xsl:apply-templates select="self::b:SyDci[b:exerciseCcy = b:depAmtAltCcy1 or b:exerciseCcy = b:depAmtAltCcy2]" mode="filter3"/>
  </xsl:template>
  <!-- Condition 4 -->
  <xsl:template match="b:SyDci" mode="filter3">
    <xsl:apply-templates select="self::b:SyDci[b:contractStatus = 'MATURED']" mode="filter4"/>
  </xsl:template>
  <!-- Condition 5 -->
  <xsl:template match="b:SyDci" mode="filter4">
    ...
    <xsl:apply-templates select="self::b:SyDci[number($check_MaturityDate) &gt;= number($check_toDay)]" mode="filter5"/>
  </xsl:template>
  <!-- Condition 6 -->
  <xsl:template match="b:SyDci" mode="filter5">
    <xsl:apply-templates select="self::b:SyDci[b:portfolio != '']" mode="filter6"/>
  </xsl:template>
  <!-- Condition 7 - Portfolio Filter -->
	<xsl:template match="b:SyDci" mode="filter6">
		<xsl:variable name="checkPortfolioConditionVar">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'SyDCIForexSpotSale'"/>
