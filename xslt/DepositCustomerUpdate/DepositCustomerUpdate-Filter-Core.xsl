...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch" version="1.0" xmlns="http://www.odcgroup.com/InfraPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/DepositCustomerUpdate" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchDepositCustomerUpdate"
                xmlns:c="http://www.temenos.com/T24/event/TTI/MultiDepositCustomerUpdate" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiDepositCustomerUpdate" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- Condition 1 -->
	<xsl:template match="b:DepositCustomerUpdate">
		<xsl:apply-templates select="self::b:DepositCustomerUpdate[(b:CustomerSecurityJoinCustomerTypeList/b:CustomerSecurityJoinCustomerType = 'DEPOSITORY') or ((b:idDxCustomer = b:id) and (b:customerType = 'BROKER') and (b:exchMemberGroup/b:exchMemberList/b:exchMember = 'CLEARING'))]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:DepositCustomerUpdate" mode="filter-custo">
		<xsl:apply-templates select="self::b:DepositCustomerUpdate" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:DepositCustomerUpdate" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="Deposit" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\Deposit\Deposit.xml" htmlbaseurl="" outputurl="..\..\TestFiles\Deposit\Deposit_Custo.xml" processortype="jaxp" useresolver="no" profilemode="0"
