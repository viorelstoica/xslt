...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/FinancialInstrumentAADepositsandLoansIL" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchFinancialInstrumentAADepositsandLoansIL" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiFinancialInstrumentAADepositsandLoansIL" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiFinancialInstrumentAADepositsandLoansIL" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/><!-- FILTER -->
   <xsl:template match="c:MultiFinancialInstrumentAADepositsandLoansIL">
      <xsl:apply-templates select="c:CurrentEvent/b:FinancialInstrumentAADepositsandLoansIL"/>
   </xsl:template>
   <xsl:template match="batch:BatchFinancialInstrumentAADepositsandLoansIL">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchFinancialInstrumentAADepositsandLoansIL" name="BatchFinancialInstrumentAADepositsandLoansIL">
         <xsl:apply-templates select="b:FinancialInstrumentAADepositsandLoansIL"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiFinancialInstrumentAADepositsandLoansIL">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiFinancialInstrumentAADepositsandLoansIL" name="BatchMultiFinancialInstrumentAADepositsandLoansIL">
         <xsl:apply-templates select="c:MultiFinancialInstrumentAADepositsandLoansIL"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:FinancialInstrumentAADepositsandLoansIL"><!-- END FILTER -->
      <FiPMS>
         <xsl:attribute name="mode">INSUPD</xsl:attribute>
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FinancialInstrumentAADepositsandLoansIL',b:arrangementId)"/>
         </xsl:attribute>
         <xsl:for-each select=".">
            <DataGroup>
               <MoneyMarket>
                  <SpecificMoneyMarket>
                     <xsl:attribute name="description">
                        <xsl:value-of select="concat('FinancialInstrumentAADepositsandLoansIL.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
                     </xsl:attribute>
               ...
               <xsl:if test="(b:coCode and b:coCode != '') and ($POSTFIX_AACOMNE = 'Y')">
                  <BusinessEntityComposition>
                     ...
                     <xsl:call-template name="infra:mbFields_becompo"/>
