...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionCoExpireManualBuy"
	...
	<xsl:template match="b:TransactionCoExpireManualBuy">
		...
		<xsl:apply-templates select="self::b:TransactionCoExpireManualBuy[$opStatus != 'REVERSE-AUTHORISE']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:TransactionCoExpireManualBuy" mode="filter1">
		<xsl:apply-templates select="self::b:TransactionCoExpireManualBuy[b:unauthAuth = 'AUTHORISED']" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:TransactionCoExpireManualBuy" mode="filter-custo">
		<xsl:apply-templates select="self::b:TransactionCoExpireManualBuy" mode="filter-final"/>
	</xsl:template>
	<!-- Copy block -->
	<xsl:template match="b:TransactionCoExpireManualBuy" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
