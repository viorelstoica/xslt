   ...
   <xsl:template match="b:FinancialInstrumentSecurity">
      <FiPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FinancialInstrumentStock-Partial',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <Stock>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('FinancialInstrumentStock-Partial.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
