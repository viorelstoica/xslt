...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch ns0 d e f g" version="1.0" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template match="b:FinancialInstrumentSecurity">
		<FiPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FinancialInstrumentStock',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Stock>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FinancialInstrumentStock.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
			...
			<!--  Exchange Event -->
			<xsl:if test="b:optionsFlag and b:optionsFlag = 'YES'">
				<DataGroup>
					<Event>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('FinancialInstrumentStock.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
	...
	<xsl:template match="d:FinancialInstrumentME">
		<FiPMS>
			<xsl:attribute name="mode">INSUPD</xsl:attribute>
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FinancialInstrumentStock',d:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Stock>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FinancialInstrumentStock.xsl+', name(/*), '+', d:eventCommon/ns0:eventId, '+', d:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					<xsl:attribute name="sequenceNumber">
						<xsl:call-template name="calcSeqNum">
							<xsl:with-param name="creationTime">
				...
				<xsl:if test="d:defaultDepository and d:defaultDepository != ''">
					<InstrumentDeposit>
						...
						<xsl:call-template name="mbFields_InstrumentDeposit_IE_MultiEntity"/>
					</InstrumentDeposit>
				</xsl:if>
			</DataGroup>
		</FiPMS>
