...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:a="http://www.temenos.com/T24/AaActivityExtractorService/AaArrInterestRec"
	...
	<xsl:template match="c:aaarecord">
		<xsl:apply-templates select="self::c:aaarecord[d:activity = 'LENDING-APPLYPAYMENT-PR.REPAYMENT' or d:activity = 'LENDING-APPLYPAYMENT-PR.PRINCIPAL' or d:activity = 'LENDING-APPLYPAYMENT-PR.PRINCIPAL.DECREASE']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	...
	<xsl:template match="c:aaarecord" mode="filter2">
		<xsl:apply-templates select="self::c:aaarecord[../c:accountrecord/g:portfolioId != '']" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
	<xsl:template match="c:aaarecord" mode="filter3">
		<xsl:apply-templates select="self::c:aaarecord[../c:aatxncontext/b:aaMasterActivity != 'LENDING-SETTLE-PAYOFF']" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
