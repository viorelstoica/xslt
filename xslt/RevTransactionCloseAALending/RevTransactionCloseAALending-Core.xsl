...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS"
   ...
   <xsl:template name="reversedTransactionCode">
      <reversedTransactionCode>
		...
		<xsl:when test="$POSTFIX_AACOMNE = 'Y'">
	         <xsl:choose>
	            <xsl:when test="d:activity = 'LENDING-APPLYPAYMENT-PR.REPAYMENT' or d:activity = 'LENDING-APPLYPAYMENT-PR.PRINCIPAL' or d:activity = 'LENDING-APPLYPAYMENT-PR.PRINCIPAL.DECREASE'">
   ...
   <xsl:template name="transactionCode">
      <transactionCode>
         <xsl:choose>
	  	<xsl:when test="$POSTFIX_AACOMNE = 'Y'">
         	<xsl:value-of select="concat(../c:aatxncontext/b:aaArrActivityId,$COMPANY_POSTFIX_SEPARATOR,../c:aatxncontext/b:aaCompanyMnemonic,'-C','_R')"/>
		</xsl:when>
		<xsl:otherwise>
			<xsl:value-of select="concat(../c:aatxncontext/b:aaArrActivityId,'-C','_R')"/>
		</xsl:otherwise>
	  </xsl:choose>
      </transactionCode>
   </xsl:template>
