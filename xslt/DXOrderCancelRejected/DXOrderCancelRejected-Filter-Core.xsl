...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch" version="1.0" xmlns="http://www.odcgroup.com/Request"
	...
	<xsl:template match="b:DXOrderReturn" mode="filter1">
		<xsl:apply-templates select="self::b:DXOrderReturn[not(b:dealerBookList/b:dealerBook)]" mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -->
	<xsl:template match="b:DXOrderReturn" mode="filter2">
		<xsl:apply-templates select="self::b:DXOrderReturn[b:customerTypeCustomerSecurityJoinGroup[@index = '1']/b:customerTypeCustomerSecurityJoinList/b:customerTypeCustomerSecurityJoin = 'CUSTOMER']" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
	...
	<!-- Condition 5 -->
	<xsl:template match="b:DXOrderReturn" mode="filter3">
		<xsl:apply-templates select="self::b:DXOrderReturn[(b:orderStatus = 'TRANSMITTED' and ../../c:PreviousEvent/b:DXOrderReturn/b:orderStatus = 'CANCELLATION REQUEST') or (b:orderStatus = 'PARTIAL' and ../../c:PreviousEvent/b:DXOrderReturn/b:orderStatus = 'CANCELLATION REQUEST')]" mode="filter4"/>
	</xsl:template>
	<!--CONDITION 6-->
	<xsl:template match="b:DXOrderReturn" mode="filter4">
		<xsl:apply-templates select="self::b:DXOrderReturn[((b:parent = 'NO' or b:parent ='' or not(b:parent)) and b:parentChildRef !='') or (not(b:parent) and not(b:parentChildRef))]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<!-- Custo placeholder -->
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="DXOrderCancelRejected" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\DXOrderCancelRejected\20161021_141601_299_47_msgin_MultiDXOrderUpdateAcceptedc.xml" htmlbaseurl=""
		          outputurl="..\..\TestFiles\DXOrderCancelRejected\DXOrderCancelRejectedCusto.xml" processortype="jaxp" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath=""
