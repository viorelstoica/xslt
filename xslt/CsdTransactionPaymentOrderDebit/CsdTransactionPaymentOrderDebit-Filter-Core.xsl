...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch" version="1.0" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PaymentOrder" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchPaymentOrder"
	...
	<xsl:template match="b:PaymentOrder">
			...
			<xsl:choose>
				<xsl:when test="b:debitAccountPortfolioNoList/b:debitAccountPortfolioNo and b:debitAccountPortfolioNoList/b:debitAccountPortfolioNo != ''">
					<xsl:call-template name="checkPortfolioConditionCsdEam">
						<xsl:with-param name="xsltName" select="'CsdTransactionPaymentOrderDebit'"/>
		...
		<xsl:apply-templates select="self::b:PaymentOrder[((not(b:tapRefId) and $checkPortfolioConditionVar = 'true') or (b:tapRefId and b:tapOperNature = 'WI') or (b:tapRefId and b:tapOperNature = 'IN' and $checkPortfolioConditionVar = 'true')) and (b:paymentOrderProduct = 'EAMOTHERDR' or b:paymentOrderProduct ='EAMOTHERCR' or b:paymentOrderProduct ='EAMSIC' or b:paymentOrderProduct ='EAMEUROSIC' or b:paymentOrderProduct ='EAMSEPA' or b:paymentOrderProduct ='EAMSWIFT')]"
		                     mode="filter-custo"/>
	</xsl:template>
