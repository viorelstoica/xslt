...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/CaEvent" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template match="b:FinancialInstrumentSecurity">
		...
		<xsl:apply-templates select="self::b:FinancialInstrumentSecurity[($g_filterBySubAssetType='Y' and $subAssetType and contains(concat(' ',$list_SubAssetType_FinancialInstrumentCorporateActions,' '),concat(' ',$subAssetType,' '))) or 
									($g_filterBySubAssetType != 'Y' and ../../c:CurrentEvent/b:FinancialInstrumentSecurity/b:subAssetTypeTapInstrType and ../../c:CurrentEvent/b:FinancialInstrumentSecurity/b:subAssetTypeTapInstrType='MUTUALFUND')]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:FinancialInstrumentSecurity">
