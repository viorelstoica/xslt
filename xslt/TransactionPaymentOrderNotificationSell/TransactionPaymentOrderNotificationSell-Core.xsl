...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PaymentOrderNotification"
	...
	<xsl:template name="transactionFeesTaxesCounter">
		<xsl:if test="b:porTransactionDebitMainAmount != b:porTransactionPostingMainAmountDebit">
			<transactionFeesTaxesCounter>
				<amount>
					<xsl:value-of select="format-number(b:porTransactionPostingMainAmountDebit,'#.#########','nan2zero') - format-number(b:porTransactionDebitMainAmount,'#.#########','nan2zero')"/>
				</amount>
				<currency>
					<infra:code>
						<xsl:value-of select="b:paymentOrderPaymentCurrency"/>
					</infra:code>
				</currency>
				<type>
					<infra:code>
						<xsl:value-of select="'CASH_OP_TOT_CHARGES'"/>
					</infra:code>
				</type>
			</transactionFeesTaxesCounter>
		</xsl:if>
	...
	<xsl:template name="transactionCode">
			...
			<xsl:choose>
				<xsl:when test="$OperationReversed = '1'">
					<transactionCode>
						<xsl:value-of select="concat(b:paymentOrderTapRefId,'_R')"/>
					</transactionCode>
				</xsl:when>
				<xsl:otherwise>
					<transactionCode>
	...
	<xsl:template name="quantity">
		<quantity>
			<xsl:value-of select="b:paymentOrderPaymentAmount"/>
		</quantity>
	</xsl:template>
