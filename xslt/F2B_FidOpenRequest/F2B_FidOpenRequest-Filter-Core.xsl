<?xml version="1.0" encoding="UTF-8"?><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:a="http://www.temenos.com/T24/OFSML/150" xmlns:b="http://www.odcgroup.com/F2B_MM_FX" xmlns:c="http://www.temenos.com/T24/BatchOFSML/150" xmlns:infra="http://www.odcgroup.com/InfraPMS" exclude-result-prefixes="b" version="1.0">
    ...
    <xsl:template match="b:F2B_MM_FX">
        <xsl:variable name="cancelledStatus">
            <xsl:call-template name="statusTranslation">
                <xsl:with-param name="boStatus">CANCELLED</xsl:with-param>
            </xsl:call-template>
        </xsl:variable>
        <xsl:apply-templates select="self::b:F2B_MM_FX[
                        starts-with(b:DataGroup/b:DepositLoan/b:Deposit/b:OrderLifeCycle/b:openRequest/b:pmsReferenceCode, 'FDOR') 
                        and b:DataGroup/b:DepositLoan/b:Deposit/b:OrderLifeCycle/b:openRequest/b:orderStatus != $cancelledStatus
                        ]" mode="filter-custo"/>
    </xsl:template>
    <!-- Custo placeholder -->
