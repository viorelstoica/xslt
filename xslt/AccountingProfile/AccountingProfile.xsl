	...
	<xsl:template match="b:Cginventory">
		<InfraPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('AccountingProfile',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<AccountingProfile>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('AccountingProfile.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
