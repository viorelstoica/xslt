...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionMaturityFutureSell"
	...
	<xsl:template match="c:MultiTransactionMaturityFutureSell">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionMaturityFutureSell"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionMaturityFutureSell">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionMaturityFutureSell" name="BatchTransactionMaturityFutureSell">
			<xsl:apply-templates select="b:TransactionMaturityFutureSell"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionMaturityFutureSell">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionMaturityFutureSell" name="BatchMultiTransactionMaturityFutureSell">
			<xsl:apply-templates select="c:MultiTransactionMaturityFutureSell"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionMaturityFutureSell">
		<TransactionPMS mode="INSUPD">
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionMaturityFutureSell',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<xsl:for-each select="b:closeoutIdList/b:closeoutId">
					<Derivative>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('TransactionMaturityFutureSell.xsl+', name(/*), '+', ../../b:eventCommon/ns0:eventId, '+', ../../b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
