	...
	<xsl:template match="b:SecurityTransfer">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('securityTransferSell',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Security>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('SecurityTransferSell.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
