...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PositionTransferPtfToPtfWithdrReversal" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b ns0" version="1.0">
   ...
   <xsl:template name="transactionCode">
      <transactionCode>
			<xsl:choose>
				<xsl:when test="../b:tapOperCode and ../b:tapOperCode != ''">
					<xsl:value-of select="concat(../b:tapOperCode,'_','ST_O_',../b:id,'_',b:securityNo,'_',b:depository,$COMPANY_POSTFIX_SEPARATOR,../b:companyMnemonic,'_R')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="concat('ST_O_',../b:id,'_',b:securityNo,'_',b:depository,$COMPANY_POSTFIX_SEPARATOR,../b:companyMnemonic,'_R')"/>
				</xsl:otherwise>
			</xsl:choose>
      </transactionCode>
   </xsl:template>
