...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/STBuyActualSettlement" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSTBuyActualSettlement" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiSTBuyActualSettlement" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiSTBuyActualSettlement" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   ...
   <xsl:template match="c:MultiSTBuyActualSettlement">
      <xsl:apply-templates select="c:CurrentEvent/b:STBuyActualSettlement"/>
   </xsl:template>
   <xsl:template match="batch:BatchSTBuyActualSettlement">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchSTBuyActualSettlement" name="BatchSTBuyActualSettlement">
         <xsl:apply-templates select="b:STBuyActualSettlement"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiSTBuyActualSettlement">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiSTBuyActualSettlement" name="BatchMultiSTBuyActualSettlement">
         <xsl:apply-templates select="c:MultiSTBuyActualSettlement"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:STBuyActualSettlement">
      <TransactionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('STBuyActualSettlement',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <Security>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('STBuyActualSettlement.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
