...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch ns0 d e f g" version="1.0" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template match="b:FinancialInstrumentSecurity">
		<FiPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FinancialInstrumentConvertibleBond',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<ConvertibleBond>
					...
					<xsl:if test="b:convertibleFlag and b:convertibleFlag = 'Y'">
						<ConversionEvent>
	...
	<xsl:template match="d:FinancialInstrumentME">
		<FiPMS>
			<xsl:attribute name="mode">INSUPD</xsl:attribute>
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FinancialInstrumentConvertibleBond',d:id)"/>
			</xsl:attribute>
			<DataGroup>
				<ConvertibleBond>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FinancialInstrumentConvertibleBond.xsl+', name(/*), '+', d:eventCommon/ns0:eventId, '+', d:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					<xsl:attribute name="sequenceNumber">
						<xsl:call-template name="calcSeqNum">
							<xsl:with-param name="creationTime">
					...
					<xsl:call-template name="ConversionEvent_MultiEntity"/>
				</ConvertibleBond>
