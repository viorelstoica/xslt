   ...
   <xsl:template match="b:FinancialInstrumentSecurity">
      <FiPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FinancialInstrumentConvertibleBond-Partial',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <ConvertibleBond>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('FinancialInstrumentConvertibleBond-Partial.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
