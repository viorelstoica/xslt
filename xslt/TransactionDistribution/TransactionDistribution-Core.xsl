...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/GenericCOA" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionEntitlement" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b ns0" version="1.0">
   ...
   <xsl:template name="transactionFeesTaxesCounter">
      ...
      <xsl:if test="format-number(../@index,'#.##','nan2zero') = format-number($idxFirstOptDescGroupWithNewSecu,'#.##','nan2zero') "><!-- need format-number to compare correctly -->
         <xsl:if test="(@index = 1)">
   ...
   <xsl:template name="Income_account1">
      <account1>
         <infra:code>
            <xsl:value-of select="../../b:accountNo"/>
         </infra:code>
   ...
   <xsl:template name="Income_account1CcyIncomeCcyExchangeRate">
      <account1CcyIncomeCcyExchangeRate>
         <xsl:value-of select="../../b:accExchRate"/>
      </account1CcyIncomeCcyExchangeRate>
   </xsl:template>
   <xsl:template name="Income_account1NetAmount"/>
   <xsl:template name="Income_account2"/>
   <xsl:template name="Income_account2CcyIncomeCcyExchangeRate"/>
   <xsl:template name="Income_account2NetAmount"/>
   <xsl:template name="Income_account3"/>
   <xsl:template name="Income_account3CcyIncomeCcyExchangeRate"/>
   <xsl:template name="Income_account3NetAmount"/>
   <xsl:template name="Income_accountingCode"/>
   <xsl:template name="Income_commonReference"/>
   <xsl:template name="Income_accountingDate">
      <accountingDate>
         <xsl:value-of select="../../b:PayDateJoin"/>
      </accountingDate>
   </xsl:template>
   <xsl:template name="Income_cashPortfolio"/>
   <xsl:template name="Income_coaReference">
      <coaReference>
         <xsl:value-of select="../../b:diaryId"/>
      </coaReference>
   </xsl:template>
   <xsl:template name="Income_communication"/>
   <xsl:template name="Income_depositaryAccount">
      <xsl:choose>
   		<xsl:when test="../../b:depository != ''">
		<depositaryAccount>
		<infra:code>
   ...
   <xsl:template name="Income_income">
      <income>
         <xsl:value-of select="format-number(../../b:optionDescGroup[@index = 1]/b:entitlementAmt div ../../b:eventNominal,'#.#########','nan2zero')"/>
      </income>
   </xsl:template>
   <xsl:template name="Income_incomeCcyGrossAmount">
      <incomeCcyGrossAmount>
         <xsl:value-of select="../../b:optionDescGroup[@index = 1]/b:entitlementAmt"/>
      </incomeCcyGrossAmount>
   </xsl:template>
   <xsl:template name="Income_incomeCcyNetAmount"/>
   <xsl:template name="Income_incomeCcyPortfolioCcyExchangeRate"/>
   <xsl:template name="Income_incomeCounter"/>
   <xsl:template name="Income_incomeCurrency">
      <incomeCurrency>
         <infra:code>
   ...
   <xsl:template name="Income_initiator"/>
   <xsl:template name="Income_nominalOrUnit">
      <nominalOrUnit>
         <xsl:value-of select="../../b:eventNominal"/>
      </nominalOrUnit>
   </xsl:template>
   <xsl:template name="Income_notepad"/>
   <xsl:template name="Income_portfolio">
      <portfolio>
         <infra:code>
            <xsl:value-of select="../../b:portfolioNo"/>
         </infra:code>
   ...
   <xsl:template name="Income_portfolioManager"/>
   <xsl:template name="Income_remark">
      <remark>
         <xsl:value-of select="../../b:optionDescGroup[@index = 1]/b:optionDesc"/>
      </remark>
   </xsl:template>
   <xsl:template name="Income_reversalIndicator">
      <xsl:if test="../../b:recordStatus = 'REVE'">
         <reversalIndicator>
            <xsl:value-of select="'1'"/>
         </reversalIndicator>
      </xsl:if>
   </xsl:template>
   <xsl:template name="Income_reversedTransactionCode">
      <xsl:if test="../../b:recordStatus = 'REVE'">
         <reversedTransactionCode>
		 	<xsl:choose>
				<xsl:when test="../../b:tapOperCode and ../../b:tapOperCode != ''">
   ...
   <xsl:template name="Income_security">
      <security>
         <infra:code>
            <xsl:value-of select="../../b:securityNo"/>
         </infra:code>
   ...
   <xsl:template name="Income_securityCcyNetAmount"/>
   <xsl:template name="Income_securityCcyPortfolioCcyExchangeRate"/>
   <xsl:template name="Income_sourceSystemCode">
         <sourceSystemCode>
         <xsl:value-of select="../../b:entitlementId"/>
      </sourceSystemCode>
   </xsl:template>
   <xsl:template name="Income_status">
      <status>
         <xsl:call-template name="statusTranslation">
            <xsl:with-param name="boStatus" select="'ACCOUNTED'"/>
         </xsl:call-template>
   ...
   <xsl:template name="Income_subType"/>
   <xsl:template name="Income_transactionCode">
      <transactionCode>
	  	<xsl:choose>
			<xsl:when test="../../b:tapOperCode and ../../b:tapOperCode != ''">
				<xsl:choose>
            		<xsl:when test="../../b:recordStatus = 'REVE'">
   ...
   <xsl:template name="Income_transactionDate">
      <transactionDate>
         <xsl:value-of select="../../b:PayDateJoin"/>
      </transactionDate>
   </xsl:template>
   <xsl:template name="Income_transactionFeesTaxesCounter"/>
   <xsl:template name="Income_type">
      <type>
         <infra:code>
            <xsl:value-of select="concat('ENT_',../../b:eventType)"/>
         </infra:code>
   ...
   <xsl:template name="Income_typeInstrument"/>
   <xsl:template name="Income_valueDate">
      <valueDate>
         <xsl:value-of select="../../b:valueDate"/>
      </valueDate>
   </xsl:template>
   <xsl:template name="Income_positionCriteria1">
   	<positionCriteria1>
		<xsl:call-template name="extCustodianTranslation">
			<xsl:with-param name="boExtCustodian">
				<xsl:value-of select="../../b:extCustodian"/>
   ...
   <xsl:template name="Income_userDefinedField"/>
   <xsl:template name="Income_mbFields"/>
   <xsl:template name="Income_exDate">
	<exDate>
		<xsl:value-of select="../../b:diaryExDate"/>
	</exDate>
	</xsl:template>
   <xsl:template name="Income_splitPortfolio">
		<splitPortfolio>
			<xsl:value-of select="'1'"/>
		</splitPortfolio>
   </xsl:template>
