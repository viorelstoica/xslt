...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionRevIssuedGuarantee" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionRevIssuedGuarantee" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiTransactionRevIssuedGuarantee" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionRevIssuedGuarantee" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   ...
   <xsl:template match="c:MultiTransactionRevIssuedGuarantee">
      <xsl:apply-templates select="c:CurrentEvent/b:TransactionRevIssuedGuarantee"/>
   </xsl:template>
   <xsl:template match="batch:BatchTransactionRevIssuedGuarantee">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionRevIssuedGuarantee" name="BatchTransactionRevIssuedGuarantee">
         <xsl:apply-templates select="b:TransactionRevIssuedGuarantee"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiTransactionRevIssuedGuarantee">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionRevIssuedGuarantee" name="BatchMultiTransactionRevIssuedGuarantee">
         <xsl:apply-templates select="c:MultiTransactionRevIssuedGuarantee"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:TransactionRevIssuedGuarantee">
      <TransactionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('TransactionRevIssuedGuarantee',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <Security>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('TransactionRevIssuedGuarantee.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
