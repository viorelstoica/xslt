...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionSettleFutureSell"
	...
	<xsl:template match="c:MultiTransactionSettleFutureSell">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionSettleFutureSell"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionSettleFutureSell">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionSettleFutureSell" name="BatchTransactionSettleFutureSell">
			<xsl:apply-templates select="b:TransactionSettleFutureSell"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionSettleFutureSell">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSettleFutureSell" name="BatchMultiTransactionSettleFutureSell">
			<xsl:apply-templates select="c:MultiTransactionSettleFutureSell"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionSettleFutureSell">
		<TransactionPMS mode="INSUPD">
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionSettleFutureSell',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Derivative>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionSettleFutureSell.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
