...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionSecuritySell" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionSecuritySell" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiTransactionSecuritySell" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSecuritySell" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   ...
   <xsl:template match="c:MultiTransactionSecuritySell">
      <xsl:apply-templates select="c:CurrentEvent/b:TransactionSecuritySell"/>
   </xsl:template>
   <xsl:template match="batch:BatchTransactionSecuritySell">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionSecuritySell" name="BatchTransactionSecuritySell">
         <xsl:apply-templates select="b:TransactionSecuritySell"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiTransactionSecuritySell">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSecuritySell" name="BatchMultiTransactionSecuritySell">
         <xsl:apply-templates select="c:MultiTransactionSecuritySell"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:TransactionSecuritySell">
      <TransactionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('transactionSecuritySell',b:id1)"/>
         </xsl:attribute>
         <xsl:for-each select="b:customerNoGroup/b:custSecAcc">
            <xsl:variable name="index" select="../@index"/><!--  exchange rates for this iteration  -->
            ...
            </xsl:variable><!-- Amounts to be credited to acc1 and acc2 -->
            ...
            </xsl:variable><!-- TransactionSecuritySell  -->
            <DataGroup>
               <Security>
                  <xsl:attribute name="description">
                     <xsl:value-of select="concat('TransactionSecuritySell.xsl+', name(/*), '+', ../../b:eventCommon/ns0:eventId, '+', ../../b:eventCommon/ns0:creationTime)"/>
                  </xsl:attribute>
            ...
            </DataGroup><!-- Corrections for cash booking for income/cap seggration: Book cash to income ptf if income account present 
            the income acc is only present if charge or income seggreg is applied on the portfolio
            -->
            <xsl:if test="../b:cuIncomeAcc and (../b:cuIncomeAmt and number(../b:cuIncomeAmt) != '0') and (../../b:segregIncomeJoinList/b:segregIncomeJoin = 'YES' or ../../b:segregChargeJoinList/b:segregChargeJoin = 'YES') and (not(../../b:custAccNoPortfolioNoGroup[@index=$index]/b:custAccNoPortfolioNoList/b:custAccNoPortfolioNo = ../../b:portfolioNoJoinGroup[@index=$index]/b:portfolioNoJoinList/b:portfolioNoJoin)) and ../../b:exclIncValuationScParameterJoin = 'YES'">
               <DataGroup>
                  <Cash>
                     <xsl:attribute name="description">
                        <xsl:value-of select="concat('TransactionSecuritySell.xsl+', name(/*), '+', ../../b:eventCommon/ns0:eventId, '+', ../../b:eventCommon/ns0:creationTime)"/>
