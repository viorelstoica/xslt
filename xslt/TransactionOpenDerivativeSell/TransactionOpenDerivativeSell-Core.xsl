...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionOpenDerivativeSell"
	...
	<xsl:template name="account1">
		<account1>
			<infra:code>
				<xsl:value-of select="b:priSecAccGroup[@index='1']/b:custAccNo"/>
			</infra:code>
		</account1>
	</xsl:template>
	...
	<xsl:template name="account3">
		<xsl:if test="b:tradeType = 'FUTURE' and (format-number(sum(b:priSecAccGroup/b:priCommTypSubGroup/b:priCommAmt) + sum(b:priSecAccGroup/b:tradeTaxesSubGroup/b:taxAmtTcy),$decimalformat,'nan2zero') != '0')">
			<account3>
				<infra:code>
					<xsl:value-of select="b:priSecAccGroup/b:custAccNo"/>
				</infra:code>
			</account3>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="account3NetAmount">
		<xsl:if test="b:tradeType = 'FUTURE'">
			<account3NetAmount>
				...
				<xsl:value-of select="format-number(($sumPriCommAmt + $sumTaxAmtTcy),$decimalformat,'nan2zero')"/>
			</account3NetAmount>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="subType">
		<xsl:if test="substring(b:parentRef,1,5)='DXTRA'">
			<subType>
				<infra:code>
					<xsl:value-of select="concat(substring(b:tradeType,1,3),'_',substring(b:priBuySell,1,3),'_',substring(b:closeoutTypeList/b:closeoutType,1,3),'_',b:parentRefOptionType)"/>
				</infra:code>
			</subType>
		</xsl:if>
	</xsl:template>
				...
				<infra:code>
					<xsl:choose>
						...
						<xsl:when test="b:tradeType = 'FUTURE'">
							<xsl:value-of select="'FUTURE_SEL_OPEN'"/>
						</xsl:when>
					</xsl:choose>
				</infra:code>
			</type>
	...
	<xsl:template name="quantity">
		<quantity>
			<xsl:value-of select="b:priSecAccGroup/b:cuQuantity"/>
		</quantity>
	</xsl:template>
	...
	<xsl:template name="transactionFeesTaxesCounter">
		<xsl:call-template name="printDxOpenPositionFeesAndTaxes">
			<xsl:with-param name="caller" select="'TransactionOpenDerivativeSell'"/>
		</xsl:call-template>
	</xsl:template>
