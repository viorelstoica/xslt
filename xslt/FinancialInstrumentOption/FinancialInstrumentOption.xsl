...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/FinancialInstrumentOption" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchFinancialInstrumentOption" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiFinancialInstrumentOption" xmlns:d="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiFinancialInstrumentOption" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/>
   <xsl:template match="c:MultiFinancialInstrumentOption">
      <xsl:apply-templates select="c:CurrentEvent/b:FinancialInstrumentOption"/>
   </xsl:template>
   <xsl:template match="batch:BatchFinancialInstrumentOption">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchFinancialInstrumentOption" name="BatchFinancialInstrumentOption">
         <xsl:apply-templates select="b:FinancialInstrumentOption"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiFinancialInstrumentOption">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiFinancialInstrumentOption" name="BatchMultiFinancialInstrumentOption">
         <xsl:apply-templates select="c:MultiFinancialInstrumentOption"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:FinancialInstrumentOption">
      <FiPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('financialInstrumentOption',translate(translate(b:id,'/',''),':',''))"/>
         </xsl:attribute>
         <DataGroup>
            <Option>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('FinancialInstrumentOption.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
