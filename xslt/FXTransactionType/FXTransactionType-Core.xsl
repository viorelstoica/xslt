...
<xsl:stylesheet xmlns="http://www.odcgroup.com/InfraPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/FXTransactionType" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiFXTransactionType" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="b c" version="1.0">
  <xsl:template name="code">
    <code>
      <xsl:value-of select="concat('FX_',b:id)"/>
    </code>
  </xsl:template>
  ...
  <xsl:template name="multilingualLongName">
    ...
    <xsl:call-template name="OldNewDenomManagment">
      ...
      <xsl:with-param name="PreviousEvent" select="../../c:PreviousEvent/b:FXTransactionType/b:descriptionList/b:description"/>
    </xsl:call-template>
  </xsl:template>
