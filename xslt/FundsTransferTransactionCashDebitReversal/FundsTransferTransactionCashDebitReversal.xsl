...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/FundsTransferTransactionCashDebitReversal"
	...
	<xsl:template match="c:MultiFundsTransferTransactionCashDebitReversal">
		<xsl:apply-templates select="c:CurrentEvent/b:FundsTransferTransactionCashDebitReversal"/>
	</xsl:template>
	<xsl:template match="batch:BatchFundsTransferTransactionCashDebitReversal">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchFundsTransferTransactionCashDebitReversal" name="BatchFundsTransferTransactionCashDebitReversal">
			<xsl:apply-templates select="b:FundsTransferTransactionCashDebitReversal"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiFundsTransferTransactionCashDebitReversal">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiFundsTransferTransactionCashDebitReversal" name="BatchMultiFundsTransferTransactionCashDebitReversal">
			<xsl:apply-templates select="c:MultiFundsTransferTransactionCashDebitReversal"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:FundsTransferTransactionCashDebitReversal">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FundsTransferTransactionCashDebitReversal',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Cash>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FundsTransferTransactionCashDebitReversal.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
