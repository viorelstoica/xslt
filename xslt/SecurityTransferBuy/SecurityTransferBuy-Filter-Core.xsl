...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SecurityTransfer" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSecurityTransfer"
	...
	<xsl:template match="b:SecurityTransfer">
		<xsl:apply-templates select="self::b:SecurityTransfer[b:peFund= 'YES' and 
									contains($peFundsTransferInTypes,b:transactionType) and 
									(b:peType= 'DRAWDOWN' and (b:custRemarksList/b:custRemarks= 'Subscription' or b:custRemarksList/b:custRemarks= 'Capital Return Unalloted' or b:custRemarksList/b:custRemarks= 'Capital Return Alloted')) or 
									(contains($securityTransferInSupportedTypes,b:transactionType) and b:deliveryInstr = 'DAP' and (b:secHoldSettle = 'NO' or b:secHoldSettle = '')) or
									(contains($securityTransferInSupportedTypes,b:transactionType) and b:deliveryInstr = 'DAP' and b:secHoldSettle = 'YES' and b:tapRefId != '')]"
		                     mode="filter1"/>
	</xsl:template>
	<!-- Map only if this condition is satisfied -->
