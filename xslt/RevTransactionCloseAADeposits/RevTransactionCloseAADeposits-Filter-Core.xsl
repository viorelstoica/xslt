...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:a="http://www.temenos.com/T24/AaActivityExtractorService/AaArrInterestRec" xmlns:b="http://www.temenos.com/T24/AaActivityExtractorService/AaTransactionContext" xmlns:c="http://www.temenos.com/T24/event/AAAExtractor/AAAFlow" xmlns:d="http://www.temenos.com/T24/AaActivityExtractorService/AaArrActivityRec" xmlns:e="http://www.temenos.com/T24/AaActivityExtractorService/AaProductRec" xmlns:f="http://www.temenos.com/T24/AaActivityExtractorService/AaAccountDetailsRec" xmlns:g="http://www.temenos.com/T24/AaActivityExtractorService/AaArrAccountRec" xmlns:h="http://www.temenos.com/T24/AaActivityExtractorService/AaArrSettlementRec" xmlns:i="http://www.temenos.com/T24/AaActivityExtractorService/AaArrengementRec" xmlns:j="http://www.temenos.com/T24/AaActivityExtractorService/AaBillRec" xmlns:k="http://www.temenos.com/T24/AaActivityExtractorService/AaIntAccrualRec" xmlns:l="http://www.temenos.com/T24/AaActivityExtractorService/AaFtRecord" version="1.0" exclude-result-prefixes="a b c d e f g h i j k l">
  ...
  <xsl:template match="c:AAAFlow">
    <xsl:apply-templates select="self::c:AAAFlow[c:accountrecord/g:portfolioId != '']" mode="filter1"/>
  </xsl:template>
  <!-- Condition 2 -->
  <xsl:template match="c:AAAFlow" mode="filter1">
    <xsl:apply-templates select="self::c:AAAFlow[c:aatxncontext/b:aaActivityStatus = 'REV']" mode="filter2"/>
  </xsl:template>
  <!-- Condition 3 -->
  <xsl:template match="c:AAAFlow" mode="filter2">
    <xsl:variable name="cond1" select=" c:aaarecord/d:txnSystemId = 'AAAA' and (
                                         (c:aaarecord/d:activity = 'DEPOSITS-APPLYPAYMENT-PO.WITHDRAWAL' or 
                                          ...
                                          c:aaarecord/d:activity = 'DEPOSITS-APPLYPAYMENT-PO.EARLY.WITHDRAWAL' ) and 
                                            (c:aatxncontext/b:aaMasterActivity = 'DEPOSITS-MATURE-ARRANGEMENT' or 
                                             c:aatxncontext/b:aaMasterActivity = 'DEPOSITS-REDEEM-ARRANGEMENT' or 
                                             c:aatxncontext/b:aaMasterActivity = 'DEPOSITS-MAKEDUE-SCHEDULE' or
                                             c:aatxncontext/b:aaMasterActivity = 'DEPOSITS-WITHDRAW-PO.EARLY.WITHDRAWAL'
                                             ) and 
                                             
                                          (c:aaarecord/d:activity = 'DEPOSITS-APPLYPAYMENT-PO.PRINCIPAL' or 
                                           c:billdetails/j:billProperyDetails[starts-with(j:repayRef,c:aatxncontext/b:aaArrActivityId)]/j:property = 'ACCOUNT')
                                        ) 
                                         "/>
    <xsl:variable name="cond2" select="c:aaarecord/d:txnSystemId = 'FT' and ((c:aaarecord/d:activity = 'DEPOSITS-APPLYPAYMENT-PO.EARLY.WITHDRAWAL') or (c:aaarecord/d:activity = 'DEPOSITS-APPLYPAYMENT-PO.WITHDRAWALL') or (c:aaarecord/d:activity = 'DEPOSITS-APPLYPAYMENT-PO.PRINCIPAL')) and ((c:recarrangement/i:arrStatus = 'CURRENT') or (c:recarrangement/i:arrStatus = 'PENDING.CLOSURE'))"/>
    <xsl:apply-templates select="self::c:AAAFlow[($cond1) or ($cond2)]" mode="filter3"/>
  </xsl:template>
  <!-- Condition 4 -->
