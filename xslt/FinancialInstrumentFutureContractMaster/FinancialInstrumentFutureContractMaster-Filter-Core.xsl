...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch" version="1.0" xmlns="http://www.odcgroup.com/FiPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/FinancialInstrumentFutureContractMaster"
                xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchFinancialInstrumentFutureContractMaster" xmlns:c="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:infra="http://www.odcgroup.com/InfraPMS"
	...
	<xsl:template match="b:FinancialInstrumentFutureContractMaster">
		<xsl:apply-templates select="self::b:FinancialInstrumentFutureContractMaster[($g_filterBySubAssetType='Y' and contains($list_SubAssetType_FinancialInstrumentFutureContractMaster,b:subAssetType)) or
									($g_filterBySubAssetType != 'Y' and b:subAssetTypeTapInstrType and b:subAssetTypeTapInstrType='FUTURE')]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:FinancialInstrumentFutureContractMaster" mode="filter-custo">
		<xsl:apply-templates select="self::b:FinancialInstrumentFutureContractMaster" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:FinancialInstrumentFutureContractMaster" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
