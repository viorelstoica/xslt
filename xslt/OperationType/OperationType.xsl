...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/OperationType" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchOperationType" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiOperationType" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiOperationType" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/>
   <xsl:template match="c:MultiOperationType">
      <xsl:apply-templates select="c:CurrentEvent/b:OperationType"/>
   </xsl:template>
   <xsl:template match="batch:BatchOperationType">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchOperationType" name="BatchOperationType">
         <xsl:apply-templates select="b:OperationType"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiOperationType">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiOperationType" name="BatchMultiOperationType">
         <xsl:apply-templates select="c:MultiOperationType"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:OperationType">
      <InfraPMS>
         <xsl:attribute name="mode">INSUPD</xsl:attribute>
         <xsl:attribute name="output">
            <xsl:value-of select="concat('operationType',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <Type>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('OperationType.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="OperationType" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\OperationType\OperationType.xml" htmlbaseurl="" outputurl="..\..\TestFiles\OperationType\OperationTypeCusto.xml" processortype="jaxp"
		          useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath=""
		          postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator="">
			<advancedProp name="sInitialMode" value=""/>
			<advancedProp name="bSchemaAware" value="false"/>
