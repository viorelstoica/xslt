...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionOpenDerivativeBuy"
	...
	<xsl:template match="c:MultiTransactionOpenDerivativeBuy">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionOpenDerivativeBuy"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionOpenDerivativeBuy">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionOpenDerivativeBuy" name="BatchTransactionOpenDerivativeBuy">
			<xsl:apply-templates select="b:TransactionOpenDerivativeBuy"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionOpenDerivativeBuy">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionOpenDerivativeBuy" name="BatchMultiTransactionOpenDerivativeBuy">
			<xsl:apply-templates select="c:MultiTransactionOpenDerivativeBuy"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionOpenDerivativeBuy">
		<TransactionPMS mode="INSUPD">
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionOpenDerivativeBuy',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Derivative>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionOpenDerivativeBuy.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
