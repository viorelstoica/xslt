...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS"
   ...
   <xsl:template name="account1">
      <account1>
         <infra:code>
            <xsl:value-of select="../c:settlementrecord/h:settPayOutDetails[h:payoutPptyClass = 'ACCOUNT' and h:payoutSettleActivity = 'LENDING-APPLYPAYMENT-PO.WITHDRAWAL']/h:payoutAccount"/>
         </infra:code>
      </account1>
   </xsl:template>
   ...
   <xsl:template name="account1NetAmount">
      <account1NetAmount>
         <xsl:value-of select=" ../c:billdetails/j:billProperyDetails[j:property='PRINCIPALINT']/j:orPropAmount"/>
      </account1NetAmount>
   </xsl:template>
