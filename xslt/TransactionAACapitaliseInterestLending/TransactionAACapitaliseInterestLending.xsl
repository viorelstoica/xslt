   ...
   <xsl:template match="c:AAAFlow">
      <TransactionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('transactionAACapitaliseInterestLending',c:aatxncontext/b:aaArrangementId)"/>
         </xsl:attribute>
         <DataGroup>
            <xsl:call-template name="mappingTransactionAACapitaliseInterestLending"/>
         </DataGroup>
      </TransactionPMS>
   </xsl:template>
   <xsl:template name="mappingTransactionAACapitaliseInterestLending">
      <xsl:for-each select="c:aaarecord">
         <DepositLoan>
            <xsl:attribute name="description">
               <xsl:value-of select="concat('TransactionAACapitaliseInterestLending.xsl+', name(/*), '+', ../c:eventCommon/ns0:eventId, '+', ../c:eventCommon/ns0:creationTime)"/>
            </xsl:attribute>
...
<metaInformation>
	<scenarios>
		<scenario default="yes" name="Scenario1" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\TransactionAACapitaliseInterestLending\TransactionAACapitaliseInterestLending.xml" htmlbaseurl=""
		          outputurl="..\..\TestFiles\TransactionAACapitaliseInterestLending\TransactionAACapitaliseInterestLendingGWP.xml" processortype="jaxp" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline=""
