...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:a="http://www.temenos.com/T24/AaActivityExtractorService/AaArrInterestRec" xmlns:b="http://www.temenos.com/T24/AaActivityExtractorService/AaTransactionContext"
	...
	<xsl:template name="account1NetAmount">
	    <account1NetAmount>
	      <xsl:choose>
	        <xsl:when test="d:txnSystemId = 'FT' and not(../c:settlementrecord/h:settPayInDetails/h:payinSettlement = 'YES') ">
	          <xsl:value-of select="substring(../c:ftrecord/l:amountDebited,4)"/>
	        </xsl:when>
	        <xsl:when test="d:txnSystemId = 'AAAA' or ../c:settlementrecord/h:settPayInDetails/h:payinSettlement = 'YES'">
	            <xsl:variable name="repayAmount">
	                <xsl:call-template name="sumAAPayments">
	                    <xsl:with-param name="nodeSet" select="../c:billdetails/j:billProperyDetails[j:property = 'ACCOUNT' and starts-with(j:repayRef, ../../c:aatxncontext/b:aaArrActivityId)]"/>
	                </xsl:call-template>
	            </xsl:variable>
	            <xsl:value-of select="format-number($repayAmount,$decimalformat,'nan2zero') "/>     
            </xsl:when>
	      </xsl:choose>
	    </account1NetAmount>
	</xsl:template>
	...
	<xsl:template name="type">
		<type>
			<infra:code>
				<xsl:value-of select="'AA_CloseLending'"/>
			</infra:code>
		</type>
	</xsl:template>
