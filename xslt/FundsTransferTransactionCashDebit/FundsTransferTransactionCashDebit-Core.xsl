...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/FundsTransferTransactionCashDebit" xmlns:infra="http://www.odcgroup.com/InfraPMS"
                xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b ns0" version="1.0">
	<xsl:template name="account1">
		<xsl:if test="b:commissionCode = 'DEBIT PLUS CHARGES'">
			<account1>
				<infra:code>
					<xsl:choose>
						<xsl:when test="b:chargesAcctNo and b:chargesAcctNo != 0">
	...
	<xsl:template name="transactionFeesTaxesCounter">
		<transactionFeesTaxesCounter>
			<amount>
				<xsl:choose>
					<xsl:when test="substring(b:totalChargeAmt,4) &gt; 0 and b:commissionCode = 'DEBIT PLUS CHARGES'">
						<xsl:choose>
		...
		<transactionFeesTaxesCounter>
			<amount>
				<xsl:choose>
					<xsl:when test="substring(b:totalTaxAmount,4) &gt; 0 and b:commissionCode = 'DEBIT PLUS CHARGES'">
	...
	<xsl:template name="quantity">
		<quantity>
			<xsl:choose>
				<xsl:when test="substring(b:totalChargeAmt,4) &gt; 0 and b:commissionCode = 'DEBIT PLUS CHARGES' and not(b:chargesAcctNo) or b:chargesAcctNo = ''">
	...
	<xsl:template name="counterpartCurrency">
		<xsl:if test="b:creditCurrency and b:creditCurrency !='' ">
			<counterpartCurrency>
				<infra:code>
	...
	<xsl:template name="counterpartAccount">
		<xsl:if test="b:creditAcctNo and b:creditAcctNo != ''">
			<counterpartAccount>
	...
	<xsl:template name="extBankAccOwnerName">
		<xsl:if test="b:creditCustomer and b:creditCustomer != ''">
			<extBankAccOwnerName>
				<xsl:value-of select="b:creditCustomer"/>
			</extBankAccOwnerName>
		</xsl:if>
	</xsl:template>
