	...
	<xsl:template match="b:TransactionEntitlement">
		<GenericCOA>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('CoaPurchasePlan',b:diaryId)"/>
			</xsl:attribute>
			<xsl:for-each select="b:optionDescGroup[b:optElectAmt &gt; '0']/b:optElectAmt">
				<xsl:if test="../../b:optStatus = 'COMPLETED' ">
					<DataGroup>
						<Buy>
							<xsl:attribute name="description">
								<xsl:value-of select="concat('TransactionSharePurchasePlan.xsl+', name(/*), '+', ../../b:eventCommon/ns0:eventId, '+', ../../b:eventCommon/ns0:creationTime)"/>
							</xsl:attribute>
					...
					<xsl:if test="../../b:upfrontPaymentDiaryJoin ='YES'">
						<xsl:choose>
							...
							<xsl:when test="../../b:optStatus = 'COMPLETED'">
								<DataGroup>
									<Investment>
										<xsl:attribute name="description">
											<xsl:value-of select="concat('TransactionSharePurchasePlan.xsl+', name(/*), '+', ../../b:eventCommon/ns0:eventId, '+', ../../b:eventCommon/ns0:creationTime)"/>
