...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/FinancialInstrumentOptionContractMaster" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchFinancialInstrumentOptionContractMaster" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiFinancialInstrumentOptionContractMaster" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiFinancialInstrumentOptionContractMaster" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
	<xsl:output encoding="UTF-8" indent="yes" method="xml"/>
	<xsl:template match="c:MultiFinancialInstrumentOptionContractMaster">
		<xsl:apply-templates select="c:CurrentEvent/b:FinancialInstrumentOptionContractMaster"/>
	</xsl:template>
	<xsl:template match="batch:BatchFinancialInstrumentOptionContractMaster">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchFinancialInstrumentOptionContractMaster" name="BatchFinancialInstrumentOptionContractMaster">
			<xsl:apply-templates select="b:FinancialInstrumentOptionContractMaster"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiFinancialInstrumentOptionContractMaster">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiFinancialInstrumentOptionContractMaster" name="BatchMultiFinancialInstrumentOptionContractMaster">
			<xsl:apply-templates select="c:MultiFinancialInstrumentOptionContractMaster"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:FinancialInstrumentOptionContractMaster">
		<FiPMS>
			<xsl:attribute name="mode">INSUPD</xsl:attribute>
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FinancialInstrumentOptionContractMaster',translate(b:id,'/',''))"/>
			</xsl:attribute>
			<DataGroup>
				<Option>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FinancialInstrumentOptionContractMaster.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
