	...
	<xsl:template match="b:SecOpenOrder">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionUnlockCashIpoDebit',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Cash>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionUnlockCashIpoDebit.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
