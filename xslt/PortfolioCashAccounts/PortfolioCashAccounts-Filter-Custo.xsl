...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/PortfolioCashAccounts"
	...
	<xsl:template match="b:PortfolioCashAccounts">
		<xsl:variable name="checkPortfolioConditionVar">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'PortfolioCashAccounts'"/>
		...
		<xsl:apply-templates select="self::b:PortfolioCashAccounts[$checkPortfolioConditionVar = 'true']" mode="filter2"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:PortfolioCashAccounts" mode="filter2">
		<xsl:variable name="currentAccountNo" select="b:accountNos"/>
		<xsl:apply-templates select="self::b:PortfolioCashAccounts[not(../../../c:PreviousEvent/b:PortfolioCashAccounts/b:accountNosGroup[b:accountNos = $currentAccountNo])]" mode="filter3"/>
	</xsl:template>
	<!-- Condition 3 -->
	<xsl:template match="b:PortfolioCashAccounts" mode="filter3">
    <xsl:copy>
        ...
        <xsl:for-each select="b:accountNosGroup">
            ...
            <xsl:if test="((number($categoryValue) &gt;= 1001 and number($categoryValue) &lt;= 1998) or number($categoryValue) = 4002 or number($categoryValue) = 4005 or number($categoryValue) = 6009) or ($IsSuspenseAccountCategoryVar = 'true')">
                <xsl:copy>
                    <xsl:apply-templates select="@* | node()"/>
                </xsl:copy>
            </xsl:if>
	...
	<!-- Custo placeholder -->
	<xsl:template match="b:PortfolioCashAccounts" mode="filter-custo">
		<xsl:apply-templates select="self::b:PortfolioCashAccounts" mode="filter-final"/>
	</xsl:template>
	<!-- Copy block -->
	<xsl:template match="b:PortfolioCashAccounts" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
