   ...
   <xsl:template match="b:FdFiduciary">
      <TransactionPMS mode="INSUPD">
         <xsl:attribute name="output">
            <xsl:value-of select="concat('TransactionOpenFiduciary',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <xsl:for-each select="b:orderNosGroup">
               <DepositLoan>
                  <xsl:attribute name="description">
                     <xsl:value-of select="concat('TransactionOpenFiduciary.xsl+', name(/*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
                  </xsl:attribute>
