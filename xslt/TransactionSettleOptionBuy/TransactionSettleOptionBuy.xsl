...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionSettleOptionBuy"
	...
	<xsl:template match="c:MultiTransactionSettleOptionBuy">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionSettleOptionBuy"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionSettleOptionBuy">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionSettleOptionBuy" name="BatchTransactionSettleOptionBuy">
			<xsl:apply-templates select="b:TransactionSettleOptionBuy"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionSettleOptionBuy">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSettleOptionBuy" name="BatchMultiTransactionSettleOptionBuy">
			<xsl:apply-templates select="c:MultiTransactionSettleOptionBuy"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionSettleOptionBuy">
		<TransactionPMS mode="INSUPD">
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionSettleOptionBuy',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Derivative>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionSettleOptionBuy.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
