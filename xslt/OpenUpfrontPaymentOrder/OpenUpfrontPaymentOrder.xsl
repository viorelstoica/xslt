	...
	<xsl:template match="b:OpenUpfrontPaymentOrder">
		<ClientPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('OpenUpfrontPaymentOrder',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<PortfolioEvent>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('OpenUpfrontPaymentOrder.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
