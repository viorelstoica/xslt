...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SyImfRollover" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiSyImfRollover" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b c ns0" version="1.0">
   ...
   <xsl:template name="account1NetAmount">
      <account1NetAmount>
         <xsl:value-of select="b:depPrinAdjust"/>
      </account1NetAmount>
   </xsl:template>
   ...
   <xsl:template name="transactionCode">
      <transactionCode>
         <xsl:value-of select="concat(b:id,$COMPANY_POSTFIX_SEPARATOR,b:mnemonic,$COMPANY_POSTFIX_SEPARATOR,'DD') "/>
      </transactionCode>
   </xsl:template>
   ...
   <xsl:template name="type">
      <type>
         <infra:code>
            <xsl:value-of select=" 'IMF_DEP_DEC' "/>
         </infra:code>
      </type>
   </xsl:template>
   <xsl:template name="valueDate">
      <valueDate>
         <xsl:call-template name="xsdate">
            <xsl:with-param name="date" select="translate(b:depAdjEffDate,'-','')"/>
         </xsl:call-template>
      </valueDate>
   </xsl:template>
   ...
   <xsl:template name="depositCapitalDecrease">
