...
<xsl:stylesheet exclude-result-prefixes="b c d e f g ns0 batch multibatch"
	...
	<xsl:template name="account1NetAmount">
		<xsl:if test= "b:portransactionrec/d:statusCode != '997' and b:portransactionrec/d:statusCode != '998'">
			<xsl:choose>
				<xsl:when test="b:porpostingandconfirmationrec[f:PorPdFeeType[contains(., 'VAT')]]">
					<account1NetAmount>
						<xsl:value-of select="format-number(b:portransactionadditionalrec/e:postingChargeAmountCredit + b:portransactionadditionalrec/e:postingVATAmountCredit,$decimalformat,'nan2zero')"/>
					</account1NetAmount>
				</xsl:when>
				<xsl:otherwise>
					<account1NetAmount>
						<xsl:value-of select="format-number(b:portransactionrec/d:creditMainAmount - b:portransactionadditionalrec/e:postingMainAmountCredit,$decimalformat,'nan2zero')"/>
					</account1NetAmount>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
	...
	<xsl:template name="portfolio">
		<portfolio>
			<infra:code>
				<xsl:value-of select="b:accountdetails/g:creditPortfolioId"/>
			</infra:code>
		</portfolio>
	</xsl:template>
	...
	<xsl:template name="status">
		...
		<status>
			<xsl:choose>
				<xsl:when test="$boStatus = '999'">
					<xsl:call-template name="statusTranslation">
						<xsl:with-param name="boStatus"
						                select="'ACCOUNTED'"/>
					</xsl:call-template>
				</xsl:when>
				<xsl:when test="$boStatus = '998' or $boStatus = '988'">
					<xsl:call-template name="statusTranslation">
						<xsl:with-param name="boStatus"
						                select="'REJECTED'"/>
					</xsl:call-template>
	...
	<xsl:template name="transactionCode">
		<transactionCode>
			<xsl:value-of select="concat('TPH_',b:portransactionrec/d:fTNumber,'_C')"/>
		</transactionCode>
	</xsl:template>
	<xsl:template name="transactionFeesTaxesCounter">
			<xsl:if test="b:portransactionrec/d:statusCode != '998' and b:portransactionrec/d:statusCode != '997'">
			<transactionFeesTaxesCounter>
				<amount>
					<xsl:choose>
						<xsl:when test="(b:portransactionrec/d:paymentDirection = 'I' or b:portransactionrec/d:paymentDirection = 'B') and b:porpostingandconfirmationrec[f:PorPdFeeType[contains(., 'VAT')]]">
							<xsl:value-of select ="format-number(b:portransactionadditionalrec/e:postingChargeAmountCredit + b:portransactionadditionalrec/e:postingVATAmountCredit,$decimalformat,'nan2zero')"/>
						</xsl:when>
						<xsl:when test="b:portransactionrec/d:paymentDirection = 'I' or b:portransactionrec/d:paymentDirection = 'B'">
							<xsl:value-of select ="format-number(b:portransactionrec/d:creditMainAmount - b:portransactionadditionalrec/e:postingMainAmountCredit,$decimalformat,'nan2zero')"/>
						</xsl:when>
						<xsl:otherwise>
				...
				<currency>
					<infra:code>
						<xsl:if test="b:portransactionrec/d:paymentDirection = 'I' or b:portransactionrec/d:paymentDirection = 'B'">
	...
	<xsl:template name="type">
		<type>
			<infra:code>
				<xsl:choose>
					<xsl:when test="b:portransactionrec/d:paymentDirection = 'I'">
						<xsl:choose>
							<xsl:when test="b:portransactionrec/d:originatingSource='SWIFT'">
	...
	<xsl:template name="valueDate">
		<valueDate>
			<xsl:choose>
				<xsl:when test="b:portransactionrec/d:statusCode = '998' and b:portransactionrec/d:creditValueDate">
					<xsl:call-template name="xsdate">
						<xsl:with-param name="date" select="b:portransactionrec/d:creditValueDate"/>
					</xsl:call-template>	
				</xsl:when>
				<xsl:when test="b:portransactionrec/d:statusCode != '998'">
					<xsl:call-template name="xsdate">
						<xsl:with-param name="date" select="b:portransactionrec/d:creditValueDate"/>
					</xsl:call-template>	
				</xsl:when>
		...
		<quantity>
			<xsl:choose>
				<xsl:when test="b:portransactionrec/d:statusCode != '998' and b:portransactionrec/d:statusCode != '997'">
					<xsl:value-of select="b:portransactionrec/d:creditMainAmount"/>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template name="extBankAccOwnerName">
		<extBankAccOwnerName>
			<xsl:value-of select="b:portransactionrec/d:debitClientID"/>
		</extBankAccOwnerName>
	</xsl:template>
	...
	<xsl:template name="fxClientRate">
		<fxClientRate>
			<xsl:value-of select="b:portransactionrec/d:creditExchangeRate"/>
		</fxClientRate>
	</xsl:template>
	<xsl:template name="fxMarketRate">
		<fxMarketRate>
			<xsl:value-of select="b:portransactionrec/d:creditTreasuryRate"/>
		</fxMarketRate>
	</xsl:template>
