...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SecOpenOrder" xmlns:infra="http://www.odcgroup.com/InfraPMS"
		...
		<transactionCode>
			<xsl:choose>
				<xsl:when test="b:dealStatus = 'CANCELLED'">
					<xsl:value-of select="concat(b:id,'_IPO_LOCKCASH_UL')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="concat(b:id,'_IPO_LOCKCASH')"/>
				</xsl:otherwise>
			</xsl:choose>
		</transactionCode>
	</xsl:template>
	...
	<xsl:template name="remark">
		<remark>
            <xsl:choose>
                <xsl:when test="b:dealStatus = 'CANCELLED'">
                    <xsl:value-of select="'Unlock cash for IPO payment due to cancellation'"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="'Lock cash for IPO payment'"/>
                </xsl:otherwise>
            </xsl:choose>       
		</remark>
	</xsl:template>
