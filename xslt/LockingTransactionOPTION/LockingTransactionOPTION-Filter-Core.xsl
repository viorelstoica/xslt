...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/LockingTransactionCOA"
    ...
    <xsl:template match="b:LockingTransactionCOA" mode="filter4">
        <xsl:apply-templates select="self::b:LockingTransactionCOA[b:blockType = 'OPTION']" mode="filter5"/>
    </xsl:template>
    <!-- Condition 6 -->
    <xsl:template match="b:LockingTransactionCOA" mode="filter5">
        ...
        <xsl:apply-templates select="self::b:LockingTransactionCOA[($opStatus = 'INPUT-INPUT' or $opStatus = 'INPUT-DELETE' or $opStatus = 'INPUT-AUTHORISE') and not($opStatus = 'INPUT-AUTHORISE' and (not(../../c:CurrentEvent/b:LockingTransactionCOA/b:blockNominal) or ../../c:CurrentEvent/b:LockingTransactionCOA/b:blockNominal = '')) and not($opStatus = 'INPUT-INPUT' and (not(../../c:CurrentEvent/b:LockingTransactionCOA/b:blockNominal) or ../../c:CurrentEvent/b:LockingTransactionCOA/b:blockNominal = '') and (not(../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal) or ../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal = ''))]"
                             mode="filter6"/>
    </xsl:template>
    <!-- Condition 7 -->
