...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionSecuritySellReversal" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionSecuritySellReversal"
                xmlns:c="http://www.temenos.com/T24/event/TTI/MultiTransactionSecuritySellReversal" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSecuritySellReversal"
	...
	<xsl:template match="b:TransactionSecuritySellReversal">
		...
		<xsl:apply-templates select="self::b:TransactionSecuritySellReversal[$opStatus = 'REVERSE-AUTHORISE']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:TransactionSecuritySellReversal" mode="filter1">
		<xsl:apply-templates select="self::b:TransactionSecuritySellReversal[($g_filterBySubAssetType='Y' and b:subAssetTypeJoin and contains($list_SubAssetType_TransactionSecuritySellReversal,b:subAssetTypeJoin)) or
	...
	<!-- Condition 3 -->
	<xsl:template match="b:TransactionSecuritySellReversal" mode="filter2">
			...
			<xsl:for-each select="b:customerNoGroup/b:custSecAcc">
				...
				<xsl:variable name="checkPortfolioConditionVar">
					<xsl:call-template name="checkPortfolioCondition">
						<xsl:with-param name="xsltName" select="'TransactionSecuritySellReversal'"/>
		...
		<xsl:apply-templates select="self::b:TransactionSecuritySellReversal[contains($ifFilterWholeRecord,'Y')]" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
	<xsl:template match="b:TransactionSecuritySellReversal" mode="filter3">
		<xsl:apply-templates select="self::b:TransactionSecuritySellReversal[(b:parent != 'YES' or not(b:parent))]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:TransactionSecuritySellReversal" mode="filter-custo">
		<xsl:apply-templates select="self::b:TransactionSecuritySellReversal" mode="filter-final"/>
	</xsl:template>
	<!-- Copy block -->
	<xsl:template match="b:TransactionSecuritySellReversal" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
