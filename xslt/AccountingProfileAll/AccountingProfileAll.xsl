	...
	<xsl:template match="b:CginventoryAll">
		<InfraPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('AccountingProfileAll',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<AccountingProfile>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('AccountingProfileAll.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
