...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch ns0 d e f g" version="1.0" xmlns="http://www.odcgroup.com/StructuredProducts" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template name="floatingRateInstrument">
		<xsl:choose>
			...
			<xsl:when test="b:buildCpnDate != '' and b:interestKey != '' and b:noOfPayments != '' and b:buildCpnDate and b:interestKey and b:noOfPayments">
				<floatingRateInstrument>
					<infra:code>
	...
	<xsl:template name="floatingCondition_userDefinedField">
	...
	<xsl:template name="capitalProtectionLevel">
		<xsl:if test="b:capitalProtection and b:capitalProtection != ''">
			<capitalProtectionLevel>
				<xsl:value-of select="b:capitalProtection"/>
			</capitalProtectionLevel>
		</xsl:if>
	</xsl:template>
	<xsl:template name="capLevel">
		<xsl:if test="b:cap and b:cap != ''">
			<capLevel>
				<xsl:value-of select="b:cap"/>
			</capLevel>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="rebate">
		<xsl:if test="b:rebate and b:rebate != ''">
			<rebate>
				<xsl:value-of select="b:rebate"/>
			</rebate>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="subNature">
		<subNature>
			<xsl:choose>
				<xsl:when test="($g_filterBySubAssetType='Y' and b:subAssetType = '162') or ($g_filterBySubAssetType='N' and b:subAssetTypeTapInstrType and b:subAssetTypeTapInstrType='CAPPROTNOT')">
					<xsl:value-of select="'75'"/>
				</xsl:when>
				<xsl:when test="($g_filterBySubAssetType='Y' and b:subAssetType = '168') or ($g_filterBySubAssetType='N' and b:subAssetTypeTapInstrType and b:subAssetTypeTapInstrType='CAPPROTNOTWC')">
	...
	<xsl:template name="capitalProtectionLevel_MultiEntity"/>
	<xsl:template name="capLevel_MultiEntity"/>
	...
	<xsl:template name="fixingDate_MultiEntity"/>
	...
	<xsl:template name="rebate_MultiEntity"/>
