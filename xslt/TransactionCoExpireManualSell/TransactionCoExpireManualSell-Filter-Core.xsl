...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionCoExpireManualSell"
	...
	<xsl:template match="b:TransactionCoExpireManualSell">
		...
		<xsl:apply-templates select="self::b:TransactionCoExpireManualSell[$opStatus != 'REVERSE-AUTHORISE']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:TransactionCoExpireManualSell" mode="filter1">
		<xsl:apply-templates select="self::b:TransactionCoExpireManualSell[b:unauthAuth = 'AUTHORISED']" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:TransactionCoExpireManualSell" mode="filter-custo">
		<xsl:apply-templates select="self::b:TransactionCoExpireManualSell" mode="filter-final"/>
	</xsl:template>
	<!-- Copy block -->
	<xsl:template match="b:TransactionCoExpireManualSell" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
