...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template match="c:MultiTransactionMaturityFutureSellCloseOutRev">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionMaturityFutureSellCloseOutRev"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionMaturityFutureSellCloseOutRev">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionMaturityFutureSellCloseOutRev" name="BatchTransactionMaturityFutureSellCloseOutRev">
			<xsl:apply-templates select="b:TransactionMaturityFutureSellCloseOutRev"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionMaturityFutureSellCloseOutRev">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionMaturityFutureSellCloseOutRev" name="BatchMultiTransactionMaturityFutureSellCloseOutRev">
			<xsl:apply-templates select="c:MultiTransactionMaturityFutureSellCloseOutRev"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionMaturityFutureSellCloseOutRev">
		<TransactionPMS mode="INSUPD">
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionMaturityFutureSellCloseOutRev',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Derivative>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionMaturityFutureSellCloseOutRev.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
