...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionSecurityBuyReversal" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionSecurityBuyReversal" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiTransactionSecurityBuyReversal" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSecurityBuyReversal" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/>
   <xsl:template match="c:MultiTransactionSecurityBuyReversal">
      <xsl:apply-templates select="c:CurrentEvent/b:TransactionSecurityBuyReversal"/>
   </xsl:template>
   <xsl:template match="batch:BatchTransactionSecurityBuyReversal">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionSecurityBuyReversal" name="BatchTransactionSecurityBuyReversal">
         <xsl:apply-templates select="b:TransactionSecurityBuyReversal"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiTransactionSecurityBuyReversal">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSecurityBuyReversal" name="BatchMultiTransactionSecurityBuyReversal">
         <xsl:apply-templates select="c:MultiTransactionSecurityBuyReversal"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:TransactionSecurityBuyReversal">
      <TransactionPMS>
         <xsl:attribute name="mode">INSUPD</xsl:attribute>
         <xsl:attribute name="output">
            <xsl:value-of select="concat('transactionSecurityBuyReversal',b:id)"/>
         </xsl:attribute>
         <xsl:for-each select="b:customerNoGroup/b:custSecAcc">
            ...
            </xsl:variable><!--TransactionSecurityReversal-->
            <DataGroup>
               <Security>
                  <xsl:attribute name="description">
                     <xsl:value-of select="concat('TransactionSecurityBuyReversal.xsl+', name(/*), '+', ../../b:eventCommon/ns0:eventId, '+', ../../b:eventCommon/ns0:creationTime)"/>
                  </xsl:attribute>
            ...
            <xsl:if test="../b:cuIncomeAcc and (../b:cuIncomeAmt and number(../b:cuIncomeAmt) != '0') and (../../b:segregIncomeJoinList/b:segregIncomeJoin = 'YES' and ../b:custIntrAmt) and (not(../../b:custAccNoPortfolioNoGroup[@index=$index]/b:custAccNoPortfolioNoList/b:custAccNoPortfolioNo = ../../b:portfolioNoJoinGroup[@index=$index]/b:portfolioNoJoinList/b:portfolioNoJoin)) and ../../b:exclIncValuationScParameterJoin = 'YES'">
               <DataGroup>
                  <Cash>
                     <xsl:attribute name="description">
                        <xsl:value-of select="concat('TransactionSecurityBuyReversal.xsl+', name(/*), '+', ../../b:eventCommon/ns0:eventId, '+', ../../b:eventCommon/ns0:creationTime)"/>
			...
			<xsl:if test="../b:custTransCode = 'UPS' and ../b:cuOrderNosList/b:cuOrderNos !=''">
					<DataGroup>
						<Cash>
							...
							<transferIn>
								<xsl:call-template name="account1_UpfrontPayInveRev"/>
								<xsl:call-template name="account1CcyTradeCcyExchangeRate_UpfrontPayInveRev"/>
								<xsl:call-template name="account1NetAmount_UpfrontPayInveRev"/>
								<xsl:call-template name="account2_UpfrontPayInveRev"/>
								<xsl:call-template name="account2CcyTradeCcyExchangeRate_UpfrontPayInveRev"/>
								<xsl:call-template name="account2NetAmount_UpfrontPayInveRev"/>
								<xsl:call-template name="account3_UpfrontPayInveRev"/>
								<xsl:call-template name="account3CcyTradeCcyExchangeRate_UpfrontPayInveRev"/>
								<xsl:call-template name="account3NetAmount_UpfrontPayInveRev"/>
								<xsl:call-template name="accountingCode_UpfrontPayInveRev"/>
								<xsl:call-template name="accountingDate_UpfrontPayInveRev"/>
								<xsl:call-template name="cashPortfolio_UpfrontPayInveRev"/>
								<xsl:call-template name="commonReference_UpfrontPayInveRev"/>
								<xsl:call-template name="ContractNumber_UpfrontPayInveRev"/>
								<xsl:call-template name="intermediary_UpfrontPayInveRev"/>
								<xsl:call-template name="market_UpfrontPayInveRev"/>
								<xsl:call-template name="notepad_UpfrontPayInveRev"/>
								<xsl:call-template name="portfolio_UpfrontPayInveRev"/>
								<xsl:call-template name="portfolioManager_UpfrontPayInveRev"/>
								<xsl:call-template name="remark_UpfrontPayInveRev"/>
								<xsl:call-template name="reversalIndicator_UpfrontPayInveRev"/>
								<xsl:call-template name="reversedTransactionCode_UpfrontPayInveRev"/>
								<xsl:call-template name="sourceSystemCode_UpfrontPayInveRev"/>
								<xsl:call-template name="status_UpfrontPayInveRev"/>
								<xsl:call-template name="subType_UpfrontPayInveRev"/>
								<xsl:call-template name="transactionCode_UpfrontPayInveRev"/>
								<xsl:call-template name="transactionFeesTaxesCounter_UpfrontPayInveRev"/>
								<xsl:call-template name="type_UpfrontPayInveRev"/>
								<xsl:call-template name="valueDate_UpfrontPayInveRev"/>
								<xsl:call-template name="userDefinedField_UpfrontPayInveRev"/>
								<xsl:call-template name="mbFields_UpfrontPayInveRev"/>
								<xsl:call-template name="currentAccount_UpfrontPayInveRev"/>
								<xsl:call-template name="quantity_UpfrontPayInveRev"/>
								<xsl:call-template name="transactionCcyGrossAmount_UpfrontPayInveRev"/>
								<xsl:call-template name="transactionCcyNetAmount_UpfrontPayInveRev"/>
								<xsl:call-template name="transactionCcyPortfolioCcyExchangeRate_UpfrontPayInveRev"/>
								<xsl:call-template name="transactionCurrency_UpfrontPayInveRev"/>
								<xsl:call-template name="transactionDate_UpfrontPayInveRev"/>
							</transferIn>
						</Cash>
					</DataGroup>
				</xsl:if>
