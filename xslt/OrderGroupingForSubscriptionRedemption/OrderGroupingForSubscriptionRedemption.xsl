...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/OrderGroupingForSubscriptionRedemption"
                ...
                xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiOrderGroupingForSubscriptionRedemption" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
	<xsl:output encoding="UTF-8" indent="yes" method="xml"/>
	<xsl:template match="c:MultiOrderGroupingForSubscriptionRedemption">
		<xsl:apply-templates select="c:CurrentEvent/b:OrderGroupingForSubscriptionRedemption"/>
	</xsl:template>
	<xsl:template match="batch:BatchOrderGroupingForSubscriptionRedemption">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchOrderGroupingForSubscriptionRedemption" name="BatchOrderGroupingForSubscriptionRedemption">
			<xsl:apply-templates select="b:OrderGroupingForSubscriptionRedemption"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiOrderGroupingForSubscriptionRedemption">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiOrderGroupingForSubscriptionRedemption" name="BatchMultiOrderGroupingForSubscriptionRedemption">
			<xsl:apply-templates select="c:MultiOrderGroupingForSubscriptionRedemption"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:OrderGroupingForSubscriptionRedemption">
		<FiPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('OrderGroupingForSubscriptionRedemption',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<FundShare>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('OrderGroupingForSubscriptionRedemption.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
