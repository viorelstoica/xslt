...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/LdLoansAndDeposits" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchLdLoansAndDeposits" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiLdLoansAndDeposits" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiLdLoansAndDeposits" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/>
   <xsl:template match="c:MultiLdLoansAndDeposits">
      <xsl:apply-templates select="c:CurrentEvent/b:LdLoansAndDeposits"/>
   </xsl:template>
   <xsl:template match="batch:BatchLdLoansAndDeposits">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchLdLoansAndDeposits" name="BatchLdLoansAndDeposits">
         <xsl:apply-templates select="b:LdLoansAndDeposits"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiLdLoansAndDeposits">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiLdLoansAndDeposits" name="BatchMultiLdLoansAndDeposits">
         <xsl:apply-templates select="c:MultiLdLoansAndDeposits"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:LdLoansAndDeposits">
      <FiPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FinancialInstrumentLD',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <MoneyMarket>
               <SpecificMoneyMarket>
                  <xsl:attribute name="description">
                     <xsl:value-of select="concat('FinancialInstrumentLD.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
                  </xsl:attribute>
...
<metaInformation>
	<scenarios>
		<scenario default="yes" name="FinancialInstrumentLD" userelativepaths="yes" externalpreview="no" url="..\..\..\..\testfiles\LdLoansAndDeposits\LdLoansAndDeposits.xml" htmlbaseurl=""
		          outputurl="..\..\TestFiles\LdLoansAndDeposits\FinancialInstrumentLDGWP.xml" processortype="saxon8" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath=""
