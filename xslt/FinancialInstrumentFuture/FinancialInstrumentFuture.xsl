...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/FinancialInstrumentFuture" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchFinancialInstrumentFuture" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiFinancialInstrumentFuture" xmlns:d="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiFinancialInstrumentFuture" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/>
   <xsl:template match="c:MultiFinancialInstrumentFuture">
      <xsl:apply-templates select="c:CurrentEvent/b:FinancialInstrumentFuture"/>
   </xsl:template>
   <xsl:template match="batch:BatchFinancialInstrumentFuture">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchFinancialInstrumentFuture" name="BatchFinancialInstrumentFuture">
         <xsl:apply-templates select="b:FinancialInstrumentFuture"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiFinancialInstrumentFuture">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiFinancialInstrumentFuture" name="BatchMultiFinancialInstrumentFuture">
         <xsl:apply-templates select="c:MultiFinancialInstrumentFuture"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:FinancialInstrumentFuture">
      <FiPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FinancialInstrumentFuture',translate(translate(b:id,'/',''),':',''))"/>
         </xsl:attribute>
         <DataGroup>
            <Future>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('FinancialInstrumentFuture.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
