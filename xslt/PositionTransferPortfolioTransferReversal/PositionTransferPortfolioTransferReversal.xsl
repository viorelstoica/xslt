...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PositionTransferPortfolioTransferReversal"
	...
	<xsl:template match="c:MultiPositionTransferPortfolioTransferReversal">
		<xsl:apply-templates select="c:CurrentEvent/b:PositionTransferPortfolioTransferReversal"/>
	</xsl:template>
	<xsl:template match="batch:BatchPositionTransferPortfolioTransferReversal">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchPositionTransferPortfolioTransferReversal" name="BatchPositionTransferPortfolioTransferReversal">
			<xsl:apply-templates select="b:PositionTransferPortfolioTransferReversal"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiPositionTransferPortfolioTransferReversal">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiPositionTransferPortfolioTransferReversal" name="BatchMultiPositionTransferPortfolioTransferReversal">
			<xsl:apply-templates select="c:MultiPositionTransferPortfolioTransferReversal"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:PositionTransferPortfolioTransferReversal">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('PositionTransferPortfolioTransferReversal',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<xsl:for-each select="b:securityNoGroup">
					<Security>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('PositionTransferPortfolioTransferReversal.xsl+', name(/*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
						...
						<portfolioTransfer>
							...
							<xsl:call-template name="depositTo"/>
							<xsl:call-template name="portfolioTo"/>
						</portfolioTransfer>
					</Security>
				</xsl:for-each>
			</DataGroup>
