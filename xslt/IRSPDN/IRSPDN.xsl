	...
	<xsl:template match="b:IRSNotionalPrinChange">
		<TransactionPMS mode="INSUPD">
			<xsl:attribute name="output">
				<xsl:value-of select="concat('IRSPDN', b:swapbalancesrecord/ns1:swapId)"/>
			</xsl:attribute>
			<DataGroup>
				<IrSwap>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('IRSPDN.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
