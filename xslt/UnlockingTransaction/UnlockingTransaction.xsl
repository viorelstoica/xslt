...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template match="batch:BatchUnlockingTransaction">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchUnlockingTransaction" name="BatchUnlockingTransaction">
			<xsl:apply-templates select="b:UnlockingTransaction"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:UnlockingTransaction">
		<TransactionPMS mode="INSUPD">
			<xsl:attribute name="output">
				<xsl:value-of select="concat('UnlockingTransaction',b:autoUnblockRec/ns1:idSecurityPosition)"/>
			</xsl:attribute>
			<DataGroup>
				<Security>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('UnlockingTransaction.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
