...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionSettleOptionSell"
	...
	<xsl:template match="c:MultiTransactionSettleOptionSell">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionSettleOptionSell"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionSettleOptionSell">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionSettleOptionSell" name="BatchTransactionSettleOptionSell">
			<xsl:apply-templates select="b:TransactionSettleOptionSell"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionSettleOptionSell">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSettleOptionSell" name="BatchMultiTransactionSettleOptionSell">
			<xsl:apply-templates select="c:MultiTransactionSettleOptionSell"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionSettleOptionSell">
		<TransactionPMS mode="INSUPD">
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionSettleOptionSell',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Derivative>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionSettleOptionSell.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
