...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PositionTransferScSettlement"
	...
	<!-- Condition 5 portfolio filter for secAcctFrom -->
	<xsl:template match="b:PositionTransferScSettlement" mode="filter4">
		<xsl:variable name="checkPortfolioConditionVar">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'PTPortfolioTransferActualSettlement'"/>
		...
		<xsl:apply-templates select="self::b:PositionTransferScSettlement[$checkPortfolioConditionVar = 'true']" mode="filter5"/>
	</xsl:template>
	<!-- Condition 6 portfolio filter for secAcctTo -->
	<xsl:template match="b:PositionTransferScSettlement" mode="filter5">
		<xsl:variable name="checkPortfolioConditionVar1">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'PTPortfolioTransferActualSettlement1'"/>
		...
		<xsl:apply-templates select="self::b:PositionTransferScSettlement[$checkPortfolioConditionVar1 = 'true']" mode="filter6"/>
	</xsl:template>
	<!-- Condition 7 -->
	...
	<xsl:template match="b:PositionTransferScSettlement" mode="filter7">
		<xsl:apply-templates select="self::b:PositionTransferScSettlement[b:application = 'POSITION.TRANSFER']" mode="filter8"/>
	</xsl:template>
	<!-- Condition 9 -->
