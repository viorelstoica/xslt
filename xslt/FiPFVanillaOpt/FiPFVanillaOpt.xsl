   ...
   <xsl:template match="b:SyFxForwards">
      <FiPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FiPFVanillaOpt',translate(translate(b:id,'/',''),':',''))"/>
         </xsl:attribute>
         <DataGroup>
            <Option>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('FiPFVanillaOpt.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
