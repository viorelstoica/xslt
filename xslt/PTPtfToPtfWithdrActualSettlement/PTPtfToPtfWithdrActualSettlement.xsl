	...
	<xsl:template match="b:PositionTransferScSettlement">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('PTPtfToPtfWithdrActualSettlement',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<xsl:for-each select="b:securityAcctGroup">
					<Security>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('PTPtfToPtfWithdrActualSettlement.xsl+', name(/*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
