...
function Table2() {
  ...
  const dataFetch = async () => {
    const response = await fetch(`http://localhost:3000/output/stats2`)
  ...
  function sortFunction(col) {
    switch (col) {
      case 1:
        data.sort((a,b) => {
          return sortOrder[col-1]*(a.cmd < b.cmd ? -1 : a.cmd > b.cmd ? 1 : 0)
        })
        break;
      case 2:
        data.sort((a,b) => {
          ...
          <div className="table-responsive">
            <table className="table table-sm table-hover align-middle">
              <thead className="table-light sticky-top">
                <tr>
                  <th className="sortable" onClick={() => sortFunction(1)}>Entity</th>
              ...
              <tbody>
                {data.map(d => (
                  <tr className="table-light">
                    <td className="fw-bold text-secondary">{d.cmd}</td>
