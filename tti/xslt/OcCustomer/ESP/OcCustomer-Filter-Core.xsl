...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/OcCustomer"
	...
	<xsl:template match="b:OcCustomer">
		<xsl:apply-templates select="self::b:OcCustomer[b:customerSecurityJoinId = b:id]" mode="filter-custo"/>
	</xsl:template>

	<!-- Custo placeholder -->
	<xsl:template match="b:OcCustomer" mode="filter-custo">
		<xsl:apply-templates select="self::b:OcCustomer" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:OcCustomer" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
