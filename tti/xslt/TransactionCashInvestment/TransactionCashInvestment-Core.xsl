      ...
      <account1NetAmount>
         <xsl:choose>
            <xsl:when test="b:statemententry[e:transactionCode/@Rank=$MinRank]/e:amountFcy and b:statemententry[e:transactionCode/@Rank=$MinRank]/e:amountFcy != ''">
               <xsl:value-of select="format-number(-1 * sum(b:statemententry[e:transactionCode/@Rank!=$MinRank]/e:amountFcy),$decimalformat,'nan2zero')"/>
            </xsl:when>
            <xsl:otherwise>
               <xsl:value-of select="format-number(-1 * sum(b:statemententry[e:transactionCode/@Rank!=$MinRank]/e:amountLcy),$decimalformat,'nan2zero')"/>
            </xsl:otherwise>
         </xsl:choose>
      </account1NetAmount>
   </xsl:template>
