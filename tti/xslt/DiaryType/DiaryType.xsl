...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/DiaryType" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchDiaryType" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiDiaryType" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiDiaryType" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/>
   <xsl:template match="c:MultiDiaryType">
      <xsl:apply-templates select="c:CurrentEvent/b:DiaryType"/>
   </xsl:template>
   <xsl:template match="batch:BatchDiaryType">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchDiaryType" name="BatchDiaryType">
         <xsl:apply-templates select="b:DiaryType"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiDiaryType">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiDiaryType" name="BatchMultiDiaryType">
         <xsl:apply-templates select="c:MultiDiaryType"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:DiaryType">
      <InfraPMS>
         <xsl:attribute name="mode">INSUPD</xsl:attribute>
         <xsl:attribute name="output">
            <xsl:value-of select="concat('DiaryType',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <Type>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('DiaryType.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="DiaryType" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\DiaryType\DiaryType.xml" htmlbaseurl="" outputurl="..\..\TestFiles\DiaryType\DiaryTypeCusto.xml" processortype="jaxp" useresolver="no"
		          profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext=""
		          validateoutput="no" validator="internal" customvalidator="">
			<advancedProp name="sInitialMode" value=""/>
