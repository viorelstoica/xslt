...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/PositionPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/SyImf" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSyImf" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiSyImf" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiSyImf" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
  ...
  <xsl:template match="b:SyImf">
    <xsl:apply-templates select="self::b:SyImf[b:suppressUnderlying = 'NO']" mode="filter1"/>
  </xsl:template>
  <!-- Condition 2 -->
  <xsl:template match="b:SyImf" mode="filter1">
    <xsl:apply-templates select="self::b:SyImf[b:contractStatus = 'ACTIVE']" mode="filter2"/>
  </xsl:template>
  <!-- Condition 3 -->
  <xsl:template match="b:SyImf" mode="filter2">
    ...
    <xsl:apply-templates select="self::b:SyImf[number($check_MaturityDate) &gt;= number($check_toDay)]" mode="filter3"/>
  </xsl:template>
  <!-- Condition 4 -->
  <xsl:template match="b:SyImf" mode="filter3">
    <xsl:apply-templates select="self::b:SyImf[b:portfolio != '']" mode="filter4"/>
  </xsl:template>
  <!-- Condition 5 -->
  <xsl:template match="b:SyImf" mode="filter4">
    <xsl:apply-templates select="self::b:SyImf[b:dealerBook = '' or not(b:dealerBook)]" mode="filter5"/>
  </xsl:template>
  <!-- Condition 6 -->
  <xsl:template match="b:SyImf" mode="filter5">
...
<metaInformation>
	<scenarios>
		<scenario default="yes" name="PositionFiduciary" userelativepaths="no" externalpreview="no" url="file:///d:/SVN/trunk/TestFiles/PositionFiduciary/PositionFiduciary.xml" htmlbaseurl=""
		          outputurl="file:///d:/SVN/trunk/TestFiles/PositionFiduciary/PositionFiduciaryCusto.xml" processortype="saxon8" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath=""
