	...
	<xsl:template match="b:FdFidOrder">
		<TransactionPMS mode="INSUPD">
			<xsl:attribute name="output">
				<xsl:value-of select="concat('PositionFiduciaryFWD',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<DepositLoan>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('PositionFiduciaryFWD.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
