...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SyImf" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b ns0" version="1.0">
   ...
   <xsl:template name="transactionCode">
      <transactionCode>
         <xsl:value-of select="concat('Loan Unwind',b:id,$COMPANY_POSTFIX_SEPARATOR,b:mnemonic,$COMPANY_POSTFIX_SEPARATOR) "/>
      </transactionCode>
   </xsl:template>
   <xsl:template name="transactionDate">
      <transactionDate>
         <xsl:call-template name="xsdate">
            <xsl:with-param name="date" select="translate(b:earlyMaturityDate,'-','')"/>
         </xsl:call-template>
      </transactionDate>
   </xsl:template>
   ...
   <xsl:template name="type">
      <type>
         <infra:code>
            <xsl:value-of select=" 'SY_LOAN_CLOSE' "/>
         </infra:code>
      </type>
   </xsl:template>
