...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/FullPartialExecution" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchFullPartialExecution"
	...
	<xsl:template match="b:FullPartialExecution">
		<xsl:apply-templates select="self::b:FullPartialExecution[b:securityMasterBlockType = 'DEBIT']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:FullPartialExecution" mode="filter1">
		<xsl:apply-templates select="self::b:FullPartialExecution[b:memoSecAccMasterJoinList/b:memoSecAccMasterJoin = 'NO']" mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -->
	<xsl:template match="b:FullPartialExecution" mode="filter2">
		<xsl:apply-templates select="self::b:FullPartialExecution[b:dealerBookSecAccMasterJoinList/b:dealerBookSecAccMasterJoin = '' or not(b:dealerBookSecAccMasterJoinList/b:dealerBookSecAccMasterJoin)]" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
