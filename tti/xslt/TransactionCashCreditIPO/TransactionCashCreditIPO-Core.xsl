...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/FullPartialExecution" xmlns:infra="http://www.odcgroup.com/InfraPMS"
	...
	<xsl:template name="transactionCode">
		<transactionCode>
			<xsl:value-of select="concat(b:id,'_IPO_CREDIT')"/>
		</transactionCode>
	</xsl:template>
	...
	<xsl:template name="quantity">
		<quantity>
			<xsl:value-of select="b:idcuSubscriptionAmountGroup/b:idcuSubscriptionAmount"/>
		</quantity>
	</xsl:template>
	<xsl:template name="remark">
		<remark>
			<xsl:value-of select="'Credit cash for IPO payment'"/>
		</remark>
	</xsl:template>
	...
	<xsl:template name="type">
		<type>
			<infra:code>
				<xsl:value-of select="'IPO_CREDIT'"/>
			</infra:code>
		</type>
	</xsl:template>
	...
	<xsl:template name="valueDate">
	   <valueDate>
            <xsl:value-of select="b:orderNumberValueDate"/>	   
	   </valueDate>
	</xsl:template>
