...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/UpfrontPaymentWith" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchUpfrontPaymentWith" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiUpfrontPaymentWith" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiUpfrontPaymentWith" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b ns0" version="1.0">
   ...
   <xsl:template match="c:MultiUpfrontPaymentWith">
      <xsl:apply-templates select="c:CurrentEvent/b:UpfrontPaymentWith"/>
   </xsl:template>
   <xsl:template match="batch:BatchUpfrontPaymentWith">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchUpfrontPaymentWith" name="BatchUpfrontPaymentWith">
         <xsl:apply-templates select="b:UpfrontPaymentWith"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiUpfrontPaymentWith">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiUpfrontPaymentWith" name="BatchMultiUpfrontPaymentWith">
         <xsl:apply-templates select="c:MultiUpfrontPaymentWith"/>
      </xsl:element>
   </xsl:template>
      ...
      <TransactionPMS>
         <xsl:attribute name="mode">INSUPD</xsl:attribute>
			<xsl:attribute name="output">
				<xsl:value-of select="concat('UpfrontPaymentWith',b:id)"/>
			</xsl:attribute>
         <DataGroup>
            <Cash>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('UpfrontPaymentWith.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
