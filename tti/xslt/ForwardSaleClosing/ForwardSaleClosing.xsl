...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/ForwardSaleClosing" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchForwardSaleClosing"
	...
	<xsl:template match="c:MultiForwardSaleClosing">
		<xsl:apply-templates select="c:CurrentEvent/b:ForwardSaleClosing"/>
	</xsl:template>
	<xsl:template match="batch:BatchForwardSaleClosing">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchForwardSaleClosing" name="BatchForwardSaleClosing">
			<xsl:apply-templates select="b:ForwardSaleClosing"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiForwardSaleClosing">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiForwardSaleClosing" name="BatchMultiForwardSaleClosing">
			<xsl:apply-templates select="c:MultiForwardSaleClosing"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:ForwardSaleClosing">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('ForwardSaleClosing',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<ForexForward>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('ForwardSaleClosing.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
