...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/ForwardSaleClosing" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchForwardSaleClosing"
	...
	<xsl:template name="operationDate">
		<operationDate>
			<xsl:call-template name="xsdate">
				<xsl:with-param name="date" select="translate(b:valueDateBuy,'-','')"/>
			</xsl:call-template>
		</operationDate>
	</xsl:template>
	...
	<xsl:template name="contractReferenceCode">
		<contractReferenceCode>
			<!-- ref_oper_code = code of opening transaction -->
			<xsl:choose>
				<xsl:when test="b:tapRefId">
					<!-- order created on TAP -->
					<xsl:value-of select="b:tapRefId"/>
	...
	<xsl:template name="soldCcyNetAmount">
		<soldCcyNetAmount>
			<!-- acc_net_amount_m -->
			<xsl:value-of select="b:amountSold"/>
		</soldCcyNetAmount>
	</xsl:template>
	<xsl:template name="soldCcyCurrentAccount">
	...
	<xsl:template name="purchasedQuantity">
		<purchasedQuantity>
			<!-- quantity_n -->
			<xsl:value-of select="b:amountSold"/>
		</purchasedQuantity>
