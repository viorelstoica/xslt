...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionMaturityFutureBuy"
	...
	<xsl:template match="c:MultiTransactionMaturityFutureBuy">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionMaturityFutureBuy"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionMaturityFutureBuy">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionMaturityFutureBuy" name="BatchTransactionMaturityFutureBuy">
			<xsl:apply-templates select="b:TransactionMaturityFutureBuy"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionMaturityFutureBuy">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionMaturityFutureBuy" name="BatchMultiTransactionMaturityFutureBuy">
			<xsl:apply-templates select="c:MultiTransactionMaturityFutureBuy"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionMaturityFutureBuy">
		<TransactionPMS mode="INSUPD">
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionMaturityFutureBuy',b:id)"/>
			</xsl:attribute>
			<xsl:for-each select="b:closeoutIdList/b:closeoutId">
				<DataGroup>
					<Derivative>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('TransactionMaturityFutureBuy.xsl+', name(/*), '+', ../../b:eventCommon/ns0:eventId, '+', ../../b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
