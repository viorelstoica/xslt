...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionMaturityFutureBuy"
	...
	<xsl:template name="account1NetAmount">
		<xsl:variable name="index" select="@index"/>
		<account1NetAmount>
			<xsl:value-of select="../../b:sellValueList/b:sellValue[@index=$index]"/>
		</account1NetAmount>
	</xsl:template>
	...
	<xsl:template name="quantity">
		<xsl:variable name="index" select="@index"/>
		<quantity>
			<xsl:value-of select="format-number(sum(../../b:tradeIdGroup[@index=$index]/b:tradeIdSubGroup/b:traBuyLots),$decimalformat,'nan2zero')"/>
		</quantity>
	</xsl:template>
	...
	<xsl:template name="transactionFeesTaxesCounter">
		<xsl:call-template name="printDxMaturityFeesAndTaxes">
			<xsl:with-param name="caller" select="'TransactionMaturityFutureBuy'"/>
		</xsl:call-template>
	</xsl:template>
