...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SyAccuDecu" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b ns0" version="1.0">
   ...
   <xsl:template name="currentAccount">
      <currentAccount>
         <infra:code>
            <xsl:value-of select="b:blockAccount"/>
         </infra:code>
      </currentAccount>
   </xsl:template>
   ...
   <xsl:template name="transactionCode">
      <transactionCode>
         <xsl:variable name="BlockedRefAC" select="b:blockedRefAc"/>
         ...
         <xsl:choose>
            <xsl:when test="b:contractStatus = 'ACTIVE'">
               <xsl:value-of select="concat($BlockedRefAC, $COMPANY_POSTFIX_SEPARATOR, $mnemonic, $COMPANY_POSTFIX_SEPARATOR, 'LOCK')"/>
            </xsl:when>
            <xsl:when test="(b:contractStatus = 'UNWIND' or 'MATURED' or 'KNOCKEDOUT') or ($opStatus = 'REVERSE-AUTHORISE')">
               <xsl:value-of select="concat($BlockedRefAC, $COMPANY_POSTFIX_SEPARATOR, $mnemonic, $COMPANY_POSTFIX_SEPARATOR, 'UNLOCK')"/>
            </xsl:when>
         </xsl:choose>
      </transactionCode>
   </xsl:template>
