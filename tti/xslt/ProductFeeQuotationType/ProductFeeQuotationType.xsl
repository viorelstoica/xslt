	...
	<xsl:template match="b:EBLookup">
		<InfraPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('EBLookup',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Type>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('ProductFeeQuotationType.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
