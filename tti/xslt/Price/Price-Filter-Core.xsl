...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch" version="1.0" xmlns="http://www.odcgroup.com/FiPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/Price" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchPrice"
                xmlns:c="http://www.temenos.com/T24/event/TTI/MultiPrice" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiPrice" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- Condition 1 -->
	<xsl:template match="b:Price">
		<xsl:apply-templates select="self::b:Price[($g_filterBySubAssetType='Y' and contains(concat(' ',$list_SubAssetType_Price,' '),concat(' ',b:SecurityMasterJoinSubAssetType,' '))) or
									($g_filterBySubAssetType != 'Y' and b:subAssetTypeTapInstrType and (b:subAssetTypeTapInstrType='STOCK' or b:subAssetTypeTapInstrType='MUTUALFUND' or b:subAssetTypeTapInstrType='FIXEDINCOME' or 
									 ...
									 b:subAssetTypeTapInstrType='REVCONVCREDLK' or b:subAssetTypeTapInstrType='MINIFUTTURBO1' or b:subAssetTypeTapInstrType='MINIFUTTURBO2' or b:subAssetTypeTapInstrType='INDEX'))]" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:Price" mode="filter1">
		<xsl:apply-templates select="self::b:Price[number(translate(b:dateChange,'-','')) &gt;= number(translate($securityPriceStartDate,'-',''))]" mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -->
	<xsl:template match="b:Price" mode="filter2">
		<xsl:apply-templates select="self::b:Price[b:newPriceGroup/b:newPrice != '']" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:Price" mode="filter-custo">
		<xsl:apply-templates select="self::b:Price" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:Price" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
