...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/F2B" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/T24StatusChange"
	...
	<xsl:template match="c:MultiT24StatusChange">
		<xsl:apply-templates select="c:CurrentEvent/b:T24StatusChange"/>
	</xsl:template>
	<xsl:template match="batch:BatchT24StatusChange">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchT24StatusChange" name="BatchT24StatusChange">
			<xsl:apply-templates select="b:T24StatusChange"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiT24StatusChange">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiT24StatusChange" name="BatchMultiT24StatusChange">
			<xsl:apply-templates select="c:MultiT24StatusChange"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:T24StatusChange">
		<F2B mode="INSUPD">
			<xsl:attribute name="output">
				<xsl:value-of select="concat('T24StatusChange',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Security>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('T24StatusChange.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
