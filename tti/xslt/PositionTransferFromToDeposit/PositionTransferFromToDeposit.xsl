...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/GenericCOA" xmlns:b="http://www.temenos.com/T24/event/TTI/PositionTransferFromToDeposit"
	...
	<xsl:template match="c:MultiPositionTransferFromToDeposit">
		<xsl:apply-templates select="c:CurrentEvent/b:PositionTransferFromToDeposit"/>
	</xsl:template>
	<xsl:template match="batch:BatchPositionTransferFromToDeposit">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchPositionTransferFromToDeposit" name="BatchPositionTransferFromToDeposit">
			<xsl:apply-templates select="b:PositionTransferFromToDeposit"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiPositionTransferFromToDeposit">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiPositionTransferFromToDeposit" name="BatchMultiPositionTransferFromToDeposit">
			<xsl:apply-templates select="c:MultiPositionTransferFromToDeposit"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:PositionTransferFromToDeposit">
		<GenericCOA>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('positionTransferFromToDeposit',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<xsl:for-each select="b:securityAcctGroup">
					<Adjustment>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('PositionTransferFromToDeposit.xsl+', name(/*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
