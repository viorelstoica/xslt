...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:a="http://www.temenos.com/T24/AaActivityExtractorService/AaArrInterestRec"
	...
	<xsl:template match="c:aaarecord">
		<xsl:if test="../c:aatxncontext/b:aaMasterActivity = 'LENDING-SETTLE-PAYOFF' 
                and d:activity = 'LENDING-SETTLE-PAYOFF'        
				and ../c:recaccountdetails/f:actBillDetails[f:billType = 'PAYOFF']/f:billId = ../c:billdetails/j:aaBillId 
				and contains($list_auth_TransactionAAPayOffInterest,../c:recarrangement/i:arrStatus) 
				and ../c:accountrecord/g:portfolioId != '' 
				and (not(../c:aatxncontext/b:aaActivityStatus) or ../c:aatxncontext/b:aaActivityStatus != 'REV')
				and not(../c:billdetails[j:aaBillId = ../c:recaccountdetails/f:actBillDetails[f:billType = 'PAYOFF']/f:billId]/j:billProperyDetails[j:property = 'PRINCIPALINT' and (not(j:orPropAmount) or j:orPropAmount = '0')])">
			<xsl:copy-of select="."/>
		</xsl:if>
	</xsl:template>
