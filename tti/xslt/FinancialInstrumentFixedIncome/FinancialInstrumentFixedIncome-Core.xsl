...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch ns0 d e f g" version="1.0" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template name="accrualRule">
		...
		<xsl:variable name="list_InterestDayBasis19" select="'U O'"/>
		<xsl:variable name="list_InterestDayBasis21" select="'C C1'"/>
		<xsl:variable name="list_InterestDayBasis24" select="'A A1 A2 A3 A4'"/>
		<xsl:variable name="list_InterestDayBasis26" select="'F F1 F2'"/>
		<xsl:variable name="list_InterestDayBasisTotal" select="'A A1 A2 A3 A4 B C C1 C2 D E E1 F F1 F2 G S W W1 U O'"/>
		<xsl:if test="contains(concat(' ',$list_InterestDayBasisTotal,' '),concat(' ',substring-before(b:interestDayBasis,' '),' '))">
			<accrualRule>
				<xsl:choose>
					...
					<xsl:when test="contains(concat(' ',$list_InterestDayBasis21,' '),concat(' ',substring-before(b:interestDayBasis,' '),' '))">
	...
	<xsl:template name="subNature">
		<xsl:if test="b:subAssetType = '102'">
			<subNature>
				<xsl:value-of select="'31'"/>
			</subNature>
		</xsl:if>
					...
					<benchmark>
						<infra:code>
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:value-of select="concat(substring(b:interestKey,'1','5'),b:rateChangeElapse)"/>
								</xsl:otherwise>
							</xsl:choose>
						</infra:code>
					</benchmark>
	...
	<xsl:template name="ie_beginDate">
		<beginDate>
			<xsl:choose>
				<xsl:when test="not(../../c:PreviousEvent/b:FinancialInstrumentSecurity/b:issueDate) and b:issueDate">
					<xsl:value-of select="b:issueDate"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="b:accrualStartDate"/>
