...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SyFxForwardsCob" xmlns:infra="http://www.odcgroup.com/InfraPMS"
	...
	<xsl:template name="type">
		<type>
			<infra:code>
				<xsl:choose>
					<xsl:when test="b:contractStatus ='MATURED'">
						<xsl:value-of select="'SYFX_CLOSE'"/>
					</xsl:when>
					<xsl:when test="b:contractStatus ='KNOCKEDOUT'">
						<xsl:value-of select="'SYFX_CONTRACT_KNOCKOUT'"/>
					</xsl:when>
					<xsl:when test="b:contractStatus ='UNWOUND'">
						<xsl:value-of select="'SYFX_UNWIND'"/>
					</xsl:when>
					<xsl:when test="b:bUndoFixing ='YES'">
						<xsl:value-of select="'SYFX_CALL_UNDOFIXING'"/>
					</xsl:when>
					<xsl:when test="b:bUndoKnockout ='YES'">
						<xsl:value-of select="'SYFX_CALL_UNDOKNOCKOUT '"/>
					</xsl:when>
					<xsl:when test="b:bKnockOut ='YES'">
						<xsl:value-of select="'SYFX_CALL_KNOCKOUT'"/>
					</xsl:when>
					<xsl:when test="b:sUndoFixing ='YES'">
						<xsl:value-of select="'SYFX_PUT_UNDOFIXING'"/>
					</xsl:when>
					<xsl:when test="b:sUndoKnockout ='YES'">
						<xsl:value-of select="'SYFX_PUT_UNDOKNOCKOUT'"/>
					</xsl:when>
					<xsl:when test="b:sKnockOut ='YES'">
						<xsl:value-of select="'SYFX_PUT_KNOCKOUT'"/>
					</xsl:when>
				</xsl:choose>
			</infra:code>
		</type>
