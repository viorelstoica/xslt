...
<xsl:stylesheet xmlns="http://www.temenos.com/T24/OFSML/150" xmlns:b="http://www.odcgroup.com/F2B_MM_FX" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:paymentorder="http://www.temenos.com/T24/paymentorder" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="b"
                version="1.0">
	...
	<xsl:template name="DEBIT.ACCOUNT">
		<field name="DEBIT.ACCOUNT">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of select="b:debitAccount/infra:code"/>
				</xsl:otherwise>
			</xsl:choose>
		</field>
	</xsl:template>
	<xsl:template name="TAP.DR.SLV.ACC">
		<field name="TAP.DR.SLV.ACC">
	...
	<xsl:template name="DEBIT.CCY">
		<field name="DEBIT.CCY">
			<xsl:value-of select="b:debitCcy/infra:code"/>
		</field>
	</xsl:template>
	...
	<xsl:template name="CREDIT.ACCOUNT">
		<field name="CREDIT.ACCOUNT">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of select="b:creditAccount/infra:code"/>
				</xsl:otherwise>
			</xsl:choose>
		</field>
	</xsl:template>
	<xsl:template name="TAP.CR.SLV.ACC">
		<field name="TAP.CR.SLV.ACC">
...
<metaInformation>
	...
	<MapperMetaTag>
		...
		<MapperBlockPosition>
			<template name="TAP.EVENT.ID"></template>
			<template name="CREDIT.ACCOUNT"></template>
		</MapperBlockPosition>
