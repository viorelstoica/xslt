...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PaymentOrder" xmlns:infra="http://www.odcgroup.com/InfraPMS"
	...
	<xsl:template name="portfolio">
		<portfolio>
			<infra:code>
				<xsl:choose>
					<xsl:when test="b:orderingPortfolio = '' or not(b:orderingPortfolio)">
						<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:value-of select="b:orderingPortfolio"/>
					</xsl:otherwise>
				</xsl:choose>
			</infra:code>
		</portfolio>
	...
	<xsl:template name="transactionCode">
		<transactionCode>
			<xsl:value-of select="concat('PO_',b:id,'_D',$COMPANY_POSTFIX_SEPARATOR,b:mnemonic,'_R')"/>
		</transactionCode>
	</xsl:template>
