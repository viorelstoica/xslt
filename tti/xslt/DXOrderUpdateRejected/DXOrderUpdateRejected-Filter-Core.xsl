...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch" version="1.0" xmlns="http://www.odcgroup.com/Request"
    			xmlns:a="http://www.odcgroup.com/Request" 
				xmlns:b="http://www.temenos.com/T24/event/TTI/DXOrderReturn"
                xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchDXOrderReturn" 
				xmlns:c="http://www.temenos.com/T24/event/TTI/MultiDXOrderReturn" 
				xmlns:infra="http://www.odcgroup.com/InfraPMS"
                xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiDXOrderReturn" 
	...
	<!-- Condition 1 -->
		...
		<xsl:apply-templates select="self::b:DXOrderReturn[b:memoAccountList/b:memoAccount = 'NO']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:DXOrderReturn" mode="filter1">
		<xsl:apply-templates select="self::b:DXOrderReturn[ not(b:dealerBook)]" mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -->
	<xsl:template match="b:DXOrderReturn" mode="filter2">
		<xsl:apply-templates select="self::b:DXOrderReturn[b:customerTypeCustomerSecurityJoinGroup/b:customerTypeCustomerSecurityJoinList/b:customerTypeCustomerSecurityJoin = 'CUSTOMER']" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
	<xsl:template match="b:DXOrderReturn" mode="filter3">
		<xsl:apply-templates select="self::b:DXOrderReturn[ ../../c:PreviousEvent/b:DXOrderReturn/b:orderStatus = 'MODIFICATION REQUEST' ]" mode="filter4"/>
	</xsl:template>
	<!-- Condition 5 -->
	<xsl:template match="b:DXOrderReturn" mode="filter4">
		<xsl:apply-templates select="self::b:DXOrderReturn[ (b:orderStatus = 'TRANSMITTED') or (b:orderStatus = 'PARTIAL')]" mode="filter5"/>
	</xsl:template>
	<!-- Condition 6 -->
	<xsl:template match="b:DXOrderReturn" mode="filter5">
		<xsl:apply-templates select="self::b:DXOrderReturn[ b:orderLots != ../../c:PreviousEvent/b:DXOrderReturn/b:orderLots or b:limitPrice != ../../c:PreviousEvent/b:DXOrderReturn/b:limitPrice or b:limitType != ../../c:PreviousEvent/b:DXOrderReturn/b:limitType or b:limitDate != ../../c:PreviousEvent/b:DXOrderReturn/b:limitDate ]" mode="filter-custo"/>
	</xsl:template>
	<!-- Condition 4 -->
	<!--<xsl:template match="b:DXOrderReturn" mode="filter3">
		<xsl:apply-templates select="self::b:DXOrderReturn[substring(b:tapRefId,1,1) = 'I']" mode="filter4"/>
	</xsl:template>-->
	<!-- Condition 5 -->
	<!-- Custo placeholder -->

	<xsl:template match="b:DXOrderReturn" mode="filter-custo">
		<xsl:apply-templates select="self::b:DXOrderReturn" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:DXOrderReturn" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="DXOrderUpdateRejected" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\DXOrderUpdateRejected\DXOrderUpdateRejected.xml" htmlbaseurl=""
		          outputurl="..\..\TestFiles\DXOrderUpdateRejected\DXOrderUpdateRejectedCusto.xml" processortype="jaxp" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath=""
