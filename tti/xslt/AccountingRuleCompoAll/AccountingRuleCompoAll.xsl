	...
	<xsl:template match="b:CginventoryAll">
		<InfraPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('AccountingRuleCompoAll',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<AccountingRuleCompo>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('AccountingRuleCompoAll.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					...
					<xsl:call-template name="accProfile_AccountingRuleCompoAll"/>
					<xsl:call-template name="beginD_AccountingRuleCompoAll"/>
					<xsl:call-template name="fusionRule_AccountingRuleCompoAll"/>			
					<xsl:call-template name="instrList_AccountingRuleCompoAll"/>
					<xsl:call-template name="taxLotMngt_AccountingRuleCompoAll"/>
					<xsl:call-template name="taxLotOpRule_AccountingRuleCompoAll"/>
				</AccountingRuleCompo>
			</DataGroup>
			<DataGroup>
				<AccountingRuleCompo>
					...
					<xsl:call-template name="accProfile_AccountingRuleCompoAllEL"/>
					<xsl:call-template name="beginD_AccountingRuleCompoAllEL"/>
					<xsl:call-template name="fusionRule_AccountingRuleCompoAllEL"/>
					<xsl:call-template name="instrList_AccountingRuleCompoAllEL"/>
					<xsl:call-template name="taxLotMngt_AccountingRuleCompoAllEL"/>
					<xsl:call-template name="taxLotOpRule_AccountingRuleCompoAllEL"/>
				</AccountingRuleCompo>
			</DataGroup>
		</InfraPMS>
	</xsl:template>
