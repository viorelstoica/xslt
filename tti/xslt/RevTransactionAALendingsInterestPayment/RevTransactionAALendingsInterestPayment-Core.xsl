...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS"
   ...
   <xsl:template name="reversedTransactionCode">
      <reversedTransactionCode>
		...
		<xsl:when test="$POSTFIX_AACOMNE = 'Y'">
	         <xsl:choose>
	            <xsl:when test="d:activity = 'LENDING-CAPITALISE-SCHEDULE'">
	               <xsl:value-of select="concat(../c:aatxncontext/b:aaArrActivityId,$COMPANY_POSTFIX_SEPARATOR,../c:aatxncontext/b:aaCompanyMnemonic,'-I','-1')"/>
	            </xsl:when>
	            <xsl:otherwise>
		...
		<xsl:otherwise>
			<xsl:choose>
	            <xsl:when test="d:activity = 'LENDING-CAPITALISE-SCHEDULE'">
	               <xsl:value-of select="concat(../c:aatxncontext/b:aaArrActivityId,'-I','-1')"/>
	            </xsl:when>
	            <xsl:otherwise>
      ...
      <transactionCode>
         <xsl:choose>
	  	<xsl:when test="$POSTFIX_AACOMNE = 'Y'">
	         <xsl:choose>
	            <xsl:when test="d:activity = 'LENDING-CAPITALISE-SCHEDULE'">
	               <xsl:value-of select="concat(../c:aatxncontext/b:aaArrActivityId,$COMPANY_POSTFIX_SEPARATOR,../c:aatxncontext/b:aaCompanyMnemonic,'-1','_R')"/>
	            </xsl:when>
	            <xsl:otherwise>
	               <xsl:value-of select="concat(../c:aatxncontext/b:aaArrActivityId,$COMPANY_POSTFIX_SEPARATOR,../c:aatxncontext/b:aaCompanyMnemonic,'_R')"/>
	            </xsl:otherwise>
	         </xsl:choose>
		</xsl:when>
		<xsl:otherwise>
			<xsl:choose>
	            <xsl:when test="d:activity = 'LENDING-CAPITALISE-SCHEDULE'">
	               <xsl:value-of select="concat(../c:aatxncontext/b:aaArrActivityId,'-1','_R')"/>
	            </xsl:when>
	            <xsl:otherwise>
	               <xsl:value-of select="concat(../c:aatxncontext/b:aaArrActivityId,'_R')"/>
	            </xsl:otherwise>
	         </xsl:choose>
		</xsl:otherwise>
	  </xsl:choose>
   ...
   <xsl:template name="transactionDate">
      ...
      <xsl:variable name="BILLID" select="../c:recaccountdetails/f:actBillDetails[f:activityRef = $MASTERAAA]/f:billId"/>
	<xsl:if test="../c:billdetails[j:aaBillId = $BILLID][j:arrangementId=$arrangement]/j:actualPayDate and ../c:billdetails[j:aaBillId = $BILLID][j:arrangementId=$arrangement]/j:actualPayDate != ''">
    <transactionDate>
      <xsl:call-template name="xsdate">
        <xsl:with-param name="date" select="../c:billdetails[j:aaBillId = $BILLID][j:arrangementId=$arrangement]/j:actualPayDate"/>
      </xsl:call-template>
    </transactionDate>
	</xsl:if>
   </xsl:template>
   ...
   <xsl:template name="valueDate">
    	...
    	<xsl:variable name="arrangement" select="../c:aaarecord/d:arrangement"/>
    	<xsl:variable name="BILLID" select="../c:recaccountdetails/f:actBillDetails[f:activityRef = $MASTERAAA]/f:billId"/>
	<xsl:if test="../c:billdetails[j:aaBillId = $BILLID][j:arrangementId=$arrangement]/j:financialDate and ../c:billdetails[j:aaBillId = $BILLID][j:arrangementId=$arrangement]/j:financialDate != ''">
    <valueDate>
      <xsl:call-template name="xsdate">
