   ...
   <xsl:template match="c:AAAFlow">
      <TransactionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('RevTransactionAALendingsInterestPayment',c:aatxncontext/b:aaArrangementId)"/>
         </xsl:attribute>
         <DataGroup>
            <xsl:call-template name="mappingRevTransactionAALendingsInterestPayment"/>
         </DataGroup>
      </TransactionPMS>
   </xsl:template>
   <xsl:template name="mappingRevTransactionAALendingsInterestPayment">
      <xsl:for-each select="c:aaarecord">
         <DepositLoan>
            <xsl:attribute name="description">
               <xsl:value-of select="concat('RevTransactionAALendingsInterestPayment.xsl+', name(/*), '+', ../c:eventCommon/ns0:eventId, '+', ../c:eventCommon/ns0:creationTime)"/>
            </xsl:attribute>
...
<metaInformation>
	<scenarios>
		<scenario default="yes" name="AAAFlow" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\AAAFlowFilter\AAAFlowFilter.xml" htmlbaseurl="" outputurl="..\..\TestFiles\AAAFlowFilter\AAAFlowFilterOut.xml" processortype="jaxp"
