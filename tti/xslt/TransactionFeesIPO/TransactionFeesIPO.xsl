	...
	<xsl:template match="b:SecOpenOrder">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionFeesIPO',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Cash>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionFeesIPO.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
