	...
	<xsl:template match="b:FdFiduciary">
		<FiPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FinancialInstrumentFiduciaryContract',b:id)"/>
			</xsl:attribute>
			<DataGroup>
					...
					<MoneyMarket>
						<SpecificMoneyMarket>
							<xsl:attribute name="description">
								<xsl:value-of select="concat('FinancialInstrumentFiduciaryContract.xsl+', name(/*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
							</xsl:attribute>
