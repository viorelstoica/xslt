...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SyFxForwards" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSyFxForwards" xmlns:c="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiSyFxForwards" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="b c batch multibatch" version="1.0">
  ...
  <xsl:template match="b:SyFxForwards">
    ...
    <xsl:apply-templates select="self::b:SyFxForwards[$opStatus = 'REVERSE-AUTHORISE']" mode="filter1"/>
  </xsl:template>
  <!-- Condition 2 -->
  <xsl:template match="b:SyFxForwards" mode="filter1">
    <xsl:apply-templates select="self::b:SyFxForwards[b:contractStatus = 'ACTIVE']" mode="filter2"/>
  </xsl:template>
  <!-- Condition 3 -->
  <xsl:template match="b:SyFxForwards" mode="filter2">
    <xsl:apply-templates select="self::b:SyFxForwards[(b:bKnockoutIntrinsic!='') and (b:sKnockoutIntrinsic!='')]" mode="filter3"/>
  </xsl:template>
  <!-- Condition 4 -->
  <xsl:template match="b:SyFxForwards" mode="filter3">
    ...
    <xsl:apply-templates select="self::b:SyFxForwards[number($check_MaturityDate) &gt;= number($check_toDay)]" mode="filter4"/>
  </xsl:template>
  <!-- Condition 5 -->
  <xsl:template match="b:SyFxForwards" mode="filter4">
    <xsl:apply-templates select="self::b:SyFxForwards[b:portfolio != '']" mode="filter5"/>
  </xsl:template>
	...
	<xsl:template match="b:SyFxForwards" mode="filter5">
		<xsl:variable name="checkPortfolioConditionVar">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'TransSYTarkoDerivativeBuyRev'"/>
