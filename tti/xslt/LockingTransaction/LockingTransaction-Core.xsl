...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/LockingTransaction"
	...
	<xsl:template name="accountingDate">
		<accountingDate>
			<xsl:value-of select="b:effFromDate"/>
		</accountingDate>
	</xsl:template>
	<xsl:template name="depositaryAccount">
		<xsl:choose>
   		<xsl:when test="b:secDepot != ''">
		<depositaryAccount>
		<infra:code>
			<xsl:choose>
				<xsl:when test="b:subAccountTypeList/b:subAccountType[@index=1] != '' and b:subAccountTypeList/b:subAccountType[@index=1] = $subAccountNature and b:subAccount != ''">
					<xsl:value-of select="concat(b:secDepot,'_',b:subAccount)"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="b:secDepot"/>
				</xsl:otherwise>
			</xsl:choose>
		</infra:code>
		</depositaryAccount>
	...
	<xsl:template name="lockingUnlocking">
		<lockingUnlocking>
			<xsl:choose>
				<xsl:when test="b:transactionType = 'BLOCK'">1</xsl:when>
				<xsl:when test="b:transactionType = 'UNBLOCK'">2</xsl:when>
			</xsl:choose>
		</lockingUnlocking>
	</xsl:template>
	<xsl:template name="portfolio">
		<portfolio>
			<infra:code>
				<xsl:value-of select="b:securitiesAccount"/>
			</infra:code>
		</portfolio>
	</xsl:template>
	<xsl:template name="quantity">
		<xsl:if test="b:newAmtBlocked != ''">
			<quantity>
				<xsl:value-of select="b:newAmtBlocked"/>
			</quantity>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="validityDate">
		<xsl:if test="b:transactionType = 'BLOCK'">
			<validityDate>
				<xsl:value-of select="b:effToDate"/>
			</validityDate>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="type">
		<type>
			<infra:code>
				<xsl:choose>
					<xsl:when test="b:transactionType = 'BLOCK'">SC_BLOCK</xsl:when>
					<xsl:when test="b:transactionType = 'UNBLOCK'">SC_UNBLOCK</xsl:when>
				</xsl:choose>
			</infra:code>
		</type>
	</xsl:template>
