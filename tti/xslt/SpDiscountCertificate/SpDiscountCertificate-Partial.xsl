   ...
   <xsl:template match="b:FinancialInstrumentSecurity">
      <StructuredProducts>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('SpDiscountCertificate-Partial',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <DiscountCertificate>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('SpDiscountCertificate-Partial.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
