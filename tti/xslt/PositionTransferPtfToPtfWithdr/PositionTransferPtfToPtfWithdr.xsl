...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PositionTransferPtfToPtfWithdr"
	...
	<xsl:template match="c:MultiPositionTransferPtfToPtfWithdr">
		<xsl:apply-templates select="c:CurrentEvent/b:PositionTransferPtfToPtfWithdr"/>
	</xsl:template>
	<xsl:template match="batch:BatchPositionTransferPtfToPtfWithdr">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchPositionTransferPtfToPtfWithdr" name="BatchPositionTransferPtfToPtfWithdr">
			<xsl:apply-templates select="b:PositionTransferPtfToPtfWithdr"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiPositionTransferPtfToPtfWithdr">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiPositionTransferPtfToPtfWithdr" name="BatchMultiPositionTransferPtfToPtfWithdr">
			<xsl:apply-templates select="c:MultiPositionTransferPtfToPtfWithdr"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:PositionTransferPtfToPtfWithdr">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('positionTransferPtfToPtfWithdr',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<xsl:for-each select="b:securityNoGroup">
					<Security>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('PositionTransferPtfToPtfWithdr.xsl+', name(/*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
