...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.temenos.com/T24/OFSML/150" xmlns:b="http://www.odcgroup.com/F2B_MM_FX" xmlns:infra="http://www.odcgroup.com/InfraPMS" exclude-result-prefixes="b" version="1.0">
   ...
   <xsl:template match="/">
      <T24Batch>
         <xsl:for-each select="b:F2B_MM_FX/b:DataGroup/b:ForexForward/b:ForexSpot/b:OrderLifeCycle/b:purchaseRequest">
            <T24>
               <serviceRequest>
                  <securityContext>
                     <xsl:choose>
                  ...
                  <ofsTransactionInput application="FOREX" operation="PROCESS" version="{$ForexFXSpotPurchaseVersion}">
   ...
   <xsl:template name="FOREX">
	  ...
	  <xsl:call-template name="TAP.FX.COMP.ORD"/>
	  ...
	  <xsl:call-template name="OUR.ACC.REC.SLV"/>
	  <xsl:call-template name="OUR.ACC.PAY.SLV"/>
   </xsl:template>
