	...
	<xsl:template match="b:TPHServiceFlow">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionPaymentOutgoingTPH',b:portransactionrec/d:fTNumber)"/>
			</xsl:attribute>
			<DataGroup>
				<Cash>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionPaymentOutgoingTPH.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
