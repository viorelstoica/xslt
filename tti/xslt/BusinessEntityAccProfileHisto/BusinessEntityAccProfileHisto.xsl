	...
	<xsl:template match="b:Cginventory">
			<InfraPMS>
				...
				<xsl:attribute name="output">
					<xsl:value-of select="concat('BusinessEntityAccProfileHisto',b:id)"/>
				</xsl:attribute>
				<DataGroup>
					<BeAccProfileHisto>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('BusinessEntityAccProfileHisto.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
