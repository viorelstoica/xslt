...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/F2B" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/PositionTransferAcknowledgment"
	...
	<xsl:template match="c:MultiPositionTransferAcknowledgment">
		<xsl:apply-templates select="c:CurrentEvent/b:PositionTransferAcknowledgment"/>
	</xsl:template>
	<xsl:template match="batch:BatchPositionTransferAcknowledgment">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchPositionTransferAcknowledgment" name="BatchPositionTransferAcknowledgment">
			<xsl:apply-templates select="b:PositionTransferAcknowledgment"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiPositionTransferAcknowledgment">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiPositionTransferAcknowledgment" name="BatchMultiPositionTransferAcknowledgment">
			<xsl:apply-templates select="c:MultiPositionTransferAcknowledgment"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:PositionTransferAcknowledgment">
		<F2B mode="INSUPD">
			<xsl:attribute name="output">
				<xsl:value-of select="concat('PositionTransferAcknowledgment',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Security>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('PositionTransferAcknowledgment.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
