...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch" version="1.0" xmlns="http://www.odcgroup.com/FiPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/OrderGroupingCutOffDateChange"
	...
	<xsl:template match="b:OrderGroupingCutOffDateChange">
		<xsl:apply-templates select="self::b:OrderGroupingCutOffDateChange[($g_filterBySubAssetType='Y' and b:subAssetType and contains($list_SubAssetType_OrderGroupingCutOffDateChange,b:subAssetType)) or
									($g_filterBySubAssetType != 'Y' and b:subAssetTypeTapInstrType and b:subAssetTypeTapInstrType='MUTUALFUND')]" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:OrderGroupingCutOffDateChange" mode="filter1">
		<xsl:apply-templates select="self::b:OrderGroupingCutOffDateChange[not(contains(b:id, '*')) or (contains(b:id, '*') and (substring-after(b:id,'*') = $list_transactionCodes_SubscriptionRedemptionRed or substring-after(b:id,'*') = $list_transactionCodes_SubscriptionRedemptionBuy))]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:OrderGroupingCutOffDateChange" mode="filter-custo">
		<xsl:apply-templates select="self::b:OrderGroupingCutOffDateChange" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:OrderGroupingCutOffDateChange" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="OrderGroupingCutOffDateChange" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\OrderGroupingCutOffDateChange\FinancialInstruemntMutualFund.xml" htmlbaseurl=""
		          outputurl="..\..\TestFiles\OrderGroupingCutOffDateChange\FinancialInstruemntMutualFundCusto.xml" processortype="jaxp" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath=""
		          additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator="">
			<advancedProp name="bSchemaAware" value="false"/>
			<advancedProp name="xsltVersion" value="2.0"/>
