...
<xsl:stylesheet xmlns="http://www.odcgroup.com/FiPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/DerivativesPrice" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchDerivativesPrice" xmlns:c="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiDerivativesPrice" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="b batch multibatch c" version="1.0">
  ...
  <xsl:template match="b:DerivativesPrice">
    ...
    <xsl:apply-templates select="self::b:DerivativesPrice[contains(concat(' ',$list_PriceSet,' '),concat(' ',b:priceSet,' '))]" mode="filter1"/>
  </xsl:template>
  <!-- Condition 2 -->
  <xsl:template match="b:DerivativesPrice" mode="filter1">
    <xsl:variable name="optionType" select="' KNOCKIN KNOCKOUT KIKI KIKO KOKO DIGITAL '"/>
    <xsl:apply-templates select="self::b:DerivativesPrice[b:optionTypeList/b:optionType ='' or contains(concat(' ',$optionType,' '),concat(' ',b:optionTypeList/b:optionType,' '))]" mode="filter2"/>
  </xsl:template>
  <!--Condition 3 -->
  <xsl:template match="b:DerivativesPrice" mode="filter2">
    <xsl:apply-templates select="self::b:DerivativesPrice[($g_filterBySubAssetType='Y' and b:DxContractMasterJoinSubAssetType and contains($list_SubAssetType_DerivativesPrice,b:DxContractMasterJoinSubAssetType)) or 
								($g_filterBySubAssetType != 'Y' and b:subAssetTypeTapInstrType and (b:subAssetTypeTapInstrType = 'FUTURE' or b:subAssetTypeTapInstrType = 'OPTION'))]" mode="filter3"/>
  </xsl:template>
  <!-- Condition 4 -->
  <xsl:template match="b:DerivativesPrice" mode="filter3">
    ...
    <xsl:variable name="check_toDay">
      <xsl:call-template name="formatDateForMaturityComparison">
        <xsl:with-param name="dateToFormat" select="b:eventCommon/c:today"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:apply-templates select="self::b:DerivativesPrice[number($check_MaturityDate) &gt;= number($check_toDay)]" mode="filter4"/>
  </xsl:template>
  <!-- Condition 5 -->
  <xsl:template match="b:DerivativesPrice" mode="filter4">
    <xsl:apply-templates select="self::b:DerivativesPrice[b:price and b:price !='']" mode="filter-custo"/>
  </xsl:template>
  <!-- Custo placeholder -->
  <xsl:template match="b:DerivativesPrice" mode="filter-custo">
    <xsl:apply-templates select="self::b:DerivativesPrice" mode="filter-final"/>
  </xsl:template>
  <!-- Copy block -->
  <xsl:template match="b:DerivativesPrice" mode="filter-final">
    <xsl:copy>
      <xsl:apply-templates select="@* | node()"/>
    </xsl:copy>
  </xsl:template>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="DerivativesPrice" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\DerivativesPrice\DerivativesPrice.xml" htmlbaseurl="" outputurl="..\..\TestFiles\DerivativesPrice\DerivativesPriceCusto.xml"
