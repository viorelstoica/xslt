...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/FdGroupPayment" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchFdGroupPayment" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiFdGroupPayment" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiFdGroupPayment" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/>
   <xsl:template match="c:MultiFdGroupPayment">
      <xsl:apply-templates select="c:CurrentEvent/b:FdGroupPayment"/>
   </xsl:template>
   <xsl:template match="batch:BatchFdGroupPayment">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchFdGroupPayment" name="BatchFdGroupPayment">
         <xsl:apply-templates select="b:FdGroupPayment"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiFdGroupPayment">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiFdGroupPayment" name="BatchMultiFdGroupPayment">
         <xsl:apply-templates select="c:MultiFdGroupPayment"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:FdGroupPayment">
      <FiPMS>
         <xsl:attribute name="mode">INSUPD</xsl:attribute>
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FinancialInstrumentFiduciaryContractNoticeTempl',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <MoneyMarket>
               <SpecificMoneyMarket>
                  <xsl:attribute name="description">
                     <xsl:value-of select="concat('FinancialInstrumentFiduciaryContractNoticeTempl.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
                  </xsl:attribute>
...
<metaInformation>
	<scenarios>
		<scenario default="yes" name="TransactionFiduciaryInitiate" userelativepaths="yes" externalpreview="no" url="..\..\..\..\testfiles\FdFidOrder\FdFidOrder.xml" htmlbaseurl="" outputurl="..\..\..\..\testfiles\FdFidOrder\FdFidOrderGWP.xml"
