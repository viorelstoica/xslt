...
<xsl:stylesheet xmlns="http://www.odcgroup.com/GenericCOA" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionEntitlement" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionEntitlement"
    ...
    <xsl:template match="b:TransactionEntitlement">
        <xsl:variable name="coaToProcess">WARRANTS.EXERCISE</xsl:variable>
        <xsl:choose>
	...
	<xsl:template match="b:TransactionEntitlement" mode="filter1">
		<xsl:variable name="checkPortfolioConditionVar">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'TransactionWarrantExercise'"/>
	...
	<xsl:template match="b:optionDescGroup">
		<xsl:apply-templates select="self::b:optionDescGroup[b:optionNom]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:optionDescGroup" mode="filter-custo">
		<xsl:apply-templates select="self::b:optionDescGroup" mode="filter-final"/>
	</xsl:template>
	<!-- Copy block -->
	<xsl:template match="b:optionDescGroup" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
