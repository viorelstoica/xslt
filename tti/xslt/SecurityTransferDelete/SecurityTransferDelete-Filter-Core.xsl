...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/F2B" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/SecurityTransfer"
	...
	<xsl:template match="b:SecurityTransfer">
		...
		<xsl:apply-templates select="self::b:SecurityTransfer[$opStatus = 'INPUT-DELETE' or $opStatus = 'HOLD-DELETE']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	...
	<xsl:template match="b:SecurityTransfer" mode="filter3">
		<xsl:apply-templates select="self::b:SecurityTransfer[b:tapRefId and b:tapRefId != '']" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
