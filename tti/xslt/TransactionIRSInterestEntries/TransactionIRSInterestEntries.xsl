...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/AcEntry/AcEntry" xmlns:batch="http://www.temenos.com/T24/event/AcEntry/BatchAcEntry"
	...
	<xsl:template match="multibatch:BatchMultiAcEntry">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiAcEntry" name="BatchMultiAcEntry">
			<xsl:apply-templates select="e:MultiAcEntry"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:AcEntry">
		...
		<TransactionPMS>
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionInterestEntries',b:statemententry[b:transactionCode/@Rank=$MinRank]/b:transactionCode)"/>
			</xsl:attribute>
			<DataGroup>
				<IrSwap>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionIRSInterestEntries.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					...
					<IRSwapInterestPayment>
						...
						<xsl:call-template name="account2CcyTradeExchangeRate"/>
						...
						<xsl:call-template name="interestCurrency"/>
