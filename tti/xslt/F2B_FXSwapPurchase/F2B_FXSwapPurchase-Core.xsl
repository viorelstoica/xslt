...
<xsl:stylesheet xmlns="http://www.temenos.com/T24/OFSML/150"
   ...
   <xsl:template name="AMOUNT.BOUGHT">
      <field name="AMOUNT.BOUGHT">
         <xsl:value-of select="b:swapAmount"/>
         <!-- account2 net amount -->
      </field>
   </xsl:template>
   ...
   <xsl:template name="CURRENCY.SOLD">
      <field name="CURRENCY.SOLD">
         <xsl:value-of select="b:paidCurrency/infra:code"/>
      </field>
   </xsl:template>
   <xsl:template name="AMOUNT.SOLD">
      <field name="AMOUNT.SOLD">
         <xsl:value-of select="format-number(b:paidCurrencyAmount * (-1),$decimalformat,'nan2zero')"/>
      </field>
   </xsl:template>
   <xsl:template name="OUR.ACCOUNT.PAY">
      <field name="OUR.ACCOUNT.PAY">
         <xsl:value-of select="b:paidCurrencyAccount/infra:code"/>
      </field>
   </xsl:template>
