   ...
   <xsl:template match="b:SyFxForwards">
      <FiPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FiTarkoVanillaOption',translate(translate(b:id,'/',''),':',''))"/>
         </xsl:attribute>
         <DataGroup>
            <Option>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('FiTarkoVanillaOption.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
		 ...
		 <xsl:for-each select="b:vbFixingDateGroup">
				<DataGroup>
					<Event>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('FiTarkoVanillaOption.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
			...
			<xsl:for-each select="b:vbFixingDateGroup">
			<xsl:if test="b:vbFixedStatus = 'FIXED'">
			<DataGroup>
				<ChronologicalData>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FiTarkoVanillaOption.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
