...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/SyFxForwards" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSyFxForwards" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiSyFxForwards" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiSyFxForwards" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:template name="infra:code">
      <infra:code>
         <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of select="concat('TARKO',$COMPANY_POSTFIX_SEPARATOR,b:id,$COMPANY_POSTFIX_SEPARATOR,'VANILLA',$COMPANY_POSTFIX_SEPARATOR,b:mnemonic)"/>
            </xsl:otherwise>
         </xsl:choose>       
      </infra:code>
   </xsl:template>
   ...
   <xsl:template name="infra:shortName">
      <infra:shortName>
         <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of select="concat(b:dxContractCode,'/',b:contractCcy,'/',b:maturityDate,'/',b:vbOptionType,'/',b:vbStrikePrice,'/',b:vbPutCcy,'/',b:vbOptionStyle)"/>
            </xsl:otherwise>
         </xsl:choose>       
      </infra:shortName>
   </xsl:template>
   ...
   <xsl:template name="paymentFrequency">
        <paymentFrequency>
            <xsl:call-template name="sySetTapPaymentFrequency">
                <xsl:with-param name="t24FixingFrequency"><xsl:value-of select="b:vbFixingFrequency"/></xsl:with-param>
            </xsl:call-template>  
        </paymentFrequency>             
   </xsl:template>
   ...
   <xsl:template name="referenceCurrency">
         <referenceCurrency>
            <infra:code>
               <xsl:value-of select="b:vbPutCcy"/>
            </infra:code>
         </referenceCurrency>
   </xsl:template>
   ...
   <xsl:template name="type">
      <xsl:if test="b:vbOptionType != ''">
         <type>
            <infra:code>
               <xsl:value-of select="b:vbOptionType"/>
            </infra:code>
         </type>
      </xsl:if>
   </xsl:template>
   ...
   <xsl:template name="eventType">
	<eventType>
		<xsl:if test="b:vbTradeStatus and b:vbTradeStatus = 'CLOSED'">
   ...
   <xsl:template name="exerciseType">
      <exerciseType>
         <xsl:choose>
            <xsl:when test="b:vbOptionStyle = '' or not(b:vbOptionStyle)">
               <xsl:value-of select=" '0' "/>
            </xsl:when>
            <xsl:when test="b:vbOptionStyle = 'AMERICAN' ">
               <xsl:value-of select=" '1' "/>
            </xsl:when>
            <xsl:when test="b:vbOptionStyle = 'EUROPEAN' ">
               <xsl:value-of select=" '2' "/>
            </xsl:when>
            <xsl:when test="b:vbOptionStyle = 'BERMUDAN' ">
               <xsl:value-of select=" '3' "/>
            </xsl:when>
         </xsl:choose>
      </exerciseType>
   ...
   <xsl:template name="strikePrice">
        <strikePrice>
           <xsl:value-of select="b:vbStrikePrice"/>
        </strikePrice>
   </xsl:template>
   <xsl:template name="underlying">
        <underlying>
            <infra:code>
                <xsl:value-of select="b:vbCallCcy"/>
            </infra:code>
        </underlying>
   </xsl:template>
   ...
   <xsl:template name="fixingDate">
        <fixingDate>
             <xsl:call-template name="xsdate">
                <xsl:with-param name="date" select="translate(substring(b:vbFixingFrequency,1,8),'-','')"/>
             </xsl:call-template>        
        </fixingDate>
   </xsl:template>        
   ...
   <xsl:template name="payOffNature_termEvent">
          <payOffNature>2</payOffNature>
   </xsl:template>
   <xsl:template name="totalQuantity_termEvent">
		  <xsl:if test="b:vbTotalCallAmt">
			<totalQuantity>
				<xsl:value-of select="b:vbTotalCallAmt"/>
			</totalQuantity>
		  </xsl:if>
   </xsl:template>
   ...
   <xsl:template name="ExchangeEvent_endDate">
		<endDate>
			<xsl:value-of select="b:vbSettlementDate"/>
		</endDate>
	</xsl:template>
	...
	<xsl:template name="ExchangeEvent_exchangeRate">
		<exchangeRate>
			<xsl:value-of select="b:vbSpotExchangeRate"/>
		</exchangeRate>
	</xsl:template>
	<xsl:template name="ExchangeEvent_exDate">
		<exDate>
			<xsl:value-of select="b:vbFixingDate"/>
		</exDate>
	</xsl:template>
	...
	<xsl:template name="ExchangeEvent_newRatio">
		<newRatio>
			<xsl:value-of select="b:vbSchedulePutAmt"/>
		</newRatio>
	</xsl:template>
	<xsl:template name="ExchangeEvent_oldRatio">
		<oldRatio>
			<xsl:value-of select="b:vbScheduleCallAmt"/>
		</oldRatio>
		</xsl:template>
   ...
   <xsl:template name="ChronologicalData_dataNature">
	<dataNature>
		<xsl:choose>
			<xsl:when test= "b:vbFixing and b:vbFixing = 'EXPIRE'">
				<xsl:value-of select="'59'"/>
			</xsl:when>
			<xsl:when test= "b:vbFixing and b:vbFixing = 'EXERCISE'">
