...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch" version="1.0" xmlns="http://www.odcgroup.com/FiPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/ChronologicalDataFixedIncome"
                ...
                xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiChronologicalDataFixedIncome" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="b:ChronologicalDataFixedIncome">
		<xsl:param name="subAssetType" select="../../c:CurrentEvent/b:ChronologicalDataFixedIncome/b:subAssetType"/>
        <xsl:param name="subAssetTypeTapInstrType" select="../../c:CurrentEvent/b:ChronologicalDataFixedIncome/b:subAssetTypeTapInstrType"/>		
		<xsl:apply-templates select="self::b:ChronologicalDataFixedIncome[
		                                                      ($g_filterBySubAssetType='Y' and $subAssetType and contains(concat(' ',$list_SubAssetType_FinancialInstrumentStock,' '),concat(' ',$subAssetType,' ')))
		                                                      or
		                                                      ($g_filterBySubAssetType != 'Y' and $subAssetTypeTapInstrType and $subAssetTypeTapInstrType = 'STOCK')
		                                                      ]" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:ChronologicalDataFixedIncome" mode="filter1">
		<xsl:apply-templates select="self::b:ChronologicalDataFixedIncome[(b:totalIssued and b:totalIssued!= '' and not(../../c:PreviousEvent/b:ChronologicalDataFixedIncome/b:totalIssued) ) or (b:totalIssued and b:totalIssued!= '' and(../../c:PreviousEvent/b:ChronologicalDataFixedIncome/b:totalIssued != b:totalIssued) )]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:ChronologicalDataFixedIncome" mode="filter-custo">
		<xsl:apply-templates select="self::b:ChronologicalDataFixedIncome" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:ChronologicalDataFixedIncome" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
