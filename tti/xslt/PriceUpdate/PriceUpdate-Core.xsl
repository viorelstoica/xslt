...
<xsl:stylesheet xmlns="http://www.odcgroup.com/InfraPMS"
   ...
   <xsl:template name="code">
      <code>
         <xsl:if test="b:id != ''">
            <xsl:value-of select="concat($PRICEPROV_PREFIX,b:id)"/>
         </xsl:if>
      </code>
   </xsl:template>
   ...
   <xsl:template name="longName">
      <longName>
         <xsl:value-of select="concat(b:descriptionList/b:description[@language=$boDefaultLanguage],'(Price Provider)')"/>
      </longName>
   </xsl:template>
   <xsl:template name="multilingualLongName">
      ...
      <xsl:call-template name="OldNewDenomManagment">
         ...
         <xsl:with-param name="PreviousEvent"
                         select="../../c:PreviousEvent/b:PriceUpdate/b:descriptionList/b:description"/>
      </xsl:call-template>
   </xsl:template>
