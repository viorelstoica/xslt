<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/CorporateActionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionEntitlement" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b ns0" version="1.0">
   <xsl:template name="account1">
      <account1>
         <infra:code>
            <xsl:value-of select="b:accountNo"/>
         </infra:code>
      </account1>
   </xsl:template>
   <xsl:template name="account1CcyDividendCcyExchangeRate">
      <xsl:if test="b:netAmount != 0">
         <account1CcyDividendCcyExchangeRate>
            <xsl:value-of select="format-number(b:netAmtAccCur div b:netAmount,'#.#########','nan2zero')"/>
         </account1CcyDividendCcyExchangeRate>
      </xsl:if>
   </xsl:template>
   <xsl:template name="account1NetAmount">
      <account1NetAmount>
         <xsl:value-of select="b:netAmtAccCur"/>
      </account1NetAmount>
   </xsl:template>
   <xsl:template name="accountingDate">
      <accountingDate>
         <xsl:value-of select="b:PayDateJoin"/>
      </accountingDate>
   </xsl:template>
   <xsl:template name="cashDividend">
      <cashDividend>
         <xsl:choose>
            <xsl:when test="b:taxCredit">
               <xsl:value-of select="format-number( (b:optionDescGroup[@index=1]/b:entitlementAmt + b:taxCredit) div b:eventNominal,'#.########','nan2zero')"/>
            </xsl:when>
            <xsl:otherwise>
               <xsl:value-of select="format-number( (b:optionDescGroup[@index=1]/b:entitlementAmt) div b:eventNominal,'#.########','nan2zero')"/>
            </xsl:otherwise>
         </xsl:choose>
      </cashDividend>
   </xsl:template>
   <xsl:template name="coaReference">
      <coaReference>
         <xsl:value-of select="b:diaryId"/>
      </coaReference>
   </xsl:template>
   <xsl:template name="depositaryAccount">
   <xsl:choose>
   		<xsl:when test="b:depository != ''">
		<depositaryAccount>
		<infra:code>
			<xsl:choose>
				<xsl:when test="b:subAccountTypeList/b:subAccountType[@index=1] != '' and b:subAccountTypeList/b:subAccountType[@index=1] = $subAccountNature and b:subAccount != ''">
					<xsl:value-of select="concat(b:depository,'_',b:subAccount)"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="b:depository"/>
				</xsl:otherwise>
			</xsl:choose>
		</infra:code>
		</depositaryAccount>
		</xsl:when>
   </xsl:choose>
      
   </xsl:template>
   <xsl:template name="dividendCcyGrossAmount">
      <dividendCcyGrossAmount>
         <xsl:choose>
            <xsl:when test="b:taxCredit">
               <xsl:value-of select="format-number( b:optionDescGroup[@index=1]/b:entitlementAmt + b:taxCredit ,'#.####','nan2zero')"/>
            </xsl:when>
            <xsl:otherwise>
               <xsl:value-of select="format-number( b:optionDescGroup[@index=1]/b:entitlementAmt ,'#.####','nan2zero')"/>
            </xsl:otherwise>
         </xsl:choose>
      </dividendCcyGrossAmount>
   </xsl:template>
   <xsl:template name="dividendCcyNetAmount">
      <dividendCcyNetAmount>
         <xsl:value-of select="b:netAmount"/>
      </dividendCcyNetAmount>
   </xsl:template>
   <xsl:template name="dividendCurrency">
      <dividendCurrency>
         <infra:code>
            <xsl:value-of select="b:currency"/>
         </infra:code>
      </dividendCurrency>
   </xsl:template>
   <xsl:template name="nominalOrUnit">
      <nominalOrUnit>
         <xsl:value-of select="b:eventNominal"/>
      </nominalOrUnit>
   </xsl:template>
   <xsl:template name="reversalIndicator">
      <xsl:variable name="opStatus">
         <xsl:call-template name="operationStatus">
            <xsl:with-param name="transactionStage" select="b:eventCommon/ns0:transactionStage"/>
            <xsl:with-param name="recordStatus" select="b:recordStatus"/>
         </xsl:call-template>
      </xsl:variable>
      <xsl:if test="$opStatus = 'REVERSE-AUTHORISE'">
         <reversalIndicator>
            <xsl:value-of select="'1'"/>
         </reversalIndicator>
         <reversedTransactionCode>
		 	<xsl:choose>
				<xsl:when test="b:optionDescGroup/b:tapRefId and (b:optionDescGroup/b:tapRefId != '' or b:optionDescGroup/b:tapRefId != 'NULL')">
					<xsl:value-of select="b:optionDescGroup/b:tapRefId"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="(b:tapOperCode and b:tapOperCode != '')">
							<xsl:value-of select="concat(b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,b:entitlementId,$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin)"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="concat(b:entitlementId,$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin)"/>
						</xsl:otherwise>
					</xsl:choose>
         		</xsl:otherwise>
			</xsl:choose>
		 </reversedTransactionCode>
      </xsl:if>
   </xsl:template>
   <xsl:template name="portfolio">
      <portfolio>
         <infra:code>
            <xsl:value-of select="b:portfolioNo"/>
         </infra:code>
      </portfolio>
   </xsl:template>
   <xsl:template name="security">
      <security>
         <infra:code>
            <xsl:value-of select="b:securityNo"/>
         </infra:code>
      </security>
   </xsl:template>
   <xsl:template name="sourceSystemCode">
      <sourceSystemCode>
         <xsl:value-of select="b:entitlementId"/>
      </sourceSystemCode>
   </xsl:template>
   <xsl:template name="status">
      <status>
         <xsl:call-template name="statusTranslation">
            <xsl:with-param name="boStatus" select="'ACCOUNTED'"/>
         </xsl:call-template>
      </status>
   </xsl:template>
   <xsl:template name="transactionCode">
      <xsl:variable name="opStatus">
         <xsl:call-template name="operationStatus">
            <xsl:with-param name="transactionStage" select="b:eventCommon/ns0:transactionStage"/>
            <xsl:with-param name="recordStatus" select="b:recordStatus"/>
         </xsl:call-template>
      </xsl:variable>
      <transactionCode>
         <xsl:choose>
            <xsl:when test="$opStatus = 'REVERSE-AUTHORISE'">
				<xsl:choose>
					<xsl:when test="b:optionDescGroup/b:tapRefId and (b:optionDescGroup/b:tapRefId != '' or b:optionDescGroup/b:tapRefId != 'NULL')">
               			<xsl:value-of select="concat(b:optionDescGroup/b:tapRefId,'_R')"/>
            		</xsl:when>
					<xsl:otherwise>
						<xsl:choose>
							<xsl:when test="b:tapOperCode and b:tapOperCode != ''">
								<xsl:value-of select="concat(b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,b:entitlementId,'_R',$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="concat(b:entitlementId,'_R',$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin)"/>
							</xsl:otherwise>
						</xsl:choose>						
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
            <xsl:otherwise>
				<xsl:choose>
					<xsl:when test="b:optionDescGroup/b:tapRefId and (b:optionDescGroup/b:tapRefId != '' or b:optionDescGroup/b:tapRefId != 'NULL')">
               			<xsl:value-of select="b:optionDescGroup/b:tapRefId"/>
            		</xsl:when>
					<xsl:otherwise>
						<xsl:choose>
							<xsl:when test="b:tapOperCode and b:tapOperCode != ''">
								<xsl:value-of select="concat(b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,b:entitlementId,$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="concat(b:entitlementId,$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin)"/>
							</xsl:otherwise>
						</xsl:choose>
            		</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
         </xsl:choose>
      </transactionCode>
   </xsl:template>
   <xsl:template name="transactionDate">
      <transactionDate>
         <xsl:value-of select="b:PayDateJoin"/>
      </transactionDate>
   </xsl:template>
   <xsl:template name="transactionFeesTaxesCounter">
      <xsl:call-template name="printEntitlementFeesAndTaxes">
         <xsl:with-param name="sourceTax" select="b:sourceTaxAmt"/>
         <xsl:with-param name="fgnCharges" select="b:fgnChargesAmt"/>
         <xsl:with-param name="foreignChgesTax" select="b:foreignChgesTax"/>
         <xsl:with-param name="sumlocalTaxAmt" select="format-number(sum(b:localTaxAmtList/b:localTaxAmt),$decimalformat,'nan2zero')"/>
         <xsl:with-param name="commission" select="b:commissionAmt"/>
         <xsl:with-param name="commTax" select="b:commTaxAmt"/>
         <xsl:with-param name="sumManTaxAcy" select="format-number(sum(b:scTaxCodeGroup/b:manTaxAcy),$decimalformat,'nan2zero')"/>
         <xsl:with-param name="sumScAmtAcy" select="format-number(sum(b:scTaxCodeGroup/b:scAmtAcy),$decimalformat,'nan2zero')"/>
         <xsl:with-param name="taxCredit" select="b:taxCredit"/>
         <xsl:with-param name="currencyFT" select="b:currency"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template name="type">
      <type>
         <infra:code>
            <xsl:value-of select="concat('ENT_',b:eventType)"/>
         </infra:code>
      </type>
   </xsl:template>
   <xsl:template name="valueDate">
      <valueDate>
         <xsl:value-of select="b:valueDate"/>
      </valueDate>
   </xsl:template>
   <xsl:template name="positionCriteria1">
	<positionCriteria1>
		<xsl:call-template name="extCustodianTranslation">
			<xsl:with-param name="boExtCustodian">
				<xsl:value-of select="b:extCustodian"/>
			</xsl:with-param>
		</xsl:call-template>
	</positionCriteria1>
   </xsl:template>
   <xsl:template name="account2"/>
   <xsl:template name="account2CcyDividendCcyExchangeRate"/>
   <xsl:template name="account2NetAmount"/>
   <xsl:template name="account3"/>
   <xsl:template name="account3CcyDividendCcyExchangeRate"/>
   <xsl:template name="account3NetAmount"/>
   <xsl:template name="accountingCode"/>
   <xsl:template name="cashPortfolio"/>
   <xsl:template name="contractNumber"/>
   <xsl:template name="dividendCcyPortfolioCcyExchangeRate"/>
   <xsl:template name="initiator"/>
   <xsl:template name="notepad"/>
   <xsl:template name="remark"/>
   <xsl:template name="reversedTransactionCode"/>
   <xsl:template name="portfolioManager"/>
   <xsl:template name="securityCcyNetAmount"/>
   <xsl:template name="securityCcyPortfolioCcyExchangeRate"/>
   <xsl:template name="subType"/>
   <xsl:template name="typeInstrument"/>
   <xsl:template name="userDefinedField"/>
   <xsl:template name="mbFields"/>
   <xsl:template name="commonReference"/>
   <xsl:template name="exDate">
	<exDate>
		<xsl:value-of select="b:diaryExDate"/>
	</exDate>
	</xsl:template>
   <xsl:template name="splitPortfolio">
		<splitPortfolio>
			<xsl:value-of select="'1'"/>
		</splitPortfolio>
   </xsl:template>

   <xsl:template name="account1_PEFUNDDIST">
      <account1>
         <infra:code>
            <xsl:value-of select="b:accountNo"/>
         </infra:code>
      </account1>
   </xsl:template>
   <xsl:template name="account1CcyDividendCcyExchangeRate_PEFUNDDIST">
      <xsl:if test="b:netAmount != 0">
         <account1CcyDividendCcyExchangeRate>
            <xsl:value-of select="format-number(b:netAmtAccCur div b:netAmount,'#.#########','nan2zero')"/>
         </account1CcyDividendCcyExchangeRate>
      </xsl:if>
   </xsl:template>
   <xsl:template name="account1NetAmount_PEFUNDDIST">
      <account1NetAmount>
         <xsl:value-of select="b:netAmtAccCur"/>
      </account1NetAmount>
   </xsl:template>
   <xsl:template name="accountingDate_PEFUNDDIST">
      <accountingDate>
         <xsl:value-of select="b:PayDateJoin"/>
      </accountingDate>
   </xsl:template>
   <xsl:template name="cashDividend_PEFUNDDIST">
      <cashDividend>
               <xsl:value-of select="b:bookCostPercGroup/b:rate"/>
      </cashDividend>
   </xsl:template>
   <xsl:template name="coaReference_PEFUNDDIST">
      <coaReference>
         <xsl:value-of select="b:diaryId"/>
      </coaReference>
   </xsl:template>
   <xsl:template name="depositaryAccount_PEFUNDDIST">
      <xsl:choose>
   		<xsl:when test="b:depository != ''">
		<depositaryAccount>
		<infra:code>
			<xsl:choose>
				<xsl:when test="b:subAccountTypeList/b:subAccountType[@index=1] != '' and b:subAccountTypeList/b:subAccountType[@index=1] = $subAccountNature and b:subAccount != ''">
					<xsl:value-of select="concat(b:depository,'_',b:subAccount)"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="b:depository"/>
				</xsl:otherwise>
			</xsl:choose>
		</infra:code>
		</depositaryAccount>
		</xsl:when>
   </xsl:choose>
   </xsl:template>
   <xsl:template name="dividendCcyGrossAmount_PEFUNDDIST"/>
   <xsl:template name="dividendCcyNetAmount_PEFUNDDIST"/>
   <xsl:template name="dividendCurrency_PEFUNDDIST"/>
   <xsl:template name="nominalOrUnit_PEFUNDDIST">
      <nominalOrUnit>
         <xsl:value-of select="b:optionDescGroup/b:optionNom"/>
      </nominalOrUnit>
   </xsl:template>
   <xsl:template name="reversalIndicator_PEFUNDDIST">
      <xsl:variable name="opStatus">
         <xsl:call-template name="operationStatus">
            <xsl:with-param name="transactionStage" select="b:eventCommon/ns0:transactionStage"/>
            <xsl:with-param name="recordStatus" select="b:recordStatus"/>
         </xsl:call-template>
      </xsl:variable>
      <xsl:if test="$opStatus = 'REVERSE-AUTHORISE'">
         <reversalIndicator>
            <xsl:value-of select="'1'"/>
         </reversalIndicator>
         <reversedTransactionCode>
					<xsl:choose>
						<xsl:when test="(b:tapOperCode and b:tapOperCode != '')">
							<xsl:value-of select="concat(b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,b:entitlementId,$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin,'_PE')"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="concat(b:entitlementId,$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin,'_PE')"/>
						</xsl:otherwise>
					</xsl:choose>
		 </reversedTransactionCode>
      </xsl:if>
   </xsl:template>
   <xsl:template name="portfolio_PEFUNDDIST">
      <portfolio>
         <infra:code>
            <xsl:value-of select="b:portfolioNo"/>
         </infra:code>
      </portfolio>
   </xsl:template>
   <xsl:template name="security_PEFUNDDIST">
      <security>
         <infra:code>
            <xsl:value-of select="b:securityNo"/>
         </infra:code>
      </security>
   </xsl:template>
   <xsl:template name="sourceSystemCode_PEFUNDDIST">
      <sourceSystemCode>
         <xsl:value-of select="b:entitlementId"/>
      </sourceSystemCode>
   </xsl:template>
   <xsl:template name="status_PEFUNDDIST">
      <status>
         <xsl:call-template name="statusTranslation">
            <xsl:with-param name="boStatus" select="'ACCOUNTED'"/>
         </xsl:call-template>
      </status>
   </xsl:template>
   <xsl:template name="transactionCode_PEFUNDDIST">
      <xsl:variable name="opStatus">
         <xsl:call-template name="operationStatus">
            <xsl:with-param name="transactionStage" select="b:eventCommon/ns0:transactionStage"/>
            <xsl:with-param name="recordStatus" select="b:recordStatus"/>
         </xsl:call-template>
      </xsl:variable>
      <transactionCode>
			<!-- _0 to be first in fusion, followed by index -->
			<xsl:choose>
				<xsl:when test="b:tapOperCode and b:tapOperCode != ''">
					<xsl:choose>
						<xsl:when test="$opStatus = 'REVERSE-AUTHORISE'">
							<xsl:value-of select="concat(b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,b:entitlementId,'_R',$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin,'_PE')"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="concat(b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,b:entitlementId,$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin,'_PE')"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="$opStatus = 'REVERSE-AUTHORISE'">
							<xsl:value-of select="concat(b:entitlementId,'_R',$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin,'_PE')"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="concat(b:entitlementId,$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin,'_PE')"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:otherwise>
			</xsl:choose>
		</transactionCode>
   </xsl:template>
   <xsl:template name="transactionDate_PEFUNDDIST">
      <transactionDate>
         <xsl:value-of select="b:PayDateJoin"/>
      </transactionDate>
   </xsl:template>
   <xsl:template name="transactionFeesTaxesCounter_PEFUNDDIST">
      <xsl:call-template name="printEntitlementFeesAndTaxes">
         <xsl:with-param name="sourceTax" select="b:sourceTaxAmt"/>
         <xsl:with-param name="fgnCharges" select="b:fgnChargesAmt"/>
         <xsl:with-param name="foreignChgesTax" select="b:foreignChgesTax"/>
         <xsl:with-param name="sumlocalTaxAmt" select="format-number(sum(b:localTaxAmtList/b:localTaxAmt),$decimalformat,'nan2zero')"/>
         <xsl:with-param name="commission" select="b:commissionAmt"/>
         <xsl:with-param name="commTax" select="b:commTaxAmt"/>
         <xsl:with-param name="sumManTaxAcy" select="format-number(sum(b:scTaxCodeGroup/b:manTaxAcy),$decimalformat,'nan2zero')"/>
         <xsl:with-param name="sumScAmtAcy" select="format-number(sum(b:scTaxCodeGroup/b:scAmtAcy),$decimalformat,'nan2zero')"/>
         <xsl:with-param name="taxCredit" select="b:taxCredit"/>
         <xsl:with-param name="currencyFT" select="b:currency"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template name="type_PEFUNDDIST">
      <type>
         <infra:code>
            <xsl:value-of select="concat('ENT_',b:eventType)"/>
         </infra:code>
      </type>
   </xsl:template>
   <xsl:template name="valueDate_PEFUNDDIST">
      <valueDate>
         <xsl:value-of select="b:valueDate"/>
      </valueDate>
   </xsl:template>
   <xsl:template name="positionCriteria1_PEFUNDDIST">
	<positionCriteria1>
		<xsl:call-template name="extCustodianTranslation">
			<xsl:with-param name="boExtCustodian">
				<xsl:value-of select="b:extCustodian"/>
			</xsl:with-param>
		</xsl:call-template>
	</positionCriteria1>
   </xsl:template>
   <xsl:template name="account2_PEFUNDDIST"/>
   <xsl:template name="account2CcyDividendCcyExchangeRate_PEFUNDDIST"/>
   <xsl:template name="account2NetAmount_PEFUNDDIST"/>
   <xsl:template name="account3_PEFUNDDIST"/>
   <xsl:template name="account3CcyDividendCcyExchangeRate_PEFUNDDIST"/>
   <xsl:template name="account3NetAmount_PEFUNDDIST"/>
   <xsl:template name="accountingCode_PEFUNDDIST"/>
   <xsl:template name="cashPortfolio_PEFUNDDIST"/>
   <xsl:template name="contractNumber_PEFUNDDIST"/>
   <xsl:template name="dividendCcyPortfolioCcyExchangeRate_PEFUNDDIST"/>
   <xsl:template name="initiator_PEFUNDDIST"/>
   <xsl:template name="notepad_PEFUNDDIST"/>
   <xsl:template name="remark_PEFUNDDIST"/>
   <xsl:template name="reversedTransactionCode_PEFUNDDIST"/>
   <xsl:template name="portfolioManager_PEFUNDDIST"/>
   <xsl:template name="securityCcyNetAmount_PEFUNDDIST"/>
   <xsl:template name="securityCcyPortfolioCcyExchangeRate_PEFUNDDIST"/>
   <xsl:template name="subType_PEFUNDDIST"/>
   <xsl:template name="typeInstrument_PEFUNDDIST"/>
   <xsl:template name="userDefinedField_PEFUNDDIST"/>
   <xsl:template name="mbFields_PEFUNDDIST"/>
   <xsl:template name="commonReference_PEFUNDDIST"/>
   <xsl:template name="exDate_PEFUNDDIST">
	<exDate>
		<xsl:value-of select="b:diaryExDate"/>
	</exDate>
	</xsl:template>
   <xsl:template name="splitPortfolio_PEFUNDDIST">
		<splitPortfolio>
			<xsl:value-of select="'1'"/>
		</splitPortfolio>
   </xsl:template>
</xsl:stylesheet><!-- Stylus Studio meta-information - (c) 2004-2009. Progress Software Corporation. All rights reserved.

<metaInformation>
	<scenarios/>
	<MapperMetaTag>
		<MapperInfo srcSchemaPathIsRelative="yes" srcSchemaInterpretAsXML="no" destSchemaPath="" destSchemaRoot="" destSchemaPathIsRelative="yes" destSchemaInterpretAsXML="no"/>
		<MapperBlockPosition></MapperBlockPosition>
		<TemplateContext></TemplateContext>
		<MapperFilter side="source"></MapperFilter>
	</MapperMetaTag>
</metaInformation>
-->