   ...
   <xsl:template match="b:TransactionsForex">
      <TransactionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('forexSpotSale',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <ForexForward>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('ForexSpotSale.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
