...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch ns0 d e f g" version="1.0" xmlns="http://www.odcgroup.com/StructuredProducts" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template match="b:FinancialInstrumentSecurity">
		<StructuredProducts>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FinancialInstrumentMiniFutureTurbo',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<FutureTurbo>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FinancialInstrumentMiniFutureTurbo.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
				...
				<!-- BEG - termEvent -->
				<xsl:if test="b:underlyingGroup/b:underlying!='' or b:underlyingGroup/b:otherUnderlying!=''">
				<Event>
					<termEvent>
						<xsl:call-template name="averagingNumber_termEvent"/>
						...
						<xsl:call-template name="effectiveLeverage_termEvent"/>
						<xsl:call-template name="endDate_termEvent"/>
						<xsl:call-template name="endValidityDate_termEvent"/>
						<xsl:call-template name="exercisePrice_termEvent"/>
						<xsl:call-template name="exerciseType_termEvent"/>
						<xsl:call-template name="fixDate_termEvent"/>
						<xsl:call-template name="frequency_termEvent"/>
						<xsl:call-template name="frequencyUnit_termEvent"/>
						<xsl:call-template name="gap_termEvent"/>
						<xsl:call-template name="gearing_termEvent"/>
						<xsl:call-template name="initialFixingPrice_termEvent"/>
						<xsl:call-template name="lowBarrierP_termEvent"/>
						<xsl:call-template name="maximumValue_termEvent"/>
						<xsl:call-template name="minimumValue_termEvent"/>
						<xsl:call-template name="payOffNature_termEvent"/>
						<xsl:call-template name="premiumPaymentRule_termEvent"/>
						<xsl:call-template name="protectedDate_termEvent"/>
						<xsl:call-template name="rebateAtHit_termEvent"/>
						<xsl:call-template name="rebatePayoff_termEvent"/>
						<xsl:call-template name="scaleFactor_termEvent"/>
						<xsl:call-template name="season_termEvent"/>
						...
						<xsl:call-template name="upperRebate_termEvent"/>
						<xsl:call-template name="upperRebateAtHit_termEvent"/>
						...
						<xsl:call-template name="instrument_termEvent"/>
					</termEvent>
				</Event>
				</xsl:if>
				<!-- END - termEvent -->
				...
				<xsl:for-each select="b:underlyingGroup/b:weight">
					<Composition>
						...
						<xsl:call-template name="basketBarrier_Composition"/>
						<xsl:call-template name="basketBarrierPercent_Composition"/>
						<xsl:call-template name="basketExcercicePrice_Composition"/>
						<xsl:call-template name="basketExcercicePricePercent_Composition"/>
						<xsl:call-template name="basketInitialFixingPrice_Composition"/>
						<xsl:call-template name="basketWeight_Composition"/>
						<xsl:call-template name="cheapest_Composition"/>
						<xsl:call-template name="compositeInstrument_Composition"/>
						<xsl:call-template name="conversionFactor_Composition"/>
						<xsl:call-template name="conversionRatio_Composition"/>
						<xsl:call-template name="divisor_Composition"/>
						<xsl:call-template name="endDate_Composition"/>
						<xsl:call-template name="instrument_Composition"/>
						<xsl:call-template name="rank_Composition"/>
						<xsl:call-template name="validityDate_Composition"/>
						<xsl:call-template name="value_Composition"/>
						<xsl:call-template name="mbFields_Composition"/>
					</Composition>
				</xsl:for-each>
				<!-- END - Composition -->
	...
	<xsl:template match="d:FinancialInstrumentME">
		<StructuredProducts>
			<xsl:attribute name="mode">INSUPD</xsl:attribute>
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FinancialInstrumentMiniFutureTurbo',d:id)"/>
			</xsl:attribute>
			<DataGroup>
				<FutureTurbo>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FinancialInstrumentMiniFutureTurbo.xsl+', name(/*), '+', d:eventCommon/ns0:eventId, '+', d:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					<xsl:attribute name="sequenceNumber">
						<xsl:call-template name="calcSeqNum">
							<xsl:with-param name="creationTime">
					...
					<xsl:call-template name="code_MultiEntity"/>
					<xsl:call-template name="synonym-custo_MultiEntity"/>
					<xsl:call-template name="synonym_MultiEntity"/>
					<xsl:call-template name="shortName_MultiEntity"/>
					<xsl:call-template name="longName_MultiEntity"/>
					<xsl:call-template name="multilingualLongName_MultiEntity"/>
					<xsl:call-template name="notepad_MultiEntity"/>
					<xsl:call-template name="userDefinedField-custo_MultiEntity"/>
					<xsl:call-template name="userDefinedField_MultiEntity"/>
					<xsl:call-template name="mbFields_MultiEntity"/>
					...
					<xsl:call-template name="endDate_MultiEntity"/>
					...
					<xsl:call-template name="settlementFrequency_MultiEntity"/>
					<xsl:call-template name="settlementFrequencyUnit_MultiEntity"/>
