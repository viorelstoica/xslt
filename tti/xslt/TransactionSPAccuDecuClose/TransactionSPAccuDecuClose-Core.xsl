...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SyAccuDecu" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b ns0" version="1.0">
      ...
      <accountingDate>
         <xsl:choose>
               ...
               <xsl:when test="b:contractStatus='UNWOUND'">
                  <xsl:value-of select="b:unwindGroup/b:unwindEffDate"/>
               </xsl:when>
               <xsl:when test="b:contractStatus='KNOCKOUT'">
   ...
   <xsl:template name="transactionCode">
      <transactionCode>
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="concat(b:id,$COMPANY_POSTFIX_SEPARATOR,b:mnemonic,$COMPANY_POSTFIX_SEPARATOR,'SELL')"/>
                </xsl:otherwise>
            </xsl:choose>         
      </transactionCode>
   </xsl:template>
   ...
   <xsl:template name="type">
      <type>
         <infra:code>
            <xsl:choose>
               <xsl:when test="(b:productType='ACCUMULATOR') and (b:contractStatus='MATURED')">
                  <xsl:value-of select="'SY_ACCU_CLOSE'"/>
               </xsl:when>
               <xsl:when test="(b:productType='DECUMULATOR') and (b:contractStatus='MATURED')">
                  <xsl:value-of select="'SY_DECU_CLOSE'"/>
               </xsl:when>
               <xsl:when test="(b:productType='ACCUMULATOR') and (b:contractStatus='KNOCKOUT')">
                  <xsl:value-of select="'ACCU_KNOCKOUT'"/>
               </xsl:when>
               <xsl:when test="(b:productType='DECUMULATOR') and (b:contractStatus='KNOCKOUT')">
                  <xsl:value-of select="'DECU_KNOCKOUT'"/>
               </xsl:when>
			   <xsl:when test="(b:productType='ACCUMULATOR') and (b:contractStatus='UNWOUND')">
                  <xsl:value-of select="'ACCU_UNWIND'"/>
               </xsl:when>
               <xsl:when test="(b:productType='DECUMULATOR') and (b:contractStatus='UNWOUND')">
                  <xsl:value-of select="'DECU_UNWIND'"/>
               </xsl:when>
            </xsl:choose>
         </infra:code>
      </type>
