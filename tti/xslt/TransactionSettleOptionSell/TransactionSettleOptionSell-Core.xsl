...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionSettleOptionSell"
		...
		<transactionCode>
			<xsl:choose>
				...
				<xsl:when test="b:tapOperCode and b:tapOperCode != ''">
					<xsl:value-of select="concat(b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,b:id,$COMPANY_POSTFIX_SEPARATOR,b:companyMnemonicJoin)"/>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template name="type">
		<type>
			<infra:code>
				<xsl:value-of select="'OPTION_SEL_CLOSE'"/>
			</infra:code>
		</type>
	</xsl:template>
	...
	<xsl:template name="transactionFeesTaxesCounter">
		<xsl:call-template name="printDxCloseOutFeesAndTaxes">
			<xsl:with-param name="caller" select="'TransactionSettleOptionSell'"/>
		</xsl:call-template>
	</xsl:template>
