	...
	<xsl:template name="transactionCode">
		<transactionCode>
			<xsl:choose>
				...
				<xsl:when test="b:tapOperCode and b:tapOperCode != ''">
					<xsl:value-of select="concat(b:tapOperCode,'_','ST_I_',b:id,$COMPANY_POSTFIX_SEPARATOR,b:companyMnemonic,'_R')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="concat('ST_I_',b:id,$COMPANY_POSTFIX_SEPARATOR,b:companyMnemonic,'_R')"/>
				</xsl:otherwise>
			</xsl:choose>
		</transactionCode>
	</xsl:template>
