	...
	<xsl:template match="b:TransactionEntitlement">
		<CorporateActionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionSpinOffProp',b:diaryId)"/>
			</xsl:attribute>
			<xsl:apply-templates select="b:optionDescGroup"/>
		</CorporateActionPMS>
	</xsl:template>
	<xsl:template match="b:optionDescGroup">
		<xsl:variable name="CheckOptionNomCriteria">
			<xsl:call-template name="CheckOptionNomCriteria"/>
		</xsl:variable>

		<!-- Check If satisfying OptionNom not null Criteria -->
		<xsl:if test="$CheckOptionNomCriteria = 'Y' ">
					...
					<Link>
						<Proportional>
							<xsl:attribute name="description">
								<xsl:value-of select="concat('TransactionSpinOffProp.xsl+', name(/*), '+', ../../b:eventCommon/ns0:eventId, '+', ../../b:eventCommon/ns0:creationTime)"/>
							</xsl:attribute>
							...
							<xsl:call-template name="originalPositionCurrency"/>
							...
							<xsl:call-template name="costProportion"/>
						</Proportional>
					</Link>
				</NewSecurity>
			</SpinOff>
