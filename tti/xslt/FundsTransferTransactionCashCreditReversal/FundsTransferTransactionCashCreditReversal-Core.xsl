...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/FundsTransferTransactionCashCreditReversal"
	...
	<xsl:template name="status">
		...
		<status>
			<xsl:choose>
				...
				<xsl:when test="$opStatus = 'REVERSAL-INPUT'">
					<xsl:call-template name="statusTranslation">
	...
	<xsl:template name="transactionCode">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<transactionCode>
					<xsl:value-of select="concat('FT_',b:id,'_C',$COMPANY_POSTFIX_SEPARATOR,b:companyMnemonic,'_R')"/>
				</transactionCode>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="quantity">
		<quantity>
			<xsl:choose>
				<xsl:when test="substring(b:totalChargeAmt,4) &gt; 0 and b:commissionCode = 'CREDIT LESS CHARGES' and not(b:chargesAcctNo)">
