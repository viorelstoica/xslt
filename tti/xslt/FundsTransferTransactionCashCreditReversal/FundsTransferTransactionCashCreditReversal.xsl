...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/FundsTransferTransactionCashCreditReversal"
	...
	<xsl:template match="c:MultiFundsTransferTransactionCashCreditReversal">
		<xsl:apply-templates select="c:CurrentEvent/b:FundsTransferTransactionCashCreditReversal"/>
	</xsl:template>
	<xsl:template match="batch:BatchFundsTransferTransactionCashCreditReversal">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchFundsTransferTransactionCashCreditReversal" name="BatchFundsTransferTransactionCashCreditReversal">
			<xsl:apply-templates select="b:FundsTransferTransactionCashCreditReversal"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiFundsTransferTransactionCashCreditReversal">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiFundsTransferTransactionCashCreditReversal" name="BatchMultiFundsTransferTransactionCashCreditReversal">
			<xsl:apply-templates select="c:MultiFundsTransferTransactionCashCreditReversal"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:FundsTransferTransactionCashCreditReversal">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FundsTransferTransactionCashCreditReversal',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Cash>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FundsTransferTransactionCashCreditReversal.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
