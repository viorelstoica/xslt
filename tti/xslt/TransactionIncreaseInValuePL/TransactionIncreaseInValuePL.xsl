   ...
   <xsl:template match="b:TransactionEntitlement">
      <CorporateActionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('CoaIncreaseInValuePL',substring-before(b:entitlementId,'.'))"/>
         </xsl:attribute>
         <xsl:apply-templates select="b:optionDescGroup"/>
      </CorporateActionPMS>
   </xsl:template><!-- optionDescGroup with chosen option -->
   <xsl:template match="b:optionDescGroup[b:optionNom != 0]">
      <DataGroup>
         <IncreaseInValue>
            <Link>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('TransactionIncreaseInValuePL.xsl+', name(../*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
