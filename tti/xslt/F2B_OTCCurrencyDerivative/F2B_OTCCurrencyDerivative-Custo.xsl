...
<xsl:stylesheet exclude-result-prefixes="b" version="1.0" xmlns="http://www.temenos.com/T24/OFSML/150" xmlns:b="http://www.odcgroup.com/F2B_FORMAT" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="USERDEF.FIELDS">
		...
		<field name="CSD.CONTACT.BY">
			<xsl:choose>
				<xsl:when test="b:attribute[@name = 'OP_COMM_PARTY'] = 'PCK_GL_ACCT_HOLDER'">
					<xsl:value-of select="'Account Holder'"/>
				</xsl:when>
				<xsl:when test="b:attribute[@name = 'OP_COMM_PARTY'] = 'PCK_GL_PWR_ATTORNEY'">
					<xsl:value-of select="'Attorney'"/>
				</xsl:when>
				<xsl:when test="b:attribute[@name = 'OP_COMM_PARTY'] = 'PCK_GL_BENEF_OWNER'">
					<xsl:value-of select="'Beneficial Owner'"/>
				</xsl:when>
				<xsl:when test="b:attribute[@name = 'OP_COMM_PARTY'] = 'PCK_GL_ACCT_OTHER'">
					<xsl:value-of select="'Other'"/>
				</xsl:when>
				<xsl:when test="b:attribute[@name = 'OP_COMM_PARTY'] = 'PCK_GL_SIGNATORY'">
					<xsl:value-of select="'Signatory'"/>
				</xsl:when>
				<xsl:when test="b:attribute[@name = 'OP_COMM_PARTY'] = 'PCK_GL_THIRD_MANAGER'">
		...
		<field name="CSD.CONT.DATE">
			<xsl:choose>
				<xsl:when test="b:attribute[@name = 'OP_CONTACT_DATE'] != 'NULL'">
					<xsl:variable name="date" select="b:attribute[@name = 'OP_CONTACT_DATE']"/>
		...
		<field name="CSD.CONT.METHOD">
			<xsl:choose>
				<xsl:when test="b:attribute[@name = 'OP_CONTACT_METHOD'] = 'PCK_GL_EMAIL'">
					<xsl:value-of select="'Email'"/>
				</xsl:when>
				<xsl:when test="b:attribute[@name = 'OP_CONTACT_METHOD'] = 'PCK_GL_OTHER'">
					<xsl:value-of select="'Other'"/>
				</xsl:when>
				...
				<xsl:when test="b:attribute[@name = 'OP_CONTACT_METHOD'] = 'PCK_GL_LETTER'">
					<xsl:value-of select="'Letter Post'"/>
				</xsl:when>
				<xsl:when test="b:attribute[@name = 'OP_CONTACT_METHOD'] = 'PCK_GL_MEETING'">
		...
		<field name="CSD.ORDER.INR">
			<xsl:choose>
				<xsl:when test="b:attribute[@name = 'OP_CLIENTINIT_E'] = '2'">
					<xsl:value-of select="'Bank'"/>
				</xsl:when>
				<xsl:when test="b:attribute[@name = 'OP_CLIENTINIT_E'] = '1'">
					<xsl:value-of select="'Client'"/>
				</xsl:when>
				<xsl:when test="b:attribute[@name = 'OP_CLIENTINIT_E'] = ''">
					<xsl:value-of select="''"/>
				</xsl:when>
			</xsl:choose>
		</field>
		...
		<field name="CSD.TRADE.INSTR">
			<!-- <xsl:choose>
			<xsl:when test="b:attribute[@name = 'OP_REMARK']!= 'NULL'">
	...
	<xsl:template name="NARRATIVE">
		<field name="NARRATIVE">
			<xsl:value-of select="substring(b:attribute[@name = 'OP_REMARK'], 1, 35)"/>
		</field>
	</xsl:template>
