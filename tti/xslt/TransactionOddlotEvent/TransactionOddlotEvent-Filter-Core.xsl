...
<xsl:stylesheet xmlns="http://www.odcgroup.com/CorporateActionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionEntitlement" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionEntitlement"
	...
	<xsl:template match="b:TransactionEntitlement">
		...
		<xsl:apply-templates select="self::b:TransactionEntitlement[$opStatus = 'INPUT-AUTHORISE']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
    <xsl:template match="b:TransactionEntitlement" mode="filter1">
        <xsl:variable name="coaToProcess">ODDLOT</xsl:variable>
        <xsl:choose>
            <xsl:when test="$g_filterCOAUsingGlobalTemplateFunction = 'Y'">
                ...
                <xsl:apply-templates select="self::b:TransactionEntitlement[contains(concat(' ',$list_EventType,' '),concat(' ',b:eventType,' '))]" mode="filter2"/>            
            </xsl:when>
            <xsl:otherwise>
                <xsl:apply-templates select="self::b:TransactionEntitlement[b:eventTypeTapCoaType and b:eventTypeTapCoaType = $coaToProcess]" mode="filter2"/>  
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
	...
	<xsl:template match="b:TransactionEntitlement" mode="filter2">
		<xsl:variable name="checkPortfolioConditionVar">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'TransactionOddlotEvent'"/>
		...
		<xsl:apply-templates select="self::b:TransactionEntitlement[$checkPortfolioConditionVar = 'true']" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4-->
	<xsl:template match="b:TransactionEntitlement" mode="filter3">
		<xsl:apply-templates select="self::b:TransactionEntitlement[b:optionDescGroup/b:newSecuritySubGroup/b:nominal &gt; '0']" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
