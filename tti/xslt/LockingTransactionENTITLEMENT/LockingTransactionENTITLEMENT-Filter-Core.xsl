...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/LockingTransactionCOA"
	...
	<xsl:template match="b:LockingTransactionCOA" mode="filter4">
		<xsl:apply-templates select="self::b:LockingTransactionCOA[b:blockType = 'ENTITLEMENT']" mode="filter5"/>
	</xsl:template>
	<!-- Condition 6 -->
	<xsl:template match="b:LockingTransactionCOA" mode="filter5">
		...
		<xsl:apply-templates select="self::b:LockingTransactionCOA[$opStatus = 'INPUT-AUTHORISE' and (b:blockNominal and b:blockNominal != '')]" mode="filter6"/>
	</xsl:template>
	<!-- Condition 7 -->
	<xsl:template match="b:LockingTransactionCOA" mode="filter6">
		...
		<xsl:apply-templates select="self::b:LockingTransactionCOA[number($check_PayDateJoin) &gt;= number($check_toDay)]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
