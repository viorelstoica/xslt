...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionOpenDerivativeSell"
	...
	<xsl:template match="c:MultiTransactionOpenDerivativeSell">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionOpenDerivativeSell"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionOpenDerivativeSell">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionOpenDerivativeSell" name="BatchTransactionOpenDerivativeSell">
			<xsl:apply-templates select="b:TransactionOpenDerivativeSell"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionOpenDerivativeSell">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionOpenDerivativeSell" name="BatchMultiTransactionOpenDerivativeSell">
			<xsl:apply-templates select="c:MultiTransactionOpenDerivativeSell"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionOpenDerivativeSell">
		<TransactionPMS mode="INSUPD">
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionOpenDerivativeSell',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Derivative>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionOpenDerivativeSell.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					...
					<sale>
						...
						<xsl:call-template name="premiumCurrency"/>
						<xsl:call-template name="premiumAmount"/>
					</sale>
				</Derivative>
			</DataGroup>
		</TransactionPMS>
