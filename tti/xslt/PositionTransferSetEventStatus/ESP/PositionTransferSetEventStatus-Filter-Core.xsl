...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch ns0" version="1.0" xmlns="http://www.odcgroup.com/Request" xmlns:b="http://www.temenos.com/T24/event/TTI/PositionTransferDelete"
	...
	<xsl:template match="b:PositionTransferDelete">
		...
		<xsl:apply-templates select="self::b:PositionTransferDelete[$opStatus = 'INPUT-DELETE']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	...
	<xsl:template match="b:PositionTransferDelete" mode="filter6">
		<xsl:apply-templates select="self::b:PositionTransferDelete[b:tapRefId and b:tapRefId != '']" mode="filter7"/>
	</xsl:template>
    <!-- Condition 7 -->
	<xsl:template match="b:PositionTransferDelete" mode="filter7">
		<xsl:apply-templates select="self::b:PositionTransferDelete[b:transferCancel and b:transferCancel = 'Cancellation.Accepted']" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
