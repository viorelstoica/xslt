	...
	<xsl:template match="b:CginventoryAll">
			<ClientPMS>
				...
				<xsl:attribute name="output">
					<xsl:value-of select="concat('ThirdPartyAccProfileHistoAll',b:id)"/>
				</xsl:attribute>
				<DataGroup>
					<ThirdAccProfileHisto>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('ThirdPartyAccProfileHistoAll.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
