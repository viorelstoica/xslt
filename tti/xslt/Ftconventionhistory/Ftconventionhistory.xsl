	...
	<xsl:template match="b:Cginventory">
			<InfraPMS>
				...
				<xsl:attribute name="output">
					<xsl:value-of select="concat('Ftconventionhistory',b:id)"/>
				</xsl:attribute>
				<DataGroup>
					<FtConventionHistory>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('Ftconventionhistory.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
						...
						<xsl:call-template name="taxConvention"/>
					</FtConventionHistory>
				</DataGroup>
			</InfraPMS>
	</xsl:template>
