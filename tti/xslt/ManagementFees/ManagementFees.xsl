...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/ManagementFees"
                xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchManagementFees" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiManagementFees" xmlns:infra="http://www.odcgroup.com/InfraPMS"
                xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiManagementFees" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
	<xsl:output encoding="UTF-8" indent="yes" method="xml"/>
	<xsl:template match="c:MultiManagementFees">
		<xsl:apply-templates select="c:CurrentEvent/b:ManagementFees"/>
	</xsl:template>
	<xsl:template match="batch:BatchManagementFees">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchManagementFees" name="BatchManagementFees">
			<xsl:apply-templates select="b:ManagementFees"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiManagementFees">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiManagementFees" name="BatchMultiManagementFees">
			<xsl:apply-templates select="c:MultiManagementFees"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:ManagementFees">
		<TransactionPMS>
			<xsl:attribute name="mode">INSUPD</xsl:attribute>
			<xsl:attribute name="output">
				<xsl:value-of select="concat('ManagementFees',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Cash>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('ManagementFees.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="ManagementFees" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\ManagementFees\ManagementFees.xml" htmlbaseurl="" outputurl="..\..\TestFiles\ManagementFees\ManagementFeesCusto.xml" processortype="jaxp"
		          useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath=""
		          postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator="">
			<advancedProp name="sInitialMode" value=""/>
