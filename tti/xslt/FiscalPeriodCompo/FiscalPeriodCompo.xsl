	...
	<xsl:template match="b:Company">
		<InfraPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FiscalPeriodCompo',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<FiscalPeriodCompo>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FiscalPeriodCompo.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					...
					<xsl:call-template name="fiscalPeriodFreq"/>
					<xsl:call-template name="fiscalPeriodFreqUnit"/>
					<xsl:call-template name="instrumentNature"/>
					<xsl:call-template name="nature_e"/>
