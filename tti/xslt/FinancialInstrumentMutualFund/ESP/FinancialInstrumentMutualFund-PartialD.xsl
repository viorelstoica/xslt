   ...
   <xsl:template match="b:FinancialInstrumentSecurity">
      <FiPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FinancialInstrumentMutualFund-PartialD',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <FundShare>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('FinancialInstrumentMutualFund-PartialD.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
