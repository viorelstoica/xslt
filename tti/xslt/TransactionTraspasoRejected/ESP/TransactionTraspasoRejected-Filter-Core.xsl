...
<xsl:stylesheet xmlns="http://www.odcgroup.com/F2B" xmlns:b="http://www.temenos.com/T24/event/TTI/EsfundTraspasoRequest" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchEsfundTraspasoRequest"
	...
	<xsl:template match="b:EsfundTraspasoRequest" mode="filter1">
		<xsl:apply-templates select="self::b:EsfundTraspasoRequest[b:transInStatus = 'REJECTED']" mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -->
	<xsl:template match="b:EsfundTraspasoRequest" mode="filter2">
		<xsl:apply-templates select="self::b:EsfundTraspasoRequest[(../../c:PreviousEvent/b:EsfundTraspasoRequest/b:transInStatus = 'REQUEST.SENT')]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
