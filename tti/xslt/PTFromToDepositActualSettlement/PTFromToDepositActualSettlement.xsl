	...
	<xsl:template match="b:PositionTransferScSettlement">
		<GenericCOA>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('PTFromToDepositActualSettlement',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<xsl:for-each select="b:securityAcctGroup">
					<Adjustment>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('PTFromToDepositActualSettlement.xsl+', name(/*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
