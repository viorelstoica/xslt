...
<xsl:stylesheet xmlns="http://www.odcgroup.com/GenericCOA" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionEntitlement" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionEntitlement"
    ...
    <xsl:template match="b:TransactionEntitlement">
        <xsl:variable name="coaToProcess">DISTRIBUTION</xsl:variable>
        <xsl:choose>
	...
	<xsl:template match="b:TransactionEntitlement" mode="filter1">
		<xsl:variable name="checkPortfolioConditionVar">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'TransactionDistribution'"/>
	...
	<xsl:template match="b:optionDescGroup">
		<xsl:apply-templates select="self::b:optionDescGroup[b:optionNom != 0]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
