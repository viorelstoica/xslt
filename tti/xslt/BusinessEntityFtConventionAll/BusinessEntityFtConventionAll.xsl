	...
	<xsl:template match="b:CginventoryAll">
			<InfraPMS>
				...
				<xsl:attribute name="output">
					<xsl:value-of select="concat('BusinessEntityFtConventionAll',b:id)"/>
				</xsl:attribute>
				<DataGroup>
					<BusinessEntityFtConvention>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('BusinessEntityFtConventionAll.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
