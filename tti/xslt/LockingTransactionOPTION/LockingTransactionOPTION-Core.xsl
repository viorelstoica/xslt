...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/LockingTransactionCOA"
			...
			<xsl:choose>
				<xsl:when test="b:tapOperCode and b:tapOperCode != ''">
					<xsl:choose>
						...
						<xsl:when test=" $opStatus = 'INPUT-INPUT' and ../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal and ../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal != '' and (not(b:blockNominal) or b:blockNominal = '')">
							<xsl:value-of select=" concat(b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,'B4_',b:id,$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin) "/>
						</xsl:when>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test=" $opStatus = 'INPUT-INPUT' and ../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal and ../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal != '' and (not(b:blockNominal) or b:blockNominal = '')">
							<xsl:value-of select=" concat($externalSequenceNo,'_','B4_',b:id,$COMPANY_POSTFIX_SEPARATOR,b:CompanyMnemonicJoin) "/>
						</xsl:when>
						<xsl:when test=" $opStatus = 'INPUT-INPUT'">
	...
	<xsl:template name="lockingUnlocking">
		<lockingUnlocking>
			...
			<xsl:choose>
				<xsl:when test=" $opStatus = 'INPUT-AUTHORISE' or $opStatus = 'INPUT-DELETE' or ($opStatus = 'INPUT-INPUT' and ../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal and ../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal != '' and (not(b:blockNominal) or b:blockNominal = ''))">
	...
	<xsl:template name="quantity">
		<quantity>
			...
			<xsl:choose>
				<xsl:when test="$opStatus = 'INPUT-INPUT' and ../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal and ../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal != '' and (not(b:blockNominal) or b:blockNominal = '')">
					<xsl:value-of select="../../c:PreviousEvent/b:LockingTransactionCOA/b:blockNominal"/>
				</xsl:when>
				<xsl:when test="$opStatus = 'INPUT-INPUT' or $opStatus = 'INPUT-AUTHORISE' or $opStatus = 'INPUT-DELETE'">
