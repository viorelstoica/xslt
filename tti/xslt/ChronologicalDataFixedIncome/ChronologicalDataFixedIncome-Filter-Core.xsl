...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch" version="1.0" xmlns="http://www.odcgroup.com/FiPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/ChronologicalDataFixedIncome"
	...
	<xsl:template match="b:ChronologicalDataFixedIncome">
		...
		<xsl:apply-templates select="self::b:ChronologicalDataFixedIncome[($g_filterBySubAssetType='Y' and $subAssetType and contains(concat(' ',$list_SubAssetType_ChronologicalData,' '), concat(' ',$subAssetType,' '))) or
                                    ($g_filterBySubAssetType != 'Y' and b:subAssetTypeTapInstrType and (b:subAssetTypeTapInstrType='CONVBOND' or b:subAssetTypeTapInstrType='FIXEDINCOME' or b:subAssetTypeTapInstrType='STOCK'))]" mode="filter-custo"/>
	</xsl:template>
