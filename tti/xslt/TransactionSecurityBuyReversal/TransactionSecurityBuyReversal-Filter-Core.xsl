...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionSecurityBuyReversal" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionSecurityBuyReversal"
                xmlns:c="http://www.temenos.com/T24/event/TTI/MultiTransactionSecurityBuyReversal" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSecurityBuyReversal"
	...
	<xsl:template match="b:TransactionSecurityBuyReversal">
		...
		<xsl:apply-templates select="self::b:TransactionSecurityBuyReversal[$opStatus = 'REVERSE-AUTHORISE']" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:TransactionSecurityBuyReversal" mode="filter1">
		<xsl:apply-templates select="self::b:TransactionSecurityBuyReversal[($g_filterBySubAssetType='Y' and b:subAssetTypeJoin and contains ($list_SubAssetType_TransactionSecurityBuyReversal,b:subAssetTypeJoin)) or
	...
	<!-- Condition 3 -->
	<xsl:template match="b:TransactionSecurityBuyReversal" mode="filter2">
			...
			<xsl:for-each select="b:customerNoGroup/b:custSecAcc">
				...
				<xsl:variable name="checkPortfolioConditionVar">
					<xsl:call-template name="checkPortfolioCondition">
						<xsl:with-param name="xsltName" select="'TransactionSecurityBuyReversal'"/>
				...
				<xsl:choose>
					<xsl:when test="$checkPortfolioConditionVar = 'true' and (contains($list_TransCode_TransactionSecurityBuy,../b:custTransCode) or (contains($peFundsTransCodeBuy,../b:custTransCode) and ../../b:peFund= 'YES' and ../../b:peType = 'CAPITAL CALL'))">
		...
		<xsl:apply-templates select="self::b:TransactionSecurityBuyReversal[contains($ifFilterWholeRecord,'Y')]" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
	<xsl:template match="b:TransactionSecurityBuyReversal" mode="filter3">
		<xsl:apply-templates select="self::b:TransactionSecurityBuyReversal[(b:parent != 'YES' or not(b:parent))]" mode="filter-custo"/>
	</xsl:template>		
	<!-- Custo placeholder -->
	<xsl:template match="b:TransactionSecurityBuyReversal" mode="filter-custo">
		<xsl:apply-templates select="self::b:TransactionSecurityBuyReversal" mode="filter-final"/>
	</xsl:template>
	<!-- Copy block -->
	<xsl:template match="b:TransactionSecurityBuyReversal" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="b:customerNoGroup/b:custSecAcc" mode="filter1">
		<xsl:variable name="index" select="../@index"/>
		<xsl:apply-templates select="self::node()[contains($list_TransCode_TransactionSecurityBuy,../b:custTransCode) or (contains($peFundsTransCodeBuy,../b:custTransCode) and ../../b:peFund= 'YES' and ../../b:peType = 'CAPITAL CALL')]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
