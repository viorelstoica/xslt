   ...
   <xsl:template match="b:FinancialInstrumentSecurity">
      <StructuredProducts>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('SpAirBagCertificates-PartialD',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <AirbagCertificates>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('SpAirBagCertificates-PartialD.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
