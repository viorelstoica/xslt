...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch ns0 d e f g" version="1.0" xmlns="http://www.odcgroup.com/StructuredProducts" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon"
	...
	<xsl:template match="b:FinancialInstrumentSecurity">
		...
		<xsl:apply-templates select="self::b:FinancialInstrumentSecurity[($g_filterBySubAssetType='Y' and $subAssetType and contains(concat(' ',$list_SubAssetType_SpAirBagCertificates,' '),concat(' ',$subAssetType,' '))) or
									($g_filterBySubAssetType != 'Y' and $subAssetTypeTapInstrType and ($subAssetTypeTapInstrType='AIRBAGCERT'))]" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	...
	<xsl:template match="d:FinancialInstrumentME">
		...
		<xsl:apply-templates select="self::d:FinancialInstrumentME[$g_filterBySubAssetType='Y' and $subAssetType and contains(concat(' ',$list_SubAssetType_SpAirBagCertificates,' '),concat(' ',$subAssetType,' '))
		                              or
		                              ($g_filterBySubAssetType != 'Y' and $subAssetTypeTapInstrType and ($subAssetTypeTapInstrType='AIRBAGCERT'))]" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
