...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/GenericCOA" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionEntitlement" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionEntitlement"
	...
	<xsl:template name="tradeCcyGrossAmount_Sell">
		<tradeCcyGrossAmount>
			<xsl:value-of select="b:optionDescGroup/b:entitlementAmt"/>
		</tradeCcyGrossAmount>
	</xsl:template>
	...
	<xsl:template name="tradePrice_Sell">
		<xsl:choose>
			<xsl:when test="b:pricetypeSecurityjoinGroup/b:pricetypeSecurityjoinSubGroup/b:pricetypeSecurityjoin = 'BOND' or b:pricetypeSecurityjoinGroup/b:pricetypeSecurityjoinSubGroup/b:pricetypeSecurityjoin = 'DBOND'">
				<tradePrice>
					<xsl:value-of select="format-number((b:optionDescGroup/b:entitlementAmt) div (b:finalDebitQty),$decimalformat,'nan2zero')*100"/>
				</tradePrice>
			</xsl:when>
			<xsl:otherwise>
				<tradePrice>
					<xsl:value-of select="format-number((b:optionDescGroup/b:entitlementAmt) div (b:finalDebitQty),$decimalformat,'nan2zero')"/>
				</tradePrice>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
