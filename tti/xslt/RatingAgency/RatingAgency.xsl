...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/RatingAgency" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchRatingAgency" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiRatingAgency" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiRatingAgency" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/>
   <xsl:template match="c:MultiRatingAgency">
      <xsl:apply-templates select="c:CurrentEvent/b:RatingAgency"/>
   </xsl:template>
   <xsl:template match="batch:BatchRatingAgency">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchRatingAgency" name="BatchRatingAgency">
         <xsl:apply-templates select="b:RatingAgency"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiRatingAgency">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiRatingAgency" name="BatchMultiRatingAgency">
         <xsl:apply-templates select="c:MultiRatingAgency"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:RatingAgency">
      <InfraPMS>
         <xsl:attribute name="mode">INSUPD</xsl:attribute>
         <xsl:attribute name="output">
            <xsl:value-of select="concat('ratingAgency',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <Codification>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('RatingAgency.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="RatingAgency" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\RatingAgency\RatingAgency.xml" htmlbaseurl="" outputurl="..\..\TestFiles\RatingAgency\RatingAgencyCusto.xml" processortype="jaxp"
		          useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath=""
		          postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator="">
			<advancedProp name="sInitialMode" value=""/>
			<advancedProp name="bSchemaAware" value="false"/>
