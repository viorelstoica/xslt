   ...
   <xsl:template match="c:AAAFlow">
      <TransactionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('transactionAAPayOffLending',c:aatxncontext/b:aaArrangementId)"/>
         </xsl:attribute>
         <DataGroup>
            <xsl:call-template name="mappingTransactionAAPayOffLending"/>
         </DataGroup>
      </TransactionPMS>
   </xsl:template>
   <xsl:template name="mappingTransactionAAPayOffLending">
      <xsl:for-each select="c:aaarecord">
         <DepositLoan>
            <xsl:attribute name="description">
               <xsl:value-of select="concat('TransactionAAPayOffLending.xsl+', name(/*), '+', ../c:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
            </xsl:attribute>
...
<metaInformation>
	<scenarios>
		<scenario default="yes" name="Scenario1" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\TransactionAAPayOffLending\TransactionAAPayOffLending.xml" htmlbaseurl=""
		          outputurl="..\..\TestFiles\TransactionAAPayOffLending\TransactionAAPayOffLendingGW.xml" processortype="jaxp" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath=""
