  ...
  <xsl:template match="c:aaarecord">
    ...
    <xsl:if test="  ../c:aatxncontext/b:aaMasterActivity = 'LENDING-SETTLE-PAYOFF'
           and d:activity = 'LENDING-SETTLE-PAYOFF'
		         and ../c:billdetails/j:aaBillId = $var_billId 
				       and ../c:accountrecord/g:portfolioId != ''   
					     and contains($list_auth_TransactionAAPayOffLending,../c:recarrangement/i:arrStatus)    
						    and not(../c:aatxncontext/b:aaActivityStatus) ">
      <xsl:copy-of select="."/>
    </xsl:if>
  </xsl:template>
</xsl:stylesheet>
