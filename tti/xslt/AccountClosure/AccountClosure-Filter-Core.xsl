...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch ns0" version="1.0" xmlns="http://www.odcgroup.com/FiPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/AccountClosure"
	...
	<xsl:template match="b:AccountClosure">
		<xsl:apply-templates select="self::b:AccountClosure[(b:closeOnline = 'Y' and b:acctCloseDateAccountClosedJoin != '')]"
		                     mode="filter-custo"/>
	</xsl:template>

	<!-- Custo placeholder -->
	<xsl:template match="b:AccountClosure" mode="filter-custo">
		<xsl:apply-templates select="self::b:AccountClosure" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:AccountClosure" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
