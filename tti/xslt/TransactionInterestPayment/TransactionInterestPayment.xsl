   ...
   <xsl:template match="b:TransactionEntitlement">
      <GenericCOA>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('CoaInterestPayment',b:diaryId)"/>
         </xsl:attribute>
         <DataGroup>
            <Income>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('TransactionInterestPayment.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
