...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/GenericCOA" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionAssignExpiryOptionSellCloseOutRev"
	...
	<xsl:template name="transactionDate">
		<xsl:choose>
			...
			<xsl:when test="b:creation = 'AUTOMATIC' and b:type = 'EXERCISE'">
				<transactionDate>
					<xsl:value-of select="b:eXAutoValueDateGroup/b:eXAutoValueDate"/>
				</transactionDate>
			</xsl:when>
			<xsl:when test="b:creation = 'AUTOMATIC' and b:type = 'ASSIGNMENT'">
				<transactionDate>
					<xsl:value-of select="b:asAutoValueDateGroup/b:asAutoValueDate"/>
				</transactionDate>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
