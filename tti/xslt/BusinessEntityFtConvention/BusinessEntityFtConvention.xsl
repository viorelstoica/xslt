	...
	<xsl:template match="b:Cginventory">
			<InfraPMS>
				...
				<xsl:attribute name="output">
					<xsl:value-of select="concat('BusinessEntityFtConvention',b:id)"/>
				</xsl:attribute>
				<DataGroup>
					<BusinessEntityFtConvention>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('BusinessEntityFtConvention.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
