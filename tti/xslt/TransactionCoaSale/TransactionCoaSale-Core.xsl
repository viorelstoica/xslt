         ...
         <reversedTransactionCode>
		 	<xsl:choose>
				<xsl:when test="../b:tapOperCode and ../b:tapOperCode != ''">
            		<xsl:value-of select="concat(../b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,../b:entitlementId,'_',@index,$COMPANY_POSTFIX_SEPARATOR,../b:CompanyMnemonicJoin)"/>
				</xsl:when>
				<xsl:otherwise>
      ...
      <tradePrice>
         <xsl:choose>
            <xsl:when test="../b:PriceTypeSecurityNoJoin = 'BOND' or ../b:PriceTypeSecurityNoJoin = 'DBOND' ">
               <xsl:value-of select="format-number((b:entitlementAmt div ../b:secNoGroup[@index=1]/b:secNominal) * -100,'#.####','nan2zero')"/>
            </xsl:when>
            <xsl:otherwise>
		 	...
		 	<xsl:when test="../b:tapOperCode and ../b:tapOperCode != ''">
				<xsl:choose>
            		<xsl:when test="$opStatus = 'REVERSE-AUTHORISE'">
               			<xsl:value-of select="concat(../b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,../b:entitlementId,'_',@index,'_R',$COMPANY_POSTFIX_SEPARATOR,../b:CompanyMnemonicJoin)"/>
            		</xsl:when>
            		<xsl:otherwise>
               			<xsl:value-of select="concat(../b:tapOperCode,$COMPANY_POSTFIX_SEPARATOR,../b:entitlementId,'_',@index,$COMPANY_POSTFIX_SEPARATOR,../b:CompanyMnemonicJoin)"/>
            		</xsl:otherwise>
