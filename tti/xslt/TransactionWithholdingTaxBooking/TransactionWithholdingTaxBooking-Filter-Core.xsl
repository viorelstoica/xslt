...
<xsl:stylesheet xmlns="http://www.odcgroup.com/GenericCOA" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionEntitlement" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionEntitlement"
    ...
    <xsl:template match="b:TransactionEntitlement">
        <xsl:variable name="coaToProcess">NOTIONAL.DIV</xsl:variable>
        <xsl:choose>
	...
	<xsl:template match="b:TransactionEntitlement" mode="filter1">
		<xsl:variable name="checkPortfolioConditionVar">
			<xsl:call-template name="checkPortfolioCondition">
				<xsl:with-param name="xsltName" select="'TransactionWithholdingTaxBooking'"/>
		...
		<xsl:apply-templates select="self::b:TransactionEntitlement[$checkPortfolioConditionVar = 'true']" mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -Transaction status: opStatus -->
	<xsl:template match="b:TransactionEntitlement" mode="filter2">
		...
		<xsl:apply-templates select="self::b:TransactionEntitlement[$opStatus = 'INPUT-AUTHORISE']" mode="filter3"/>
	</xsl:template>
	<!-- Condition 4 -->
	<xsl:template match="b:TransactionEntitlement" mode="filter3">
		<xsl:apply-templates select="self::b:TransactionEntitlement[count(b:optionDescGroup/b:entitlementAmt) = '1']" mode="filter4"/>
	</xsl:template>
	<!-- Condition 5 -->
	<xsl:template match="b:TransactionEntitlement" mode="filter4">
		<xsl:apply-templates select="self::b:TransactionEntitlement[b:netAmtAccCur != '']" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
