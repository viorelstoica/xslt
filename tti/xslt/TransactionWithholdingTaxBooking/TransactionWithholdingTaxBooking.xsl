	...
	<xsl:template match="b:TransactionEntitlement">
		<GenericCOA>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('CoaWithholdingTaxBooking',b:diaryId)"/>
			</xsl:attribute>
			<DataGroup>
				<Income>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionWithholdingTaxBooking.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
