	...
	<xsl:template match="b:NdDeal">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('NdForwardSaleClosing',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<ForexForward>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('NdForwardSaleClosing.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
