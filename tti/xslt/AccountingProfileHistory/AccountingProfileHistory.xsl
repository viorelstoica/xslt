	...
	<xsl:template match="b:Cginventory">
			<ClientPMS>
				...
				<xsl:attribute name="output">
					<xsl:value-of select="concat('AccountingProfileHistory',b:id)"/>
				</xsl:attribute>
				<DataGroup>
					<AccountingProfileHisto>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('AccountingProfileHistory.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
