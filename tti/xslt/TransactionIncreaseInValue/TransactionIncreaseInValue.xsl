   ...
   <xsl:template match="b:TransactionEntitlement">
      <CorporateActionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('CoaIncreaseInValue',substring-before(b:entitlementId,'.'))"/>
         </xsl:attribute><!-- Handle optionDescGroup -->
         <xsl:apply-templates select="b:optionDescGroup"/>
      </CorporateActionPMS>
   </xsl:template><!-- optionDescGroup -->
   <xsl:template match="b:optionDescGroup"><!-- Print record -->
      <DataGroup>
         <IncreaseInValue>
            <Link>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('TransactionIncreaseInValue.xsl+', name(/*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
			   ...
			   <xsl:call-template name="localPrintEntitlementFeesAndTaxes"/>
               ...
               <xsl:call-template name="increaseAmount"/>			   
			   <xsl:call-template name="splitPortfolio"/>
            </Link>
         </IncreaseInValue>
      </DataGroup>
   </xsl:template><!-- End of optionDescGroup -->
