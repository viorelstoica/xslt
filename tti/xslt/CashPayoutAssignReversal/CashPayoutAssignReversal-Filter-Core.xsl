...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionAssignExpiryOptionSellCloseOutRev"
	...
	<xsl:template match="b:TransactionAssignExpiryOptionSellCloseOutRev" mode="filter1">
		<xsl:apply-templates select="self::b:TransactionAssignExpiryOptionSellCloseOutRev[not(b:dealerBookList/b:dealerBook) or b:dealerBookList/b:dealerBook='']" mode="filter2"/>
	</xsl:template>
	<!-- Condition 3 -->
	...
	<xsl:template match="b:TransactionAssignExpiryOptionSellCloseOutRev" mode="filter4">
		<xsl:apply-templates select="self::b:TransactionAssignExpiryOptionSellCloseOutRev[(b:coPgmList/b:coPgm and b:coPgmList/b:coPgm='DX.XO.INSTANT.CASHPAYOUT') or (b:settlementMethod and b:settlementMethod='CASH')]" mode="filter5"/>
	</xsl:template>
	...
	<xsl:template match="b:TransactionAssignExpiryOptionSellCloseOutRev" mode="filter6">
		<xsl:apply-templates select="self::b:TransactionAssignExpiryOptionSellCloseOutRev[(b:type = 'EXPIRY' or b:type = 'ASSIGNMENT')]" mode="filter-custo"/>
	</xsl:template>
