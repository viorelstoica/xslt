   ...
   <xsl:template match="b:SyFxForwards">
      <FiPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('FiPFCallOpt',translate(translate(b:id,'/',''),':',''))"/>
         </xsl:attribute>
         <DataGroup>
            <Option>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('FiPFCallOpt.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
