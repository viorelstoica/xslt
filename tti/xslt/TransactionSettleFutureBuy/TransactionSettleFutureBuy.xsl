...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionSettleFutureBuy"
	...
	<xsl:template match="c:MultiTransactionSettleFutureBuy">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionSettleFutureBuy"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionSettleFutureBuy">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionSettleFutureBuy" name="BatchTransactionSettleFutureBuy">
			<xsl:apply-templates select="b:TransactionSettleFutureBuy"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionSettleFutureBuy">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionSettleFutureBuy" name="BatchMultiTransactionSettleFutureBuy">
			<xsl:apply-templates select="c:MultiTransactionSettleFutureBuy"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionSettleFutureBuy">
		<TransactionPMS mode="INSUPD">
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionSettleFutureBuy',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<Derivative>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('TransactionSettleFutureBuy.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
