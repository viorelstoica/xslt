		...
		<transactionCode>
			<xsl:choose>
				<xsl:when test="../b:tapOperCode and ../b:tapOperCode != ''">
					<xsl:value-of select="concat(../b:tapOperCode,'_','ST_O_',$IdVar,'_',b:securityNo,'_',b:depository,$COMPANY_POSTFIX_SEPARATOR,../b:companyMnemonicJoin)"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="concat('ST_O_',$IdVar,'_',b:securityNo,'_',b:depository,$COMPANY_POSTFIX_SEPARATOR,../b:companyMnemonicJoin)"/>
				</xsl:otherwise>
			</xsl:choose>
		</transactionCode>
	</xsl:template>
