	...
	<xsl:template match="b:CginventoryAll">
		<InfraPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('FtConventionAll',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<FtConvention>
					<xsl:attribute name="description">
						<xsl:value-of select="concat('FtConventionAll.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
					</xsl:attribute>
					...
					<xsl:call-template name="taxStatus"/>
				</FtConvention>
			</DataGroup>
		</InfraPMS>
	</xsl:template>
