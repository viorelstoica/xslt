	...
	<xsl:template match="b:PaymentOrderNotification">
			...
			<xsl:choose>
				<xsl:when test="b:paymentOrderCreditAccountPortfolioNoList/b:paymentOrderCreditAccountPortfolioNo and b:paymentOrderCreditAccountPortfolioNoList/b:paymentOrderCreditAccountPortfolioNo != ''">
					<xsl:call-template name="checkPortfolioCondition">
						<xsl:with-param name="xsltName" select="'TransactionPaymentOrderNotificationCredit'"/>
		...
		<xsl:choose>
			<xsl:when test="(b:paymentOrderTapRefId != '' and b:paymentOrderTapOperNature = 'IN')">
				<xsl:apply-templates select="self::b:PaymentOrderNotification[b:paymentComplete = 'YES' and b:paymentOrderTapRefId != '' and $checkPortfolioConditionVar = 'true']" mode="filter-custo"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="self::b:PaymentOrderNotification[b:paymentComplete = 'YES' and b:paymentOrderTapRefId != '' and b:paymentOrderTapOperNature = 'WI']" mode="filter-custo"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!-- Custo placeholder -->
