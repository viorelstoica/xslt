...
<xsl:stylesheet exclude-result-prefixes="b c d e f g ns0 batch multibatch"
	...
	<xsl:template match="b:TPHServiceFlow">
		<xsl:variable name="checkPortfolioConditionVar">
			<xsl:if test="b:accountdetails/g:creditPortfolioId and b:accountdetails/g:creditPortfolioId != ''">
				<xsl:call-template name="checkPortfolioCondition">
					<xsl:with-param name="xsltName" select="'TransactionPaymentIncomingTPH'"/>
					<xsl:with-param name="memoAccount" select="b:accountdetails/g:creditMemoAccount"/>
					<xsl:with-param name="dealerBook" select="b:accountdetails/g:creditDealerBook"/>
					<xsl:with-param name="customerType" select="b:accountdetails/g:creditCustomerType"/>
				</xsl:call-template>
			</xsl:if>
		</xsl:variable>
		<xsl:apply-templates select="self::b:TPHServiceFlow[(not(b:tapRefId) or b:tapRefId = '') and $checkPortfolioConditionVar = 'true' and b:portransactionrec/d:statusCode != '993' and b:portransactionrec/d:statusCode != '996']"
		                     mode="filter-custo"/>
	</xsl:template>
