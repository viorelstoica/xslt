...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/InfraPMS" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionOperationType" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionOperationType" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiTransactionOperationType" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionOperationType" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   <xsl:output encoding="UTF-8" indent="yes" method="xml"/>
   <xsl:template match="c:MultiTransactionOperationType">
      <xsl:apply-templates select="c:CurrentEvent/b:TransactionOperationType"/>
   </xsl:template>
   <xsl:template match="batch:BatchTransactionOperationType">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionOperationType" name="BatchTransactionOperationType">
         <xsl:apply-templates select="b:TransactionOperationType"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="multibatch:BatchMultiTransactionOperationType">
      <xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionOperationType" name="BatchMultiTransactionOperationType">
         <xsl:apply-templates select="c:MultiTransactionOperationType"/>
      </xsl:element>
   </xsl:template>
   <xsl:template match="b:TransactionOperationType">
      <InfraPMS>
         <xsl:attribute name="mode">INSUPD</xsl:attribute>
         <xsl:attribute name="output">
            <xsl:value-of select="concat('transactionOperationType',b:id)"/>
         </xsl:attribute>
         <DataGroup>
            <Type>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('TransactionOperationType.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="TransactionOperationType" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\TransactionOperationType\TransactionOperationType.xml" htmlbaseurl=""
		          outputurl="..\..\TestFiles\TransactionOperationType\TransactionOperationTypeCusto.xml" processortype="jaxp" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath=""
