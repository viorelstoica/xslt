	...
	<xsl:template match="b:Cginventory">
			<ClientPMS>
				...
				<xsl:attribute name="output">
					<xsl:value-of select="concat('ThirdPartyAccProfileHisto',b:id)"/>
				</xsl:attribute>
				<DataGroup>
					<ThirdAccProfileHisto>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('ThirdPartyAccProfileHisto.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
