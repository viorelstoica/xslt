...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/PositionTransferPortfolioTransfer"
	...
	<xsl:template match="c:MultiPositionTransferPortfolioTransfer">
		<xsl:apply-templates select="c:CurrentEvent/b:PositionTransferPortfolioTransfer"/>
	</xsl:template>
	<xsl:template match="batch:BatchPositionTransferPortfolioTransfer">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchPositionTransferPortfolioTransfer" name="BatchPositionTransferPortfolioTransfer">
			<xsl:apply-templates select="b:PositionTransferPortfolioTransfer"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiPositionTransferPortfolioTransfer">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiPositionTransferPortfolioTransfer" name="BatchMultiPositionTransferPortfolioTransfer">
			<xsl:apply-templates select="c:MultiPositionTransferPortfolioTransfer"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:PositionTransferPortfolioTransfer">
		<TransactionPMS>
			...
			<xsl:attribute name="output">
				<xsl:value-of select="concat('PositionTransferPortfolioTransfer',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<xsl:for-each select="b:securityNoGroup">
					<Security>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('PositionTransferPortfolioTransfer.xsl+', name(/*), '+', ../b:eventCommon/ns0:eventId, '+', ../b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
						...
						<portfolioTransfer>
							...
							<xsl:call-template name="adjBoPortfolio"/>
