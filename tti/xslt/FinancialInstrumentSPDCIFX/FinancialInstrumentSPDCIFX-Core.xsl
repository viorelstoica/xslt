...
<xsl:stylesheet xmlns="http://www.odcgroup.com/StructuredProducts" 
   ...
   <xsl:template name="shortName">
      <infra:shortName>
         <xsl:choose>
            ...
            <xsl:otherwise>
                 <xsl:value-of select="concat(b:id,$COMPANY_POSTFIX_SEPARATOR,'FX')"/>
            </xsl:otherwise>
         </xsl:choose>             
      </infra:shortName>
   </xsl:template>
   ...
   <xsl:template name="barrier">
      <barrier>
        <xsl:if test="b:exoticTypeGroup[b:exoticType = 'KNOCKIN']/b:usrFldTextSubGroup/b:usrFldVal">
            <xsl:value-of select="b:exoticTypeGroup[b:exoticType = 'KNOCKIN']/b:usrFldTextSubGroup/b:usrFldVal"/>
        </xsl:if> 
      </barrier>
   </xsl:template>
   <xsl:template name="barrierNature">
        <xsl:if test="b:exoticTypeGroup[b:exoticType = 'KNOCKIN']/b:usrFldTextSubGroup/b:usrFldVal">
            <barrierNature>3</barrierNature>
        </xsl:if>    
   </xsl:template>
   ...
   <xsl:template name="upperBarrier">
      <upperBarrier>
        <xsl:if test="b:exoticTypeGroup[b:exoticType = 'KNOCKOUT']/b:usrFldTextSubGroup/b:usrFldVal">
            <xsl:value-of select="b:exoticTypeGroup[b:exoticType = 'KNOCKOUT']/b:usrFldTextSubGroup/b:usrFldVal"/>
        </xsl:if>  
      </upperBarrier>
   </xsl:template>   
   <xsl:template name="upperBarrierNature">
        <xsl:if test="b:exoticTypeGroup[b:exoticType = 'KNOCKOUT']/b:usrFldTextSubGroup/b:usrFldVal">
            <upperBarrierNature>4</upperBarrierNature>
        </xsl:if>    
   </xsl:template>
   ...
   <xsl:template name="underlyingInstrument">
      <underlyingInstrument>
         <infra:code>
            <xsl:value-of select="b:alternateCcy1"/>
         </infra:code>
      </underlyingInstrument>
   </xsl:template>
