...
<xsl:stylesheet xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SyDci" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSyDci" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiSyDci" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiSyDci" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
  ...
  <xsl:template match="b:SyDci">
    <xsl:apply-templates select="self::b:SyDci[b:unwindChgAmt!='0']" mode="filter1"/>
  </xsl:template>
  <!-- Condition 2 -->
  ...
  <xsl:template match="b:SyDci" mode="filter3">
    <xsl:apply-templates select="self::b:SyDci[b:dealerBook = '' or not(b:dealerBook)]" mode="filter4"/>
  </xsl:template>
  <!-- Condition 5 -->
  <xsl:template match="b:SyDci" mode="filter4">
    <xsl:apply-templates select="self::b:SyDci[b:memoAccount = 'NO']" mode="filter-custo"/>
  </xsl:template>
  <!-- Custo placeholder -->
