...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/CorporateActionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionEntitlement" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchTransactionEntitlement" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiTransactionEntitlement" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionEntitlement" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
   ...
   <xsl:template match="b:TransactionEntitlement">
      <CorporateActionPMS>
         ...
         <xsl:attribute name="output">
            <xsl:value-of select="concat('CoaFinalMaturity',b:diaryId)"/>
         </xsl:attribute><!-- Print record -->
         <DataGroup>
            <FinalMaturity>
               <xsl:attribute name="description">
                  <xsl:value-of select="concat('TransactionFinalMaturity.xsl+', name(/*), '+', b:eventCommon/ns0:eventId, '+', b:eventCommon/ns0:creationTime)"/>
               </xsl:attribute>
               ...
               <xsl:call-template name="account1CcyRedemptionCcyExchangeRate"/>
               <xsl:call-template name="account1NetAmount"/>
               <xsl:call-template name="account2"/>
               <xsl:call-template name="account2CcyRedemptionCcyExchangeRate"/>
               <xsl:call-template name="account2NetAmount"/>
               <xsl:call-template name="account3"/>
               <xsl:call-template name="account3CcyRedemptionCcyExchangeRate"/>
               ...
               <xsl:call-template name="redemptionCcyGrossAmount"/>
               <xsl:call-template name="redemptionCcyNetAmount"/>
               <xsl:call-template name="redemptionCcyPortfolioCcyExchangeRate"/>
               <xsl:call-template name="redemptionCurrency"/>
               <xsl:call-template name="redemptionPrice"/>
               <xsl:call-template name="redemptionQuantity"/>
   ...
   </xsl:template><!-- End of TransactionEntitlement -->
