...
<xsl:stylesheet xmlns="http://www.odcgroup.com/StructuredProducts"
   ...
   <xsl:template name="code">
      <infra:code>
         <xsl:value-of select=" concat(b:id,$COMPANY_POSTFIX_SEPARATOR,b:mnemonic,$COMPANY_POSTFIX_SEPARATOR,'D') "/>
      </infra:code>
   </xsl:template>
   <xsl:template name="synonym"/>
   <xsl:template name="shortName">
      <infra:shortName>
         <xsl:value-of select="concat(b:id,$COMPANY_POSTFIX_SEPARATOR,b:mnemonic,$COMPANY_POSTFIX_SEPARATOR,'DEPOSIT') "/>
      </infra:shortName>
   </xsl:template>
   ...
   <xsl:template name="accrualRule">
      <accrualRule>
         <xsl:choose>
            <xsl:when test="b:depDayBasis != ' ' and contains('A A1 A2 A3',b:dayBasis)">
               <xsl:value-of select=" '24' "/>
            </xsl:when>
            <xsl:when test="b:depDayBasis = 'B'">
               <xsl:value-of select=" '31' "/>
            </xsl:when>
            <xsl:when test="b:depDayBasis != ' ' and contains('C C1',b:dayBasis)">
               <xsl:value-of select=" '19' "/>
            </xsl:when>
            <xsl:when test="b:depDayBasis = 'C2'">
               <xsl:value-of select=" '20' "/>
            </xsl:when>
            <xsl:when test="b:depDayBasis = 'D'">
               <xsl:value-of select=" '13' "/>
            </xsl:when>
            <xsl:when test="b:depDayBasis = 'E'">
               <xsl:value-of select=" '29' "/>
            </xsl:when>
            <xsl:when test="b:depDayBasis = 'E1'">
               <xsl:value-of select=" '30' "/>
            </xsl:when>
            <xsl:when test="b:depDayBasis != ' ' and contains('F F1 F2',b:dayBasis)">
               <xsl:value-of select=" '26' "/>
            </xsl:when>
            <xsl:when test="b:depDayBasis != ' ' and contains('G S W W1',b:dayBasis)">
               <xsl:value-of select=" '0' "/>
            </xsl:when>
         </xsl:choose>
      </accrualRule>
