    ...
    <xsl:template match="c:aaarecord">
        ...
        <xsl:choose>
            <xsl:when test="(d:activity = 'LENDING-APPLYPAYMENT-PR.REPAYMENT' or d:activity = 'LENDING-APPLYPAYMENT-PR.INTEREST') 
                         and (../c:recaccountdetails/f:actRepayment[starts-with(f:repayReference,$aaArrActivityId)]/f:rpyBillId != '')">
                <xsl:if test="../c:billdetails/j:billProperyDetails[j:property = 'PENALTYINT' and starts-with(j:repayRef,$aaArrActivityId)]">
            ...
            <xsl:when test="d:activity = 'LENDING-APPLYPAYMENT-PR.CURRENT.BALANCES' and $paidBackPenaltyInterest > 0">
                <xsl:if test="not(../c:aatxncontext/b:aaActivityStatus) or ../c:aatxncontext/b:aaActivityStatus != 'REV'">
                    <xsl:copy-of select="."/>
                </xsl:if>
            </xsl:when>
