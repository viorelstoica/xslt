...
<xsl:stylesheet exclude-result-prefixes="b c batch multibatch" version="1.0" xmlns="http://www.odcgroup.com/FiPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/OrderGroupingForSubscriptionRedemption"
                xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchOrderGroupingForSubscriptionRedemption" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiOrderGroupingForSubscriptionRedemption" xmlns:infra="http://www.odcgroup.com/InfraPMS"
                xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiOrderGroupingForSubscriptionRedemption" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- Condition 1 -->
	<xsl:template match="b:OrderGroupingForSubscriptionRedemption">
		<xsl:apply-templates select="self::b:OrderGroupingForSubscriptionRedemption[($g_filterBySubAssetType='Y' and b:subAssetType and contains($list_SubAssetType_OrderGroupingForSubscriptionRedemption,b:subAssetType)) or
									($g_filterBySubAssetType != 'Y' and b:subAssetTypeTapInstrType and b:subAssetTypeTapInstrType='MUTUALFUND')]" mode="filter1"/>
	</xsl:template>
	<!-- Condition 2 -->
	<xsl:template match="b:OrderGroupingForSubscriptionRedemption" mode="filter1">
		<xsl:apply-templates select="self::b:OrderGroupingForSubscriptionRedemption[not(contains(b:id, '*')) or (contains(b:id, '*') and (substring-after(b:id,'*') = $list_transactionCodes_SubscriptionRedemptionRed or substring-after(b:id,'*') = $list_transactionCodes_SubscriptionRedemptionBuy))]" mode="filter-custo"/>
	</xsl:template>
	<!-- Custo placeholder -->
	<xsl:template match="b:OrderGroupingForSubscriptionRedemption" mode="filter-custo">
		<xsl:apply-templates select="self::b:OrderGroupingForSubscriptionRedemption" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:OrderGroupingForSubscriptionRedemption" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="OrderGroupingForSubscriptionRedemption" userelativepaths="yes" externalpreview="no" url="..\..\TestFiles\OrderGroupingForSubscriptionRedemption\FinancialInstruemntMutualFund.xml" htmlbaseurl=""
		          outputurl="..\..\TestFiles\OrderGroupingForSubscriptionRedemption\FinancialInstruemntMutualFundCusto.xml" processortype="jaxp" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline=""
