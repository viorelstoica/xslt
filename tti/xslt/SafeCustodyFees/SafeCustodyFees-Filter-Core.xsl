<?xml version="1.0" encoding="UTF-8"?><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/SafeCustodyFees" xmlns:ns0="http://www.temenos.com/T24/event/Common/EventCommon" xmlns:batch="http://www.temenos.com/T24/event/TTI/BatchSafeCustodyFees" xmlns:c="http://www.temenos.com/T24/event/TTI/MultiSafeCustodyFees" xmlns:infra="http://www.odcgroup.com/InfraPMS" xmlns:multibatch="http://www.temenos.com/T24/event/TTI/BatchMultiSafeCustodyFees" exclude-result-prefixes="b c batch multibatch ns0" version="1.0">
	...
	<xsl:template match="b:SafeCustodyFees">
		<xsl:apply-templates select="self::b:SafeCustodyFees[b:processStage = 'POSTED']" mode="filter-custo"/>
	</xsl:template>

	<!-- Custo placeholder -->
	<xsl:template match="b:SafeCustodyFees" mode="filter-custo">
		<xsl:apply-templates select="self::b:SafeCustodyFees" mode="filter-final"/>
	</xsl:template>

	<!-- Copy block -->
	<xsl:template match="b:SafeCustodyFees" mode="filter-final">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
		</xsl:copy>
	</xsl:template>
...
<metaInformation>
	<scenarios>
		...
		<scenario default="yes" name="ManagementFees" userelativepaths="yes" externalpreview="no" url="..\safe1.xml" htmlbaseurl="" outputurl="..\safefil.xml" processortype="jaxp" useresolver="no" profilemode="0" profiledepth="" profilelength=""
