...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.odcgroup.com/TransactionPMS" xmlns:b="http://www.temenos.com/T24/event/TTI/TransactionCoExpireManualBuy"
	...
	<xsl:template match="c:MultiTransactionCoExpireManualBuy">
		<xsl:apply-templates select="c:CurrentEvent/b:TransactionCoExpireManualBuy"/>
	</xsl:template>
	<xsl:template match="batch:BatchTransactionCoExpireManualBuy">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchTransactionCoExpireManualBuy" name="BatchTransactionCoExpireManualBuy">
			<xsl:apply-templates select="b:TransactionCoExpireManualBuy"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="multibatch:BatchMultiTransactionCoExpireManualBuy">
		<xsl:element xmlns="http://www.temenos.com/T24/event/TTI/BatchMultiTransactionCoExpireManualBuy" name="BatchMultiTransactionCoExpireManualBuy">
			<xsl:apply-templates select="c:MultiTransactionCoExpireManualBuy"/>
		</xsl:element>
	</xsl:template>
	<xsl:template match="b:TransactionCoExpireManualBuy">
		<TransactionPMS mode="INSUPD">
			<xsl:attribute name="output">
				<xsl:value-of select="concat('TransactionCoExpireManualBuy',b:id)"/>
			</xsl:attribute>
			<DataGroup>
				<xsl:for-each select="b:closeoutIdList/b:closeoutId">
					<Derivative>
						<xsl:attribute name="description">
							<xsl:value-of select="concat('TransactionCoExpireManualBuy.xsl+', name(/*), '+', ../../b:eventCommon/ns0:eventId, '+', ../../b:eventCommon/ns0:creationTime)"/>
						</xsl:attribute>
